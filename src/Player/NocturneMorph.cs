...
internal static class NocturneMorph
{
    ...
    private static bool Ready()
    {
        try
        {
            return AmongUsClient.Instance != null && ShipStatus.Instance != null && ((InnerNetClient)AmongUsClient.Instance).AmHost;
        }
        catch
        {
            return false;
    ...
    {
        if (!Ready())
            return NocturneText.T("хост, в матче", "host, in match");
        if (!Valid(victim) || !Valid(target))
    ...
    internal static string IntoAll(PlayerControl target)
    {
        ...
        try
        {
            ...
            while (e.MoveNext())
            {
                ...
                if (Valid(pc) && pc != target)
                    list.Add(pc);
            }
        }
        catch { }
        if (list.Count == 0)
            return NocturneText.T("нет игроков", "no players");
        Run(list, target);
    ...
    internal static string RevertAll()
    {
        ...
        try
        {
            ...
            {
                PlayerControl pc = e.Current;
                if (Valid(pc) && pc.shapeshiftTargetPlayerId != 255)
                    list.Add(pc);
            }
        }
        catch { }
        if (list.Count == 0)
            return NocturneText.T("нет морфов", "no morphs");
        Run(list, null);
    ...
    private static void Run(List<PlayerControl> vics, PlayerControl into)
    {
        try
        {
            ((MonoBehaviour)AmongUsClient.Instance).StartCoroutine(Co(vics, into).WrapToIl2Cpp());
        }
        catch { }
    }
    ...
    private static IEnumerator Co(List<PlayerControl> vics, PlayerControl into)
    {
        ...
        foreach (PlayerControl vic in vics)
        {
            if (!Valid(vic))
                continue;
            if (roles.TryGetValue(vic.PlayerId, out ushort orig) && orig != Shifter)
                SetRole(vic, orig);
        }
    }
    ...
    private static string Nm(PlayerControl p)
    {
        try
        {
            return NocturneNameColor.Strip(p.Data.PlayerName ?? "?").Trim();
        }
        catch
        {
            return "?";
