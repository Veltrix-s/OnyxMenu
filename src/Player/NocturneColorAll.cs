...
internal static class NocturneColorAll
{
    ...
    {
        if (!NocturneConfig.ColorAll.Value)
            return;
        ...
        byte col = (byte)Mathf.Clamp(NocturneConfig.ColorAllId.Value, 0, max);
        ...
        try
        {
            ...
            while (e.MoveNext())
            {
                ...
                if (p == null || p.Data == null || p.Data.Disconnected || p.Data.DefaultOutfit == null)
                    continue;
                if (p.Data.DefaultOutfit.ColorId == col)
                    continue;
                try
                {
                    p.RpcSetColor(col);
