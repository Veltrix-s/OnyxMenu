...
internal static class NocturneNameHistory
{
    ...
    internal static string RecordJoin(ClientData c)
    {
        ...
        if (nick.Length == 0 || nick == "???" || nick == "Игрок")
            return null;
        string fc = SafeFc(c);
        if (fc.Length == 0)
            return null;
        ...
        if (Cache.TryGetValue(key, out Rec r))
        {
            ...
            if (!string.Equals(r.Nick, nick, StringComparison.Ordinal))
            {
                ...
                prev = r.Nick;
                if (!r.Nicks.Contains(nick))
                    r.Nicks.Insert(0, nick);
                r.Nick = nick;
                r.Last = now;
                changed = true;
            }
            if (level != "?" && level != r.Level)
            {
                r.Level = level;
                changed = true;
            }
            if (puid.Length > 0 && puid != r.Puid)
            {
                r.Puid = puid;
                changed = true;
            }
            if (platform != "?" && platform != r.Platform)
            {
                r.Platform = platform;
                changed = true;
            }
            if (raw.Length > 0 && raw != r.Raw)
            {
                r.Raw = raw;
                changed = true;
            }
    ...
    {
        if (string.IsNullOrEmpty(iso) || iso.Length < 10)
            return "?";
    ...
    private static string ResolveNick(ClientData c)
    {
        ...
        try
        {
            if (!string.IsNullOrWhiteSpace(c.PlayerName))
                return NocturneNameColor.Strip(c.PlayerName).Trim();
        }
        catch { }
        return string.Empty;
    }
    ...
    private static string SafeFc(ClientData c)
    {
        try
        {
            return (c.FriendCode ?? string.Empty).Trim();
        }
        catch
        {
            return string.Empty;
    ...
    private static string SafeFc(PlayerControl p)
    {
        try
        {
            return p != null && p.Data != null ? (p.Data.FriendCode ?? string.Empty).Trim() : string.Empty;
        }
        catch
        {
            return string.Empty;
    ...
    private static string SafePuid(ClientData c)
    {
        try
        {
            return (c.ProductUserId ?? string.Empty).Trim();
        }
        catch
        {
            return string.Empty;
    ...
    private static void LoadBlocks(string[] lines)
    {
        ...
        foreach (string raw in lines)
        {
            ...
            if (line.StartsWith("Name: ", StringComparison.Ordinal))
            {
                ...
                if (cur.Nick.Length > 0)
                    cur.Nicks.Add(cur.Nick);
                continue;
            }
            if (cur == null)
                continue;
            if (line.StartsWith("Aliases: ", StringComparison.Ordinal))
            {
                ...
                if (a.Length > 0 && a != "—")
                    ...
                    {
                        string n = part.Trim();
                        if (n.Length > 0 && !cur.Nicks.Contains(n))
                            cur.Nicks.Add(n);
                    }
            }
            else if (line.StartsWith("Level: ", StringComparison.Ordinal))
                cur.Level = line.Substring(7).Trim();
            else if (line.StartsWith("FriendCode: ", StringComparison.Ordinal))
                cur.Fc = line.Substring(12).Trim();
            ...
            else if (line.StartsWith("Platform: ", StringComparison.Ordinal))
            {
                ...
                if (idx >= 0)
                {
                    cur.Platform = v.Substring(0, idx).Trim();
                    cur.Raw = v.Substring(idx + 3).Trim();
                }
                else
                    cur.Platform = v;
            }
            else if (line.StartsWith("First seen: ", StringComparison.Ordinal))
                cur.First = line.Substring(12).Trim();
            else if (line.StartsWith("Last seen: ", StringComparison.Ordinal))
                cur.Last = line.Substring(11).Trim();
        }
        Commit(cur);
    }
    ...
    private static void Commit(Rec r)
    {
        ...
        if (fc.Length == 0)
            return;
        if (r.Nicks == null)
            r.Nicks = new List<string>();
        if (r.Nicks.Count == 0 && !string.IsNullOrEmpty(r.Nick))
            r.Nicks.Add(r.Nick);
        r.Fc = fc;
    ...
    private static void LoadLegacy(string[] lines)
    {
        foreach (string raw in lines)
        {
            ...
            if (nicks.Count == 0 && p[1].Trim().Length > 0)
                nicks.Add(p[1].Trim());
...
public sealed class NocturneHistoryTracker : MonoBehaviour
{
    ...
    public void Update()
    {
        if (!NocturneConfig.NameHistory.Value)
            return;
    ...
    private static void Greet(ClientData c, string since, string oldNick)
    {
        ...
        if (!joinToast)
            NocturneToast.Push(NocturneText.T("Знакомый игрок", "Known player"), detail, 4.5f, NocturneNotifyKind.Warning);
        try
        {
            NocturneEventLog.Add("☺ " + NocturneText.T("Знакомый: ", "Known: ") + detail, NocturneNotifyKind.Warning, NocturneEventCat.Join);
        }
        catch { }
    }
}
