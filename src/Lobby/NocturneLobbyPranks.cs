...
public sealed class NocturneLobbyPranks : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (!live && _scaleMode == 0 && _spinMode == 0 && _animMode == 0)
            return;
        ...
        if (!Utils.Host || PlayerControl.AllPlayerControls == null) return;
    ...
    public void LateUpdate()
    {
        if (_animMode != 1)
            return;
        if (!Utils.InGame || !Utils.Host || PlayerControl.AllPlayerControls == null)
            return;
    ...
    private void ApplyOnce(PlayerControl pc, int colorId)
    {
        ...
        if (_skinCycle)
        {
            try
            {
                pc.RpcSetHat(NocturneOutfits.RandomHat());
            }
            catch { }
            try
            {
                pc.RpcSetSkin(NocturneOutfits.RandomSkin());
            }
            catch { }
            try
            {
                pc.RpcSetVisor(NocturneOutfits.RandomVisor());
            }
            catch { }
        }
    ...
    private void EnsureEggSetup()
    {
        ...
        try
        {
            foreach (PlayerControl pc in PlayerControl.AllPlayerControls)
            {
                ...
                try
                {
                    _originalColors[pc.PlayerId] = pc.Data.DefaultOutfit.ColorId;
                }
                catch { }
                try
                {
                    _originalSkins[pc.PlayerId] = pc.Data.DefaultOutfit.SkinId;
                }
                catch { }
                try
                {
                    _originalHats[pc.PlayerId] = pc.Data.DefaultOutfit.HatId;
                }
                catch { }
                try
                {
                    _originalVisors[pc.PlayerId] = pc.Data.DefaultOutfit.VisorId;
                }
                catch { }
    ...
    private void TeardownEggIfIdle()
    {
        ...
        if (Utils.Host && PlayerControl.AllPlayerControls != null)
        {
            try
            {
                foreach (PlayerControl pc in PlayerControl.AllPlayerControls)
    ...
    internal static string ToggleRainbow()
    {
        ...
        if (!Utils.InGame)
            return NocturneText.T("Только в игре (не в лобби).", "In-game only (not in lobby).");
    ...
    internal static string ToggleSkinCycle()
    {
        ...
        if (!Utils.InGame) return NocturneText.T("Только в игре (не в лобби).", "In-game only (not in lobby).");
    ...
    internal static string MorphAllIntoSelected()
    {
        ...
        if (sel == null || sel.Data == null)
            return NocturneText.T("Цель не выбрана (ЛКМ по игроку).", "No target (LMB a player).");
    ...
    private void LoopTick()
    {
        ...
        PlayerControl t = Utils.ById(_loopId);
