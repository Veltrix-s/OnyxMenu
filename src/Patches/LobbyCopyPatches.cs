...
internal static class NocturneCopyFeedback
{
    ...
    internal static void Apply(LobbyInfoPane pane)
    {
        try
        {
            if (pane == null || !NocturneConfig.LobbyAnims.Value)
                return;
            ...
            if (codeText == null)
                codeText = FindCode((Component)pane);
            if (codeText == null)
                return;
            if (copyBtn == null)
                copyBtn = FindButton((Component)pane, codeText);
    ...
    {
        if (copyBtn == null || (UnityEvent)(object)copyBtn.OnClick == null)
            return;
    ...
    private static void Tick()
    {
        ...
        if (active)
        {
            ...
            if (!string.Equals(raw, want, StringComparison.Ordinal))
                codeText.text = want;
        }
        else
        {
    ...
    private static TMP_Text FindCode(Component root)
    {
        ...
        {
            if (t == null || string.IsNullOrEmpty(Extract(t.text ?? "")))
                continue;
    ...
    private static PassiveButton FindButton(Component root, TMP_Text codeLabel)
    {
        ...
        foreach (PassiveButton b in root.GetComponentsInChildren<PassiveButton>(true))
        {
            ...
            if (name.Contains("copy") || name.Contains("clipboard") || name.Contains("code"))
                score -= 0.5f;
            if (score < bestScore)
            {
                bestScore = score;
