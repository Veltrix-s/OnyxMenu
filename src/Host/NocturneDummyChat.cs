...
internal static class NocturneDummyChat
{
    ...
    private static bool On => NocturneConfig.DummyReportBodies.Value;
    ...
    internal static void OnMurder(PlayerControl killer, PlayerControl victim)
    {
        try
        {
            ...
            {
                if (d == null || d.Data == null || d.Data.IsDead)
                    continue;
                if (((Vector2)d.GetTruePosition() - at).magnitude > SightRange + 1.5f)
                    continue;
    ...
    internal static bool TryReport(PlayerControl d)
    {
        try
        {
            if (!On || d == null || d.Data == null || d.Data.IsDead)
                return false;
            if (Time.time < _reportUntil || MeetingHud.Instance != null)
                return false;
        ...
        try
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (p == null || p.Data == null || p.Data.IsDead)
                    continue;
                if (p.PlayerId == victimId || NocturneDummies.IsDummy(p.PlayerId))
                    continue;
            ...
            if (witness != byte.MaxValue)
                ...
                for (int i = _near.Count - 1; i >= 0; i--)
                    if (!NocturneDummies.IsDummy(_near[i]))
                    {
                        _suspect = _near[i];
                        break;
                    }
    ...
    internal static void TickVision()
    {
        try
        {
            if (!On || MeetingHud.Instance != null || Time.time < _nextScan)
                return;
            ...
            foreach (PlayerControl d in NocturneDummies.Live())
            {
                ...
                {
                    PlayerControl p = ((Il2CppArrayBase<PlayerControl>)(object)all)[i];
                    if (p == null || p.Data == null || p.Data.IsDead || p.PlayerId == d.PlayerId)
                        continue;
    ...
    private static void PickTalkers()
    {
        ...
        try
        {
            ...
            {
                if (pc == null || pc.Data == null || pc.Data.IsDead || pc.PlayerId == _reporter)
                    continue;
    ...
    private static bool VotingPhase(MeetingHud hud)
    {
        try
        {
            return (int)hud.state == 2 || (int)hud.state == 3;
        }
        catch
        {
            return true;
    ...
    {
        if (pc == null || string.IsNullOrEmpty(text))
            return;
        try
        {
            pc.RpcSendChat(text);
    ...
    private static string Name(byte id)
    {
        try
        {
            var d = GameData.Instance.GetPlayerById(id);
            return d != null ? Strip(d.PlayerName) : "?";
        }
        catch
        {
            return "?";
    ...
    private static string Names(List<byte> ids)
    {
        if (ids == null || ids.Count == 0)
            return NocturneText.T("никого", "no one");
        ...
        try
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (r == null || r.roomArea == null)
                    continue;
                if (r.roomArea.OverlapPoint(pos))
                    return RoomName(r.RoomId);
            }
        }
        catch { }
        return "";
    ...
    private static string RoomName(SystemTypes room)
    {
        ...
        {
            case 1:
                return ru ? "Админка" : "Admin";
            case 2:
                return ru ? "Связь" : "Comms";
            case 3:
                return ru ? "Реактор" : "Reactor";
            case 4:
                return ru ? "Электрощит" : "Electrical";
            case 5:
                return ru ? "Навигация" : "Navigation";
            case 6:
                return ru ? "О2" : "O2";
            case 7:
                return ru ? "Щиты" : "Shields";
            case 8:
                return ru ? "Столовая" : "Cafeteria";
            case 9:
                return ru ? "Хранилище" : "Storage";
            case 10:
                return ru ? "Медотсек" : "MedBay";
            case 11:
                return ru ? "Камеры" : "Security";
            case 12:
                return ru ? "Оружейная" : "Weapons";
            case 13:
                return ru ? "Нижний двигатель" : "Lower Engine";
            case 14:
                return ru ? "Связь" : "Comms";
            case 16:
                return ru ? "Запуск" : "Launchpad";
            case 17:
                return ru ? "Верхний двигатель" : "Upper Engine";
            case 19:
                return ru ? "Двигатель" : "Engine Room";
            case 21:
                return ru ? "Реактор" : "Reactor";
            case 24:
                return ru ? "Кабина" : "Cockpit";
            case 25:
                return ru ? "Архив" : "Records";
            default:
                return ru ? "коридор" : "hallway";
        }
    }
}
