...
internal static class NocturneVentKick
{
        ...
        try
        {
            ...
            foreach (PlayerControl pc in PlayerControl.AllPlayerControls)
            {
                ...
                if (pc == PlayerControl.LocalPlayer)
                    continue;
                if (net != null && pc.OwnerId == net.HostId)
                    continue;
    ...
    internal static string KickSelected()
    {
        ...
        foreach (byte pid in _selected)
            _pending.Enqueue(pid);
        _selected.Clear();
    ...
    private static PlayerControl ById(byte pid)
    {
        try
        {
            ...
            while (e.MoveNext())
                if (e.Current != null && e.Current.PlayerId == pid)
                    return e.Current;
        }
        catch { }
        return null;
    ...
    internal static string Kick(PlayerControl target)
    {
        ...
        if (target.OwnerId == net.HostId)
            return NocturneText.T("хоста так не выкинуть", "can't kick the host this way");
        ...
        if (!HasAnticheat())
            return NocturneText.T("нужен официальный сервер", "needs an official server");
