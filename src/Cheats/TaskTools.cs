...
internal static class TaskTools
{
    private static bool Ready(PlayerControl pc)
        => Utils.Host && ShipStatus.Instance != null && pc != null && pc.Data != null;

    private static void Send(PlayerControl pc, byte[] ids)
    {
        try
        {
            pc.Data.RpcSetTasks((Il2CppStructArray<byte>)ids);
        }
        catch { }
    }
    ...
    {
        if (!Ready(pc))
            return NocturneText.T("Только хост, в матче.", "Host only, in match.");
    ...
    internal static string Normal(PlayerControl pc)
    {
        ...
        if (ids.Count == 0)
            return NocturneText.T("Нет заданий на карте.", "No tasks on this map.");
        Send(pc, ids.ToArray());
