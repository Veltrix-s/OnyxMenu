...
public sealed class NocturneMenu : MonoBehaviour
{
    ...
    public void Update()
    {
        bool menuKey = NocturneConfig.MenuKey != null && NocturneConfig.MenuKey.Value != KeyCode.None && Input.GetKeyDown(NocturneConfig.MenuKey.Value);
    ...
    internal void DrawGui()
    {
        ...
        float userS = NocturneConfig.MenuScale != null ? NocturneConfig.MenuScale.Value : 1f;
    ...
    private void DrawWindow(int id)
    {
        ...
        if (_collapsed) NocturneStyle.FillRounded(new Rect(0f, 0f, w, h), p.Window, 16);
        else NocturneStyle.DrawTex(new Rect(0f, 0f, w, h), _gradient);
        NocturneStyle.FillRounded(new Rect(0f, 0f, w, h), A(p.Accent, 0.04f), 16);
        NocturneStyle.StrokeRounded(new Rect(0f, 0f, w, h), A(p.Accent, 0.22f), 16, 1);
        if (!_collapsed)
            NocturneStyle.DrawTex(new Rect(2f, 2f, w - 4f, HeaderH - 3f), _gloss);
        NocturneStyle.Fill(new Rect(16f, 1f, w - 32f, 1f), A(Color.white, 0.11f));
        ...
        NocturneStyle.FillRounded(new Rect(8f, sbTop, SidebarW - 14f, sbBot - sbTop), A(Color.black, 0.14f), 12);
        NocturneStyle.Fill(new Rect(10f, sbTop + 1f, SidebarW - 18f, 1f), A(Color.white, 0.05f));
        NocturneStyle.Fill(new Rect(SidebarW - 2f, HeaderH + 12f, 1f, h - HeaderH - FooterH - 24f), A(Color.white, 0.06f));
        ...
        float tabTop = HeaderH + 14f;
        float tabAvail = (h - FooterH - 2f) - tabTop - 6f;
        float tabStep = Mathf.Min(TabH + 4f, tabAvail / (Tabs.Length + 1));
        float th = Mathf.Min(TabH, tabStep - 3f);
        float settingsY = Mathf.Max(tabTop + Tabs.Length * tabStep, h - FooterH - th - 8f);
        var settingsRect = new Rect(14f, settingsY, SidebarW - 26f, th);
        Rect activeRect = _tab == 9 ? settingsRect : new Rect(14f, tabTop + _tab * tabStep, SidebarW - 26f, th);
        ...
        {
            DrawTab(new Rect(14f, ty, SidebarW - 26f, th), Tabs[i].Icon, Tabs[i].Name, i);
            ty += tabStep;
        }
        DrawTab(settingsRect, NocturneIcon.Gear, NocturneText.T("Настройки", "Settings"), 9);

        DrawSearchBar(new Rect(SidebarW, HeaderH + 8f, w - SidebarW - 16f - 34f, 26f));
        DrawCollapseAll(new Rect(w - 16f - 30f, HeaderH + 8f, 30f, 26f));
        ...
        if (se != null)
        {
            ...
            else if (se.type == EventType.MouseDrag && _scrollGrab && _slider == 0)
            {
                ...
                if (_scrollMoved) { _scroll -= dy; se.Use(); }
            }
        ...
        NocturneStyle.Fill(new Rect(SidebarW, h - FooterH, w - SidebarW - 16f, 1f), A(Color.white, 0.06f));
        string acc = Hex(p.Accent);
        GUI.Label(new Rect(x, fy, cw, FooterH - 8f),
            $"{NocturneText.T("Меню", "Menu")} <color=#{acc}><b>{KeyName(NocturneConfig.MenuKey)}</b></color>     {NocturneText.T("Код лобби", "Lobby code")} <color=#{acc}><b>{KeyName(NocturneConfig.CopyCodeKey)}</b></color>     {NocturneText.T("Закрыть", "Close")} <color=#{acc}><b>Esc</b></color>", _footer);
    ...
    {
        NocturnePalette p = NocturneStyle.Current;
        NocturneStyle.FillRounded(r, A(p.Accent, 0.18f), 11);
        NocturneStyle.StrokeRounded(r, A(p.Accent, 0.38f), 11, 1);
        NocturneStyle.Fill(new Rect(r.x + 12f, r.y + 1f, r.width - 24f, 1f), A(Color.white, 0.08f));
        NocturneStyle.FillRounded(new Rect(r.x + 3f, r.y + 8f, 3f, r.height - 16f), p.Accent, 2);
    }
    ...
    private void DrawTab(Rect r, NocturneIcon icon, string label, int index)
    {
        ...
        if (!active && hover)
        {
            NocturneStyle.FillRounded(r, A(Color.white, 0.05f), 11);
            NocturneStyle.StrokeRounded(r, A(Color.white, 0.06f), 11, 1);
        }

        Color ink = active ? p.Accent : (hover ? p.Text : p.Muted);
        if (active)
            NocturneStyle.FillRounded(new Rect(r.x + 9f, r.y + r.height / 2f - 14f, 28f, 28f), A(p.Accent, 0.16f), 14);
        NocturneIcons.Draw(icon, new Rect(r.x + 14f, r.y + r.height / 2f - 9f, 18f, 18f), ink);
    ...
    private bool Cull(float y, float h) => y + h < -4f || y > _viewH + 4f;
    ...
    private Rect Card(float x, ref float y, float w, string title, float bodyH)
    {
        ...
        var card = new Rect(x, y, w, head + shownBody + 14f);
        ...
        if (!Cull(y, card.height))
        {
            ...
            if (hover) NocturneStyle.FillRounded(new Rect(card.x - 2f, card.y + 4f, card.width + 4f, card.height), A(p.Accent, 0.10f), 14);
            NocturneStyle.FillRounded(card, p.Panel, 12);
            if (hover) NocturneStyle.FillRounded(card, A(Color.white, 0.045f), 12);
            NocturneStyle.StrokeRounded(card, hover ? A(p.Accent, 0.30f) : A(Color.white, 0.08f), 12, 1);
            NocturneStyle.Fill(new Rect(card.x + 12f, card.y + 1f, card.width - 24f, 1f), A(Color.white, 0.10f));
            NocturneStyle.Fill(new Rect(card.x + 12f, card.yMax - 2f, card.width - 24f, 1f), A(Color.black, 0.20f));

            NocturneStyle.FillRounded(new Rect(card.x + 18f, card.y + head / 2f - 7f, 3f, 14f), p.Accent, 2);
            GUI.Label(new Rect(card.x + 32f, card.y, card.width - 70f, head), title.ToUpperInvariant(), _cardTitle);
            GUI.Label(new Rect(card.xMax - 34f, card.y, 22f, head), collapsed ? "▸" : "▾", _centerMuted);
            if (!collapsed) NocturneStyle.Fill(new Rect(card.x + 18f, card.y + head - 1f, card.width - 36f, 1f), A(Color.white, 0.05f));
        }

        y = card.yMax + 14f;
        return collapsed
            ? new Rect(-20000f, -20000f, card.width - 36f, bodyH)
            : new Rect(card.x + 18f, card.y + head + 8f, card.width - 36f, bodyH);
    }
    ...
    private void DrawGuard(float x, ref float y, float w)
    {
        ...
        b = Card(x, ref y, w, NocturneText.T("Детект входящих", "Join detect"), RowH + 26f);
        by = b.y;
    ...
    private void KillRow(float x, ref float y, float w, PlayerControl pc)
    {
        ...
        string nm = pc.Data != null ? pc.Data.PlayerName : "?";
        if (pc.Data != null && pc.Data.IsDead) nm += "  <size=80%>†</size>";
        GUI.Label(new Rect(r.x + 26f, r.y, r.width - 246f, r.height), nm, _rowName);
        if (SmallButton(new Rect(r.xMax - 240f, r.y + 3f, 70f, 24f), NocturneText.T("КИЛЛ", "KILL"), new Color(0.9f, 0.4f, 0.4f)))
            NocturneToast.Push(NocturneText.T("Килл", "Kill"), NocturneKillTools.KillOne(pc), 2f, NocturneNotifyKind.Info);
        if (SmallButton(new Rect(r.xMax - 166f, r.y + 3f, 74f, 24f), NocturneText.T("ТЕЛЕ", "TELE"), new Color(0.78f, 0.42f, 0.95f)))
            NocturneToast.Push(NocturneText.T("Телекилл", "Telekill"), NocturneKillTools.Telekill(pc), 2f, NocturneNotifyKind.Info);
        if (SmallButton(new Rect(r.xMax - 88f, r.y + 3f, 84f, 24f), NocturneText.T("ВЫГНАТЬ", "EJECT"), new Color(0.5f, 0.78f, 0.92f)))
    ...
    private void DrawHome(float x, ref float y, float w)
    {
        ...
        b = Card(x, ref y, w, NocturneText.T("Ссылки", "Links"), 2f * RowH);
        by = b.y;
    ...
    private void DrawHostTab(float x, ref float y, float w)
    {
        ...
        if (!NocturneLobbySettings.Ready())
        {
    ...
    private void DrawCheats(float x, ref float y, float w)
    {
        ...
        b = Card(x, ref y, w, NocturneText.T("Рофл: цикл смерти", "Prank: murder loop"), inMatch ? 38f + (loopN > 0 ? loopN * 32f : 26f) : 26f);
        by = b.y;
        if (!inMatch)
        ...
        b = Card(x, ref y, w, NocturneText.T("Бафы ролей", "Role buffs"), 38f + RowH * 26f + 208f + (reach ? 54f : 4f));
        by = b.y;
        ...
        b = Card(x, ref y, w, NocturneText.T("Отправить в чат", "Chat sender"), 3f * RowH + 34f);
        by = b.y;
    ...
    private void DrawNet(float x, ref float y, float w)
    {
        ...
        Rect b = Card(x, ref y, w, NocturneText.T("Соединение", "Connection"), 3f * 30f);
        float by = b.y;
    ...
    {
        Rect b = Card(x, ref y, w, NocturneText.T("Косметика", "Cosmetics"), 4f * RowH + 6f);
        float by = b.y;
        ...
        float espH = 8f * RowH
            + (NocturneConfig.EspBoxes.Value ? 2f * RowH : 0f)
        ...
        b = Card(x, ref y, w, NocturneText.T("Анимации (на себе)", "Animations (self)"), 6f * 28f + 24f);
        by = b.y;
    ...
    {
        ConfigEntry<string> slot = NocturneConfig.FavoriteOutfits != null && i < NocturneConfig.FavoriteOutfits.Length ? NocturneConfig.FavoriteOutfits[i] : null;
    ...
    {
        float gw = w - 56f;
        Rect b = Card(x, ref y, w, NocturneText.T("Интерфейс", "Interface"), RowH + 50f + 30f + ThemeGridHeight(gw));
        float by = b.y;
        ...
        b = Card(x, ref y, w, NocturneText.T("Клавиши читов", "Cheat keys"), 13f * RowH);
        by = b.y;
    ...
    private void KeyRow(float x, ref float y, float w, string label, ConfigEntry<KeyCode> entry)
    {
        ...
        string txt = listening ? NocturneText.T("Нажми клавишу…", "Press a key…") : (entry.Value == KeyCode.None ? "—" : entry.Value.ToString());
    ...
    private void Build()
    {
        int ti = NocturneConfig.ThemeIndex != null ? NocturneConfig.ThemeIndex.Value : 0;
