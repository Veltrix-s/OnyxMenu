...
internal static class NocturneLobbyAnim
{
    ...
    internal static void Apply(GameStartManager m)
    {
        ...
        if (el <= PanelDelay + PanelDuration + 0.2f)
            AnimatePanel(m, el);
    ...
    private static void Arm(GameStartManager m)
    {
        ...
        if (mgrId == id && gameId == gid && introAt >= 0f)
            return;
        mgrId = id;
        gameId = gid;
    ...
    {
        if (t != null && ids.Add(t.GetInstanceID()))
            targets.Add(t);
    }
...
internal static class NocturneActionHover
{
    ...
    internal static void Apply(GameStartManager m)
    {
        ...
        if (!Vis(hostView) && !Vis(clientView) && !Vis(edit) && !anyClick)
            return;
    ...
    {
        int id = b.GetInstanceID();
        if (colliders.TryGetValue(id, out Collider2D c))
            return c;
