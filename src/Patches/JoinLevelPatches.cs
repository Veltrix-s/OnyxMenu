...
internal static class NocturneJoinLevels
{
    ...
    internal static void Remember(PlayerControl player, uint raw)
    {
        ...
        try
        {
            ...
            if (c != null && c.Id >= 0) { ByClientId[c.Id] = raw; PushToClient(c.Id, raw); }
        }
        catch { }
    }
    ...
    {
        try { if (player != null && player.Data != null && !player.Data.IsIncomplete && ValidRaw(player.Data.PlayerLevel)) Remember(player, player.Data.PlayerLevel); }
        catch { }
    }
...
internal static class NocturneLevelJoinPushPatch
{
    ...
    {
        if (client == null || client.Id < 0 || client.PlayerLevel > 0u) return;
        try
        {
