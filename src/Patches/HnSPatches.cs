...
internal static class HnSSeekers
{
    ...
    {
        if (!Enabled || !IsHnS() || !IsHost() || (int)team != 1)
            return HarmonyControl.Continue;
    ...
    private static void SetHnSImpostorCount(int count)
    {
        try
        {
            if (GameOptionsManager.Instance == null || GameOptionsManager.Instance.CurrentGameOptions == null)
                return;
            HideNSeekGameOptionsV11 options = ((Il2CppObjectBase)GameOptionsManager.Instance.CurrentGameOptions).Cast<HideNSeekGameOptionsV11>();
            if (options != null)
                options.NumImpostors = count;
        }
        catch { }
    }
...
internal static class HnSRoleSelectionPatch
{
    public static bool Prefix(Il2CppSystem.Collections.Generic.List<NetworkedPlayerInfo> players, IGameOptions opts, RoleTeamTypes team, ref int teamMax)
    {
        try
        {
            return HnSSeekers.TryAssignSeekers(players, opts, team, ref teamMax);
        }
        catch
        {
            return HarmonyControl.Continue;
...
internal static class HnSLeadTimePatch
{
    ...
    {
        if (NocturneConfig.SeekerInstantStart.Value)
            __result = 0;
    }
}
