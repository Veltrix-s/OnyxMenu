...
internal static class NocturneKillTools
{
    ...
    private static bool Imp(PlayerControl pc)
    {
        try
        {
            return pc.Data != null && pc.Data.Role != null && pc.Data.Role.IsImpostor;
        }
        catch
        {
            return false;
    ...
    internal static string Telekill(PlayerControl t)
    {
        if (!Ready(out PlayerControl me))
            return Deny();
        if (!Valid(t) || t == me || me.NetTransform == null)
            return NocturneText.T("Нет цели.", "No target.");
        Vector2 back = me.GetTruePosition();
        try
        {
            me.NetTransform.SnapTo(t.GetTruePosition());
        }
        catch { }
        Murder(me, t);
        try
        {
            me.NetTransform.RpcSnapTo(back);
        }
        catch { }
