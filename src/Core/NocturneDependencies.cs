...
internal static class NocturneDependencies
{
    ...
    internal static void FlushLog()
    {
        ...
        if (log == null)
            return;
        for (int i = 0; i < _pendingInfo.Count; i++)
            log.LogInfo((object)_pendingInfo[i]);
        for (int i = 0; i < _pendingWarn.Count; i++)
            log.LogWarning((object)_pendingWarn[i]);
        _pendingInfo.Clear();
        _pendingWarn.Clear();
    }
...
#pragma warning restore SYSLIB0012
    ...
    private static Assembly OnResolve(object sender, ResolveEventArgs args)
    {
        ...
        foreach (string lib in Libs)
        {
            if (!string.Equals(lib, name, StringComparison.OrdinalIgnoreCase))
                continue;
            if (!string.IsNullOrEmpty(_pluginDir))
            {
