...
internal static class VisualAssist
{
    ...
    private static void PlaceChatButton(HudManager hud)
    {
        try
        {
            ...
            if (!inMatch)
            {
                if (_placed)
                {
                    ...
                    try
                    {
                        chatAp.DistanceFromEdge = _origEdge;
                        chatAp.AdjustPosition();
                    }
                    catch { }
                }
                return;
            ...
            int chatId = ((UnityEngine.Object)chatAp).GetInstanceID();

            var aps = ((Component)hud).GetComponentsInChildren<AspectPosition>(true);
            ...
            for (int i = 0; i < aps.Length; i++)
            {
                ...
                if (ap == null || ((UnityEngine.Object)ap).GetInstanceID() == chatId)
                    continue;
                if (ap.Alignment != chatAp.Alignment)
                    continue;
            ...
            if (minGap < float.PositiveInfinity && minGap >= 0.25f)
                step = minGap;
            ...
            if (!_placed)
            {
                _origEdge = chatAp.DistanceFromEdge;
                _placed = true;
            }

            if (Mathf.Abs(chatAp.DistanceFromEdge.x - want.x) < 1e-4f)
                return;
    ...
    private static bool VanillaPanelOpen()
    {
        try
        {
            ...
            if (_optMenu != null && ((Component)_optMenu).gameObject.activeInHierarchy)
                return true;

            if (_gameMenu == null && scan)
                _gameMenu = UnityEngine.Object.FindObjectOfType<GameSettingMenu>();
            if (_gameMenu != null && ((Component)_gameMenu).gameObject.activeInHierarchy)
                return true;

            if (_custMenu == null && scan)
                _custMenu = UnityEngine.Object.FindObjectOfType<PlayerCustomizationMenu>();
            if (_custMenu != null && ((Component)_custMenu).gameObject.activeInHierarchy)
                return true;

            if (_friendsPanel == null && scan)
                _friendsPanel = UnityEngine.Object.FindObjectOfType<FriendsListUI>();
            if (_friendsPanel != null && ((Component)_friendsPanel).gameObject.activeInHierarchy)
                return true;
        }
        catch { }
        return false;
    ...
    internal static void UpdatePlayerName(PlayerPhysics physics)
    {
        ...
        try
        {
            ...
            if (LocalColored(player) && NocturneConfig.NameColorAnimated.Value)
                needRebuild = true;
    ...
    private static string BuildUnmaskLine(PlayerControl player)
    {
        try
        {
            if (player == PlayerControl.LocalPlayer || player.Data == null)
                return string.Empty;
            if ((int)player.CurrentOutfitType == 0)
                return string.Empty;
    ...
    private static Color RoleColor(int roleId, Color fallback)
    {
        ...
        {
            case 0:
                return new Color(0.70f, 0.95f, 1f);
            case 1:
                return new Color(1f, 0.25f, 0.25f);
            case 2:
                return new Color(0.45f, 0.70f, 1f);
            case 3:
                return new Color(0.35f, 1f, 0.80f);
            case 4:
                return new Color(0.85f, 0.90f, 1f);
            case 5:
                return new Color(1f, 0.60f, 0.15f);
            case 8:
                return new Color(1f, 0.50f, 0.80f);
            case 9:
                return new Color(0.80f, 0.20f, 0.25f);
            case 10:
                return new Color(0.60f, 0.50f, 1f);
            case 12:
                return new Color(0.90f, 0.85f, 0.45f);
            case 18:
                return new Color(0.75f, 1f, 0.30f);
            case 19:
