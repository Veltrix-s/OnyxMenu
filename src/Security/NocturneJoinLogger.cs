...
public sealed class NocturneJoinLogger : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        while (e.MoveNext())
        {
            ...
            try
            {
                Patches.NocturneJoinLevels.RememberCurrent(c.Character);
            }
            catch { }
            ...
            if (_seen.Contains(c.Id))
                continue;
            if (c.Id == net.ClientId)
            {
                _seen.Add(c.Id);
                continue;
            }

            if (!_pending.ContainsKey(c.Id))
            {
                _pending[c.Id] = now;
                _recheck[c.Id] = now + JoinSettle;
                continue;
            }
            if (_recheck.TryGetValue(c.Id, out float at) && now < at) continue;
            if (!Patches.NocturneJoinLevels.Ready(c) && now - _pending[c.Id] < JoinMaxWait)
            {
                _recheck[c.Id] = now + JoinRetry;
                continue;
            }
    ...
    private void Forget()
    {
        if (_seen.Count == 0 && _pending.Count == 0 && !_primed)
            return;
        _seen.Clear();
        _pending.Clear();
        _recheck.Clear();
    ...
    private static void Log(ClientData c)
    {
        ...
        try
        {
            fc = c.FriendCode ?? string.Empty;
        }
        catch { }
        try
        {
            puid = c.ProductUserId ?? string.Empty;
        }
        catch { }
