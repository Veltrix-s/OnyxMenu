...
internal static class NocturneHostOptions
{
    ...
    internal static bool TryImpostorCount(ref int count)
    {
        ...
        if (!Loose || mgr == null || mgr.CurrentGameOptions == null)
            return HarmonyControl.Continue;
    ...
    private static bool IsRoleOption(NumberOption option)
    {
        try
        {
            return ((OptionBehaviour)option).GetComponentInParent<RolesSettingsMenu>() != null;
        }
        catch
        {
            return false;
    ...
    private static void RelaxData(NumberOption option)
    {
        try
        {
            ...
            if (f != null)
            {
                f.ValidRange = new FloatRange(Floor, 999999999f);
                return;
            }
            IntGameSetting i = ((Il2CppObjectBase)(object)data).TryCast<IntGameSetting>();
            if (i != null)
                i.ValidRange = new IntRange(0, 999999999);
        }
        catch { }
    }
...
internal static class OptionSyncPatch
{
    public static bool Prefix()
    {
        try
        {
            return NocturneHostOptions.AllowSync();
        }
        catch
        {
            return HarmonyControl.Continue;
