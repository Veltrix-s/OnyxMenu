...
public sealed class NocturneMenu : MonoBehaviour
{
    ...
    {
        if (!byte.TryParse((idText ?? "").Trim(), out byte pid))
            return null;
    ...
    internal void DrawGui()
    {
        ...
        float userS = Mobile ? NocturneConfig.MenuScale.Value : 1f;
    ...
    private void DrawWindow(int id)
    {
        ...
        if (lite || _collapsed)
            ...
            NocturneStyle.DrawTex(new Rect(0f, 0f, w, h), _gradient);
        if (!lite)
        ...
        if (Time.unscaledTime > _nextIndexFlush)
        {
            _nextIndexFlush = Time.unscaledTime + 2f;
        ...
        int accSig = ((int)(p.Accent.r * 255f) << 16) | ((int)(p.Accent.g * 255f) << 8) | (int)(p.Accent.b * 255f);
        int fsig = (int)NocturneConfig.MenuKey.Value * 397 + (int)NocturneConfig.CopyCodeKey.Value * 17 + (NocturneText.IsRussian ? 3 : 0) + accSig;
    ...
    {
        if (Time.unscaledTime - _statAt < 0.3f)
            return;
        ...
        try
        {
            ...
            if (net != null && net.GameId != 0 && (LobbyBehaviour.Instance != null || ShipStatus.Instance != null))
            {
                ...
                if (GameData.Instance != null)
                    alive = GameData.Instance.PlayerCount;
    ...
    private static string FavKey(ConfigEntryBase e)
    {
        try
        {
            return e == null ? null : e.Definition.Section + "/" + e.Definition.Key;
        }
        catch
        {
            return null;
    ...
    private Rect Card(float x, ref float y, float w, string title, float bodyH)
    {
        ...
        if (!Cull(y, card.height))
        {
            ...
            if (_curCard == _flashKey && Time.unscaledTime < _flashUntil)
                NocturneStyle.StrokeRounded(card, A(p.Accent, 0.7f * Mathf.Clamp01((_flashUntil - Time.unscaledTime) / 1.6f)), 10, 2);
        }
    ...
    {
        bool ready = NocturneShield.Host() && ShipStatus.Instance != null && LobbyBehaviour.Instance == null;
        ...
        Rect b = Card(x, ref y, w, NocturneText.T("Щит игрокам (хост)", "Shield players (host)"),
            ready ? 28f + 30f + RowH + (n > 0 ? n * 32f : 26f) : 26f);
        float by = b.y;

        if (!ready)
        {
            GUI.Label(new Rect(b.x + 2f, by, b.width - 2f, 24f),
                NocturneShield.Host() ? NocturneText.T("Только в матче.", "In match only.") : NocturneText.T("Только хост.", "Host only."), _muted);
            return;
        }
    ...
    private void DrawQoL(float x, ref float y, float w)
    {
        ...
        for (int i = 0; i < _roleClients.Count; i++)
            if (_roleClients[i] != PlayerControl.LocalPlayer)
                wsCount++;
        Grp(1);
    ...
    private void DrawHostTab(float x, ref float y, float w)
    {
        ...
        string[] ventModes =
        {
            NocturneText.T("Все", "Everyone"), NocturneText.T("Только мирные", "Crew only"),
            NocturneText.T("Только преды", "Impostors only"), NocturneText.T("Никто", "Nobody")
        };
        int vm = Mathf.Clamp(NocturneConfig.VentMode.Value, 0, 3);
        CycleRow(rb.x, ref rby, rb.width, NocturneText.T("Кто может вентить", "Who may vent"), ventModes[vm], () => NocturneConfig.VentMode.Value = (vm + 1) % 4);
    ...
    private void DrawNet(float x, ref float y, float w)
    {
        ...
        if (brOn)
        {
            ...
            GUI.Label(new Rect(b.x + 2f, by, b.width - 2f, 24f), NocturneText.T("Пусто = судить по уровню. Nocturne/GlichRooms.txt", "Empty = judge by level. Nocturne/GlichRooms.txt"), _muted);
    }
    ...
    private void DrawVisual(float x, ref float y, float w)
    {
        ...
        b = Card(x, ref y, w, NocturneText.T("Избранные образы", "Favorite outfits"), 4f * 30f + 60f + pickH);
        by = b.y;
        for (int i = 0; i < 4; i++)
            FavoriteRow(b.x, ref by, b.width, i);
        if (picking)
        {
            GUI.Label(new Rect(b.x + 2f, by, b.width - 2f, 22f), NocturneText.T($"Взять образ в слот {_outfitPickSlot + 1}:", $"Copy outfit into slot {_outfitPickSlot + 1}:"), _muted);
            by += 24f;
            if (outfitN == 0)
            {
                GUI.Label(new Rect(b.x + 2f, by, b.width - 2f, 24f), NocturneText.T("Нет других игроков.", "No other players."), _muted);
        ...
        by += 28f;
        GUI.Label(new Rect(b.x + 2f, by, b.width - 2f, 22f), NocturneText.T("«ВЫБР.» — выбрать из списка. Кнопки снизу — цель мышью.", "'SEL.' — pick from list. Buttons below use mouse target."), _muted);
        ...
        GUI.Label(new Rect(b.x + 2f, by, b.width - 2f, 22f), NocturneText.T("Взять образ игрока на себя (временно):", "Borrow a player's outfit (temporary):"), _muted);
        by += 24f;
        if (qoN == 0)
        {
            GUI.Label(new Rect(b.x + 2f, by, b.width - 2f, 24f), NocturneText.T("Нет других игроков.", "No other players."), _muted);
    ...
    private void OutfitPickRow(float x, ref float y, float w, PlayerControl pc)
    {
        ...
        int colorId = pc.Data != null && pc.Data.DefaultOutfit != null ? pc.Data.DefaultOutfit.ColorId : 0;
        DrawColorDot(new Rect(r.x + 6f, r.y + 9f, 12f, 12f), colorId);
        GUI.Label(new Rect(r.x + 26f, r.y, r.width - 110f, r.height), pc.Data != null ? pc.Data.PlayerName : "?", _rowName);
    ...
    private void DrawSettings(float x, ref float y, float w)
    {
        ...
        float scaleH = Mobile ? 50f : 0f;
        float accentExtra = RowH + (NocturneConfig.AccentCustom.Value ? 44f + 2f * 50f + 6f : 0f);
        Grp(0);
        Rect b = Card(x, ref y, w, NocturneText.T("Интерфейс", "Interface"), 2f * RowH + scaleH + 30f + gridH + accentExtra);
        float by = b.y;
    ...
    private void ActionRow(Rect body, NocturneIcon icon, string label, string btn, Action onClick)
    {
        ...
        if (GUI.Button(bt, GUIContent.none, _invisible))
            onClick();
    }
    ...
    {
        GUI.Label(new Rect(x + 12f, y, w - 24f, 20f), NocturneText.T("Оттенок акцента", "Accent hue"), _muted);
        y += 22f;
    ...
    private void DrawSearch(float x, ref float y, float w)
    {
        ...
        Rect b = Card(x, ref y, w, NocturneText.T("Фичи", "Features") + "  (" + n + ")", n > 0 ? n * RowH + 6f : 30f);
        float by = b.y;
        if (n == 0)
        ...
        float by2 = b2.y;
        if (m == 0)
            GUI.Label(new Rect(b2.x + 2f, by2, b2.width - 2f, 24f), NocturneText.T("Пусто. Открой вкладки один раз, чтобы проиндексировать.", "Empty. Open the tabs once to index them."), _muted);
        else
            for (int i = 0; i < m; i++)
    ...
    private void SearchCardRow(float x, ref float y, float w, SearchCard c)
    {
        ...
        var r = new Rect(x, y, w, RowH - 2f);
        HoverFill(r);
        GUI.Label(new Rect(r.x + 10f, r.y, r.width - 190f, r.height), c.Title, _rowName);
        GUI.Label(new Rect(r.xMax - 178f, r.y, 82f, r.height), TabName(c.Tab), _muted);
    ...
    private void CycleRow(float x, ref float y, float w, string label, string valueText, Action onClick)
    {
        ...
        if (NocturneStyle.Lite)
        {
            if (GUI.Button(new Rect(x + 4f, y, w - 8f, RowH - 4f), label + ":  " + valueText))
                onClick();
            y += RowH;
            return;
        }
    ...
    private void DrawPill(Rect track, bool on, object key)
    {
        ...
        if (key != null)
        {
            if (!_pillAnim.TryGetValue(key, out a))
                a = target;
    ...
    private string CustomText(Rect r, string text, string key)
    {
        ...
        if (focused && e != null && e.type == EventType.KeyDown)
        {
            ...
            else if (e.character != '\0' && !char.IsControl(e.character) && text.Length < 24)
            {
                text += e.character;
                e.Use();
            }
