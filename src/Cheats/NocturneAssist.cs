...
internal static class ConsoleReachPatch
{
    ...
    {
        if (!NocturneConfig.ConsoleReach.Value || __instance == null || !couldUse || canUse)
            return;
        PlayerControl me = PlayerControl.LocalPlayer;
        if (me == null || pc == null || pc.PlayerId != me.PlayerId)
            return;

        float d = Vector2.Distance(me.GetTruePosition(), (Vector2)__instance.transform.position);
        if (d > NocturneConfig.ConsoleDist.Value)
            return;
        canUse = true;
        __result = d;
    }
...
internal static class SkipDeconPatch
{
    ...
    {
        if (!NocturneConfig.SkipDecon.Value || __instance == null)
            return;
        try
        {
            if (__instance.UpperDoor != null)
                __instance.UpperDoor.SetDoorway(true);
        }
        catch { }
        try
        {
...
internal static class AirshipSpawnPatch
{
    ...
    {
        if (!NocturneConfig.AirshipSpawn.Value || __instance == null)
            return;
        try
        {
