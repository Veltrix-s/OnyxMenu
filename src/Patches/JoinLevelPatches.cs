...
internal static class NocturneJoinLevels
{
    ...
    internal static bool TryRaw(int clientId, PlayerControl pc, out uint raw)
    {
        ...
        if (TryCache(pc, out raw))
            return true;
        if (clientId >= 0 && ByClientId.TryGetValue(clientId, out raw))
            return true;
    ...
    internal static string Display(ClientData c)
    {
        ...
        if (TryGet(c.Id, out uint raw))
            return (raw + 1u).ToString();
        try
        {
            if (c.PlayerLevel > 0u && ValidRaw(c.PlayerLevel))
            {
...
internal static class NocturneLevelJoinPushPatch
{
    public static void Postfix([HarmonyArgument(0)] ClientData client)
    {
        ...
        try
        {
            if (NocturneJoinLevels.TryGet(client.Id, out uint raw) && raw > 0u)
                client.PlayerLevel = raw;
        }
        catch { }
    }
}
