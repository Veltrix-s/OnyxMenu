...
internal static class NocturneMainArt
{
    ...
    private static void BindVideoTexture(SpriteRenderer sr)
    {
        ...
        {
            NocturnePlugin.Logger?.LogWarning((object)("[MainArt] видео-текстура: " + e.Message));
        }
    }
    ...
    private static Sprite Canvas16x9()
    {
        ...
        Sprite sp = Sprite.Create(tex, new Rect(0f, 0f, 16f, 9f), new Vector2(0.5f, 0.5f));
    ...
    private static void Fit()
    {
        ...
        {
            NocturnePlugin.Logger?.LogWarning((object)"[MainArt] видео не запустилось, беру картинку");
            _vidFailed = true;
            Reload();
            return;
        }
        ...
        if (_vidAspect > 0f && _vid != null && _vid.isPrepared && _vid.height > 0u)
        {
            ...
            if (Mathf.Abs(a - _vidAspect) > 0.01f)
            {
                _vidAspect = a;
                sz = new Vector2(sz.y * a, sz.y);
            }
        }

        float k = Mathf.Max(w / Mathf.Max(0.001f, sz.x), h / Mathf.Max(0.001f, sz.y));
        _back.transform.localPosition = new Vector3(0f, 0f, 20f);
        _back.transform.localScale = new Vector3(k, k, 1f);
    }
    ...
    private static bool StartVideo(string file)
    {
        ...
        {
            NocturnePlugin.Logger?.LogWarning((object)("[MainArt] видео: " + e.Message));
            StopVideo();
            return false;
        }
    }
    ...
    private static void StopVideo()
    {
        try
        {
            ...
            {
                _vid.Stop();
                Object.Destroy(((Component)_vid).gameObject);
                _vid = null;
            }
            if (_rt != null)
            {
    ...
    private static Sprite LoadOwn()
    {
        ...
        {
            NocturnePlugin.Logger?.LogWarning((object)("[MainArt] своя картинка: " + e.Message));
        }
        return null;
    }
