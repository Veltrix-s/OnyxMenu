...
public sealed class NocturneMouseTools : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (NocturneChatWindow.Open || NocturneMenu.Typing || GameChatOpen())
        {
            _dragging = false;
            return;
        }
        ...
        if (_sel != null && (_sel.Data == null || _sel.Data.Disconnected))
            Clear();
    ...
    private bool SelfDrag(Camera cam)
    {
        ...
        if (!_dragging || !me.CanMove)
            return _dragging;
        if (_dragBnd != null && !_dragBnd.OverlapPoint(m))
            return true;
        ...
        if ((next - cur).sqrMagnitude < 0.0001f)
            return true;
        if (Time.time - _lastTp < 0.05f)
            return true;
    ...
    private static EdgeCollider2D Boundary()
    {
        try
        {
            ...
            if (s == null)
                return null;
            Transform b = null;
            if (s.TryCast<SkeldShipStatus>() != null)
                b = s.transform.Find("starfield");
            else if (s.TryCast<MiraShipStatus>() != null)
                b = s.transform.Find("CloudGen");
            else if (s.TryCast<PolusShipStatus>() != null)
                b = s.transform.Find("OuterBoundary");
            else if (s.TryCast<AirshipStatus>() != null)
                b = s.transform.Find("Boundary");
            else if (s.TryCast<FungleShipStatus>() != null)
                b = s.transform.Find("GhostBoundary");
