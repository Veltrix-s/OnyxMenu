...
internal static class NocturneAccess
{
    ...
    {
        EnsureLoaded();
        if (RemoveByKey(Bans, key)) Persist(NocturneConfig.BanList, Bans, BanTxt);
    }
    internal static void RemoveWhite(string key)
    {
        EnsureLoaded();
        if (RemoveByKey(Whites, key)) Persist(NocturneConfig.WhiteList, Whites, WhiteTxt);
    }
    ...
    internal static string SafeName(ClientData c)
    {
        try
        {
            if (c != null && !string.IsNullOrWhiteSpace(c.PlayerName)) return c.PlayerName.Trim();
        }
        catch { }
        return c != null ? "#" + c.Id : "?";
    }
