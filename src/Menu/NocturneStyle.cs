...
internal static class NocturneStyle
{
    ...
    internal static NocturnePalette Current =>
        ThemeAt(NocturneConfig.ThemeIndex.Value);
    ...
    {
        if (Lite) return;
        if (_glowTex == null) _glowTex = BuildGlow(96);
        if (_glowStyle == null) { _glowStyle = new GUIStyle(); _glowStyle.normal.background = _glowTex; }
        Color prev = GUI.color;
        GUI.color = new Color(c.r, c.g, c.b, c.a * prev.a);
