...
public sealed class NocturneTracers : MonoBehaviour
{
    ...
    // 0 мирный, 1 пред, 2 ангел
    internal static int Side(PlayerControl p)
    {
        try
        {
            if (p == null || p.Data == null || p.Data.Role == null) return 0;
            if (p.Data.Role.IsImpostor) return 1;
            if (p.Data.Role.Role == RoleTypes.GuardianAngel) return 2;
        }
        catch { }
        return 0;
    }
    ...
    private static bool SideShown(PlayerControl p)
    {
        switch (Side(p))
        {
            case 1: return NocturneConfig.EspSeeImps.Value;
            case 2: return NocturneConfig.EspSeeAngels.Value;
            default: return NocturneConfig.EspSeeCrew.Value;
        }
    }
