...
public sealed class NocturneTracers : MonoBehaviour
{
    ...
    internal void DrawGui()
    {
        ...
        if (tracers)
        {
            ...
            while (it.MoveNext())
            {
                ...
                if (p == null || p == me || p.Data == null || p.Data.Disconnected || p.Data.IsDead)
                    continue;
        ...
        if (timers && ShipStatus.Instance != null)
        {
            ...
            {
                PlayerControl p = it.Current;
                if (p == null || p == me || !Patches.KillTimers.Killer(p))
                    continue;
        ...
        if (esp)
        {
            ...
            {
                PlayerControl p = it.Current;
                if (p == null || p == me || p.Data == null || p.Data.Disconnected)
                    continue;
                ...
                if (sp.x < -margin || sp.x > sw + margin || sp.y < -margin || sp.y > sh + margin)
                    continue;
    ...
    private void RebuildTaskTargets(PlayerControl me)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < me.myTasks.Count; i++)
            {
                ...
                if (pt == null || pt.IsComplete)
                    continue;
                NormalPlayerTask n = pt.TryCast<NormalPlayerTask>();
                if (n == null || n.Locations == null || n.Locations.Count == 0)
                    continue;
    ...
    internal static int Side(PlayerControl p)
    {
        try
        {
            if (p == null || p.Data == null || p.Data.Role == null)
                return 0;
    ...
    private static Color EspColor(PlayerControl p, bool dead)
    {
        ...
        {
            if (dead)
                return SideCol(NocturneConfig.EspDeadColor.Value);
            bool imp = false;
            try
            {
