...
public sealed class NocturneOverheadChat : MonoBehaviour
{
    ...
    {
        if (src == null || src.Data == null || string.IsNullOrWhiteSpace(msg))
            return;
    ...
    internal void DrawGui()
    {
        ...
        if ((where == 1 && !inMatch) || (where == 2 && !inLobby))
            return;
        ...
        while (it.MoveNext())
        {
            ...
            if (p == null || p.Data == null || p.Data.Disconnected)
                continue;
            if (!_bubbles.TryGetValue(p.PlayerId, out Bubble b))
                continue;
            ...
            if (sx < -200f || sx > Screen.width + 200f || sy < -120f || sy > Screen.height + 120f)
                continue;
    ...
    private void Draw(float sx, float sy, string msg, float fade, Color tint)
    {
        ...
        try
        {
            h = _text.CalcHeight(_gc, w - PadX * 2f) + PadY * 2f;
        }
        catch
        {
            h = 30f;
