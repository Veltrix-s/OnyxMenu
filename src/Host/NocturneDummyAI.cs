...
internal static class NocturneDummyAI
{
    ...
    private static Vector2 Pos(PlayerControl pc)
    {
        ...
        {
            return ((Component)pc).transform.position;
        }
    }
    ...
    private static ISystemType Sys(int id)
    {
        try
        {
            return ShipStatus.Instance != null && ShipStatus.Instance.Systems != null ? ShipStatus.Instance.Systems[(SystemTypes)id] : null;
        }
        catch
        {
            return null;
