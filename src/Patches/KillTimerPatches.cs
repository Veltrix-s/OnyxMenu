...
internal static class KillTimers
{
    ...
    internal static bool Killer(PlayerControl p)
    {
        try
        {
            return p.Data != null && p.Data.Role != null && p.Data.Role.CanUseKillButton && !p.Data.IsDead;
        }
        catch
        {
            return false;
...
internal static class KillTimersSeedPatch
{
    ...
    {
        if (NocturneConfig.KillTimers.Value)
            KillTimers.SeedAll(KillTimers.KillCooldown());
    }
}
...
internal static class KillTimersHitPatch
{
    ...
    {
        if (NocturneConfig.KillTimers.Value && __instance != null)
            KillTimers.Hit(__instance.PlayerId);
    }
}
