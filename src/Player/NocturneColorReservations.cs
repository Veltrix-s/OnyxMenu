...
internal static class NocturneColorReservations
{
    internal sealed class Entry
    {
        ...
        internal Entry(string fc, int colorId, string name)
        {
            Fc = fc ?? string.Empty;
            ColorId = colorId;
    ...
    internal static void Remove(string fc)
    {
        EnsureLoaded();
        string n = Norm(fc);
        if (Cache.RemoveAll(e => Norm(e.Fc) == n) > 0)
            Save();
    }

    internal static bool TryApplyOnJoin(PlayerControl player)
    {
        if (!NocturneConfig.ColorReservationsEnabled.Value)
            return false;
        ...
        if (string.IsNullOrWhiteSpace(fc))
            return false;
        if (!TryGet(fc, out Entry res))
            return false;

        int target = Mathf.Clamp(res.ColorId, 0, MaxColor());
        if (player.Data.DefaultOutfit.ColorId == target)
            return false;
        ...
        try
        {
            ...
            while (e.MoveNext())
            {
                ...
                if (other == null || other.Data == null || other.Data.DefaultOutfit == null)
                    continue;
                if (other.PlayerId == player.PlayerId)
                    continue;
                if (other.Data.DefaultOutfit.ColorId != target)
                    continue;
        ...
        try
        {
            ...
            foreach (string raw in File.ReadAllLines(FilePath, Encoding.UTF8))
            {
                ...
                if (p.Length < 2)
                    continue;
                string fc = p[0].Trim();
                if (fc.Length == 0 || !int.TryParse(p[1].Trim(), out int col))
                    continue;
