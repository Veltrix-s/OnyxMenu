...
internal static class ZoomShadowPatch
{
    ...
    public static void Postfix(HudManager __instance)
    {
        try
        {
            if (__instance == null || __instance.ShadowQuad == null || __instance.ShadowQuad.gameObject == null)
                return;

            bool wall = NocturneConfig.Wallhack.Value;
            bool hide = wall || VisualAssist.IsZoomActive();
            if (hide)
            {
                if (__instance.ShadowQuad.gameObject.activeSelf)
