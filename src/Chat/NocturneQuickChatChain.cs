...
internal static class NocturneQuickChatChain
{
    ...
    private static string RootGuard(PlayerControl me, int root)
    {
        if (me == null || AmongUsClient.Instance == null || !AmongUsClient.Instance.AmConnected)
            return NocturneText.T("не в игре", "not in game");
    ...
    internal static string Send(int root, int[] subs, bool alsoToSelf = false)
    {
        ...
        if (count > byte.MaxValue)
            return NocturneText.T("слишком много фраз", "too many phrases");
        for (int i = 0; i < count; i++)
        {
            if (subs[i] <= 0 || subs[i] > ushort.MaxValue)
                return NocturneText.T("плохой id фразы", "bad phrase id");
        }
        ...
        try
        {
            ...
            if (ok && alsoToSelf)
                PushChain(net, me, root, subs, me.OwnerId);
    ...
    internal static string SendFromText(string ids, bool alsoToSelf = false)
    {
        ...
        if (parts.Length == 0)
            return NocturneText.T("пусто", "empty");
        if (!int.TryParse(parts[0], out int root))
            return NocturneText.T("нет корневой фразы", "no root phrase");

        var subs = new int[parts.Length - 1];
        for (int i = 1; i < parts.Length; i++)
        {
            if (!int.TryParse(parts[i], out subs[i - 1]))
                return NocturneText.T("плохой id фразы", "bad phrase id");
        }
    ...
    {
        if (!int.TryParse((rootText ?? "").Trim(), out int root))
            return NocturneText.T("нет корневой фразы", "no root phrase");
