...
internal static class NocturneMeetingTools
{
    ...
    {
        if (NocturneConfig.NukeGame == null || !NocturneConfig.NukeGame.Value)
            return;
        if (Time.unscaledTime < _nukeNext)
            return;
    ...
    internal static string RequestMeeting(PlayerControl reporter, NetworkedPlayerInfo target, bool silent = false)
    {
        try
        {
            ...
            if (!ready)
                return silent ? "" : WhyNotReady(reporter, net);
            ...
            try
            {
                reporter.RemainingEmergencies = 999999;
            }
            catch { }
        ...
        {
            return silent ? "" : NocturneText.T("Ошибка: ", "Error: ") + ex.Message;
        }
    }

    private static string WhyNotReady(PlayerControl reporter, InnerNetClient net)
    {
        if (reporter == null || reporter.Data == null)
            return NocturneText.T("Нет игрока.", "No player.");
        if (net == null)
            return NocturneText.T("Не подключен.", "Not connected.");
        if (ShipStatus.Instance == null)
            return NocturneText.T("Только в матче.", "In match only.");
        if (MeetingHud.Instance != null)
            return NocturneText.T("Собрание уже идёт.", "Meeting already in progress.");
    ...
    internal static string CloseVoting()
    {
        ...
        if (m == null)
            return NocturneText.T("Нет собрания.", "No meeting.");
        try
        {
        ...
        {
            return NocturneText.T("Не удалось закрыть.", "Failed to close.");
        }
    }
