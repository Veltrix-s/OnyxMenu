...
internal static class NocturneChatLog
{
    ...
    {
        if (!NocturneConfig.ChatLog.Value)
            return;
        try
        {
            ...
            try
            {
                if (src != null && src.Data != null && !string.IsNullOrEmpty(src.Data.PlayerName))
                    name = Strip(src.Data.PlayerName).Trim();
            }
            catch { }
            ...
            try
            {
                ...
                {
                    id = c.Id;
                    if (!string.IsNullOrWhiteSpace(c.FriendCode))
                        fc = c.FriendCode.Trim();
                    if (!string.IsNullOrWhiteSpace(c.ProductUserId))
                        puid = c.ProductUserId.Trim();
                    if (puid.Length > 12)
                        puid = puid.Substring(0, 12);
                    if (c.PlatformData != null)
                        plat = c.PlatformData.Platform.ToString();
                }
            }
            catch { }
    ...
    private static void Reload()
    {
        try
        {
            ...
            foreach (string line in File.ReadAllLines(Path0, Encoding.UTF8))
            {
                ...
                if (!string.IsNullOrEmpty(w) && !w.StartsWith("#"))
                    fresh.Add(w);
            }
            lock (Lock)
            {
...
internal static class ChatCensorPatch
{
    ...
    {
        if (!NocturneConfig.BanWords.Value)
            return;
