...
internal static class RevealVotesPatch
{
    ...
    private static void Remember(byte voter, byte votedFor)
    {
        if (votedFor == PlayerVoteArea.HasNotVoted || votedFor == PlayerVoteArea.MissedVote || votedFor == PlayerVoteArea.DeadVote)
            ...
            _targets[voter] = votedFor;
    }

    public static void Postfix(MeetingHud __instance)
    {
        if (!NocturneConfig.RevealVotes.Value || __instance == null || __instance.playerStates == null || GameData.Instance == null)
            return;
        try
        {
            ...
            foreach (PlayerVoteArea voter in __instance.playerStates)
            {
                if (voter == null)
                    continue;
                Remember(voter.PlayerId, voter.VotedForId);
                if (_drawn.Contains(voter.PlayerId))
                    continue;
                byte tgt = _targets.TryGetValue(voter.PlayerId, out byte t) ? t : voter.VotedForId;
                DrawVote(__instance, voter.PlayerId, tgt);
            }
    ...
    {
        if (hud == null || hud.playerStates == null || GameData.Instance == null || _drawn.Contains(voterId))
            return;
        if (votedFor == PlayerVoteArea.HasNotVoted || votedFor == PlayerVoteArea.MissedVote || votedFor == PlayerVoteArea.DeadVote)
            return;
        ...
        if (votedFor == PlayerVoteArea.SkippedVote)
        {
            ...
            {
                hud.BloopAVoteIcon(info, 0, hud.SkippedVoting.transform);
                Paint(hud.SkippedVoting.transform, info);
            }
            return;
        }
        ...
        {
            if (target == null || target.PlayerId != votedFor)
                continue;
    ...
    {
        if (!NocturneConfig.RevealAnonVotes.Value)
            return;
        try
        {
            if (holder == null || voter == null || voter.DefaultOutfit == null)
                return;
    ...
    {
        if (hud == null || hud.playerStates == null)
            return;
