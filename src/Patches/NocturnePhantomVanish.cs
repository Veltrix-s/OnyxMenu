...
internal static class NocturnePhantomVanish
{
    ...
    {
        if (me == null || me.NetTransform == null)
            return;
        ...
        try
        {
            me.NetTransform.RpcSnapTo(Away);
        }
        catch { }
        try
        {
            me.transform.position = _home;
        }
        catch { }
        ...
        try
        {
            ((MonoBehaviour)me).StartCoroutine(Co(me).WrapToIl2Cpp());
        }
        catch
        {
            _hidden = false;
    ...
    private static IEnumerator Co(PlayerControl me)
    {
        ...
        if (_hidden && me != null && me.AmOwner && MeetingHud.Instance == null)
            try
            {
                me.NetTransform.RpcSnapTo(me.transform.position);
            }
            catch { }
...
internal static class NocturnePhantomVanishPatch
{
    ...
    {
        if (!NocturneConfig.PhantomNoVanish.Value)
            return;
        if (__instance == null || __instance != PlayerControl.LocalPlayer || __instance.inVent)
            return;
...
internal static class NocturnePhantomFreezeNet
{
    ...
    {
        if (!NocturnePhantomVanish.Active || __1)
            return true;
