...
public sealed class NocturneDummies : MonoBehaviour
{
    ...
    public void FixedUpdate()
    {
        ...
        _scratch.Clear();
        foreach (var kv in _bots)
            if (kv.Value != null && kv.Value.Pc != null) _scratch.Add(kv.Value.Pc);
        try
        {
    ...
    public void Update()
    {
        if (NocturneConfig.DummyEnabled.Value && !NocturneMenu.Rebinding
            && NocturneKeys.Down(NocturneConfig.DummyKey))
            SpawnNow();
    ...
    private static void SetName(PlayerControl pc, string name)
    {
        ...
        try
        {
            if (pc.cosmetics != null && pc.cosmetics.nameText != null) ((TMP_Text)pc.cosmetics.nameText).text = name;
        }
        catch { }
    }
