...
internal static class CosmeticAccess
{
    ...
    {
        if (!NocturneConfig.FreeCosmetics.Value)
    ...
    {
        if (!NocturneConfig.HideCosmeticsInMatch.Value || player == null)
            return;
        ...
        if (nextHide.TryGetValue(pid, out float at) && now < at)
            return;
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(outfit.HatId))
                player.SetHat(string.Empty, 0);
            if (!string.IsNullOrEmpty(outfit.SkinId))
                player.SetSkin(string.Empty, 0);
            if (!string.IsNullOrEmpty(outfit.VisorId))
                player.SetVisor(string.Empty, 0);
            if (!string.IsNullOrEmpty(outfit.NamePlateId))
                player.SetNamePlate(string.Empty);
            if (!string.IsNullOrEmpty(outfit.PetId))
                player.SetPet(string.Empty, 0);
        }
        catch { }
    }
