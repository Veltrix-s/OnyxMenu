...
internal static class NocturneAutoHostService
{
    ...
    internal static void Tick()
    {
        ...
        if (!IsEnabled)
        {
            ...
            SetState(AutoHostState.Disabled, NocturneText.T("выключен", "disabled"));
            return;
        }
        ...
        if (client == null)
        {
            ...
            SetState(AutoHostState.Idle, NocturneText.T("клиент недоступен", "client unavailable"));
            return;
        }
        ...
        {
            ResetLobbyFlow(clearBackoff: false);
            SetState(AutoHostState.Idle, NocturneText.T("ожидаю хост-контекст", "waiting for host context"));
            return;
        }
        ...
        {
            ResetLobbyFlow(clearBackoff: false);
            SetState(ShouldReturnAfterMatch ? AutoHostState.Returning : AutoHostState.InGame, ShouldReturnAfterMatch ? NocturneText.T("возврат в лобби", "returning to lobby") : NocturneText.T("матч завершен", "match finished"));
            return;
        }
        ...
        {
            ResetLobbyFlow(clearBackoff: true);
            SetState(AutoHostState.InGame, NocturneText.T("матч идет", "match running"));
            return;
        }
        ...
        if (LobbyBehaviour.Instance == null)
        {
            ...
            lobbyOpenedAt = -1f;
            lobbyGameId = -1;
            SetState(AutoHostState.Idle, NocturneText.T("вне лобби", "not in a lobby"));
            return;
        }
    ...
    internal static void ResetTransientState()
    {
        nextTickAt = 0f;
        ResetLobbyFlow(clearBackoff: true);
        SetState(IsEnabled ? AutoHostState.Idle : AutoHostState.Disabled, IsEnabled ? NocturneText.T("сброшен", "reset") : NocturneText.T("выключен", "disabled"));
    }
    ...
    private static void TickHostedLobby(InnerNetClient client, float now)
    {
        ...
        if (!forceStart && warmupRemaining > 0.05f)
        {
            ...
            SetState(AutoHostState.Warmup, (NocturneText.IsRussian ? $"прогрев лобби {Mathf.CeilToInt(warmupRemaining)}с" : $"lobby warmup {Mathf.CeilToInt(warmupRemaining)}s"));
            return;
        }
        ...
        if (waitingForLoad && !forceStart && !CanBypassLoadWait(now, readyPlayers, connectedPlayers, loadingName))
        {
            ...
            lastStartIssuedAt = -1f;
            lastCountdownNotice = -1;
            SetState(AutoHostState.WaitingLoad, (NocturneText.IsRussian ? $"ожидаю прогрузку {readyPlayers}/{connectedPlayers}: {loadingName}" : $"waiting for load {readyPlayers}/{connectedPlayers}: {loadingName}"));
            return;
        }
        if (!waitingForLoad)
        {
        ...
        if (lastStartIssuedAt > 0f)
        {
            ...
            {
                SetState(AutoHostState.Starting, NocturneText.T("старт отправлен", "start sent"));
                return;
            }
        ...
        if (backoffUntil > now)
        {
            SetState(AutoHostState.Backoff, NocturneText.T("пауза после попытки", "cooldown after attempt"));
            return;
        }
        ...
        if (!forceStart && !enoughPlayers && !continueBelowMin)
        {
            ...
            activeCountdownDelay = -1f;
            lastCountdownNotice = -1;
            SetState(AutoHostState.WaitingPlayers, (NocturneText.IsRussian ? $"игроки {connectedPlayers}/{requiredPlayers}" : $"players {connectedPlayers}/{requiredPlayers}"));
            return;
        }
        ...
        if (!forceStart && countdownStartedAt < 0f)
        {
            ...
            lastCountdownNotice = -1;
            SetState(AutoHostState.Countdown, IsFastStartActive(connectedPlayers) ? NocturneText.T("быстрый старт", "fast start") : NocturneText.T("минимум игроков набран", "minimum players reached"));
        ...
        if (!forceStart && now - countdownStartedAt < delay)
        {
            ...
            SetState(AutoHostState.Countdown, NocturneText.T("отсчет", "countdown"));
            return;
        }
        ...
        lastCountdownNotice = -1;
        SetState(AutoHostState.Starting, forceStart ? forceReason : NocturneText.T("старт матча", "starting match"));
    ...
    private static void TrackLobby(InnerNetClient client, float now)
    {
        ...
        lobbyGameId = gameId;
        ResetLobbyFlow(clearBackoff: true);
        SetState(AutoHostState.WaitingPlayers, NocturneText.T("новое лобби", "new lobby"));
    }
    ...
    {
        readyPlayers = 0;
        loadingName = NocturneText.T("игрок", "player");
    ...
    private static bool CanBypassLoadWait(float now, int readyPlayers, int connectedPlayers, string loadingName)
    {
        ...
        int grace = Mathf.Clamp(NocturneConfig.AutoHostLoadGraceSeconds.Value, 0, 90);
        if (grace <= 0)
        {
            loadWaitStartedAt = -1f;
            return false;
        ...
        {
            SetState(AutoHostState.WaitingLoad, (NocturneText.IsRussian ? $"жду прогрузку {readyPlayers}/{connectedPlayers}: {loadingName}" : $"waiting for load {readyPlayers}/{connectedPlayers}: {loadingName}"));
            return false;
        }

        SetState(AutoHostState.Countdown, NocturneText.T("прогрузка задержалась, старт по готовым", "load stalled, starting with ready players"));
        return true;
    }
    ...
    private static bool ShouldForceStart(int connectedPlayers, out string reason)
    {
        ...
        int forceAfterMinutes = Mathf.Clamp(NocturneConfig.AutoHostForceAfterMinutes.Value, 0, 10);
    ...
    {
        int threshold = Mathf.Clamp(NocturneConfig.AutoHostFastStartPlayers.Value, 0, 15);
    ...
    private static float EffectiveStartDelay(int connectedPlayers)
    {
        ...
        {
            delay = Mathf.Min(delay, Mathf.Clamp(NocturneConfig.AutoHostFastStartDelaySeconds.Value, 0, 60));
        }
    ...
    private static int RequiredPlayers => Mathf.Clamp(NocturneConfig.AutoHostMinPlayers.Value, 1, 15);
    private static int ForceMinPlayers => Mathf.Clamp(NocturneConfig.AutoHostForceMinPlayers.Value, 1, 15);
    private static float StartDelaySeconds => Mathf.Clamp(NocturneConfig.AutoHostStartDelaySeconds.Value, 0f, 180f);
    private static float BackoffSeconds => Mathf.Clamp(NocturneConfig.AutoHostBackoffSeconds.Value, 2f, 60f);
    ...
    private static float WarmupRemaining => lobbyOpenedAt < 0f ? 0f : Mathf.Clamp((NocturneConfig.AutoHostWarmupSeconds.Value) - (Time.unscaledTime - lobbyOpenedAt), 0f, 120f);
}
