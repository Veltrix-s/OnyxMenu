...
internal static class NocturneVentTp
{
    ...
    internal static int VentCount()
    {
        try
        {
            return ShipStatus.Instance != null ? ShipStatus.Instance.AllVents.Count : 0;
        }
        catch
        {
            return 0;
    ...
    internal static string KickAllFromVents()
    {
        ...
        if (!AmongUsClient.Instance.AmHost)
            return NocturneText.T("Только хост.", "Host only.");
        if (MeetingHud.Instance != null)
            return NocturneText.T("Не в собрании.", "Not in a meeting.");
        ...
        try
        {
            ...
            while (e.MoveNext())
            {
                ...
                if (p == null || p.Data == null || p.Data.Disconnected || p.Data.IsDead)
                    continue;
                if (p == PlayerControl.LocalPlayer || !p.inVent)
                    continue;
    ...
    private static byte VentId(int index)
    {
        try
        {
    ...
    internal static int NearestVent(Vector2 pos)
    {
        int best = -1;
    ...
    internal static string CycleVent(int dir)
    {
        int c = VentCount();
        if (c <= 0)
            return NocturneText.T("нет вентов", "no vents");
        Vent = ((Vent + dir) % c + c) % c;
    ...
    internal static string SendMarked()
    {
        if (AmongUsClient.Instance == null || ShipStatus.Instance == null)
            return NocturneText.T("только в матче", "in-match only");
        if (_marked.Count == 0)
            return NocturneText.T("нет отмеченных", "no marked players");
    ...
    internal static void AutoTick()
    {
        if (!NocturneConfig.VentTpAuto.Value)
            return;
        if (AmongUsClient.Instance == null || ShipStatus.Instance == null || _marked.Count == 0)
            return;
        if (VentCount() <= 0)
            return;
    ...
    private static int Scatter()
    {
        ...
        try
        {
            ...
            while (e.MoveNext())
            {
                ...
                if (p == null || p.Data == null || p.Data.Disconnected || p.Data.IsDead)
                    continue;
                if (!_marked.Contains(p.PlayerId))
                    continue;
                int vent = ((Vent + idx) % count + count) % count;
                if (Send(p, vent).StartsWith(NocturneText.T("в венте", "vent")))
                    n++;
                idx++;
            }
        }
    ...
    internal static string Send(PlayerControl target, int ventId)
    {
        ...
        if (target.Data.IsDead)
            return NocturneText.T("цель мертва", "target is dead");

        int count = VentCount();
        if (count <= 0)
            return NocturneText.T("нет вентов", "no vents");
        byte vent = VentId(ventId);
    ...
    private static float _lastVentCheck = -99f;

    internal static bool VentAllowed(int mode, bool imp) => mode switch
    {
        1 => !imp,
        2 => imp,
        3 => false,
    ...
    internal static void EnforceVentMode()
    {
        int mode = NocturneConfig.VentMode.Value;
        if (mode <= 0)
            return;
        ...
        {
            PlayerControl p = e.Current;
            if (p == null || p.Data == null || p.Data.Disconnected || p.Data.IsDead || !p.inVent)
                continue;

            bool imp = p.Data.Role != null && p.Data.Role.IsImpostor;
            if (VentAllowed(mode, imp))
                continue;
...
[HarmonyLib.HarmonyPatch(typeof(PlayerPhysics), nameof(PlayerPhysics.CoEnterVent))]
internal static class NocturneVentModeEnterPatch
{
    public static bool Prefix(PlayerPhysics __instance)
    {
        int mode = NocturneConfig.VentMode.Value;
        if (mode <= 0 || AmongUsClient.Instance == null || !AmongUsClient.Instance.AmHost)
            return Patches.HarmonyControl.Continue;

        PlayerControl me = __instance.myPlayer;
        if (me == null || !me.AmOwner || me.Data == null)
            return Patches.HarmonyControl.Continue;

        bool imp = me.Data.Role != null && me.Data.Role.IsImpostor;
        return NocturneVentTp.VentAllowed(mode, imp) ? Patches.HarmonyControl.Continue : Patches.HarmonyControl.SkipOriginal;
    }
}
