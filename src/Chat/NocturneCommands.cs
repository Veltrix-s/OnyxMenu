...
internal static class NocturneCommands
{
    public static bool Prefix(PlayerControl __instance, string chatText)
    {
        try
        {
            if (__instance != PlayerControl.LocalPlayer || string.IsNullOrEmpty(chatText))
                return true;
            ...
            if (!NocturneConfig.ChatCmds.Value)
                return true;
            ...
            switch (cmd)
            {
                ...
                case "/role":
                    ...
                    return false;
                case "/start":
                    Simple(host, () => { if (GameStartManager.Instance != null) GameStartManager.Instance.BeginGame(); }, "Старт", "Start");
                    return false;
                case "/end":
                    Simple(host, () => { GameManager.Instance.RpcEndGame((GameOverReason)1, false); }, "Матч завершён", "Match ended");
                    return false;
                case "/meeting":
                    Simple(host, () => { PlayerControl.LocalPlayer.CmdReportDeadBody(null); }, "Собрание", "Meeting");
                    return false;
                case "/close":
                    if (RequireHost(host))
                        Toast(NocturneText.T("Собрание", "Meeting"), NocturneMeetingTools.CloseMeeting());
                    return false;
                case "/fix":
                    if (RequireHost(host))
                        FixSabs();
    ...
    {
        if ((System.DateTime.UtcNow - _lastHelp).TotalSeconds < 2.5)
            return;
    ...
    private static void Kick(string name, bool ban, bool host)
    {
        ...
        try
        {
            AmongUsClient.Instance.KickPlayer(p.OwnerId, ban);
        }
        catch { }
    ...
    private static void Role(string rest, bool host)
    {
        ...
        NocturneForceRoles.Assign(p, role);
    ...
    private static RoleTypes RoleFromArg(string a)
    {
        ...
        foreach (var r in NocturneForceRoles.Roles)
        {
            ...
            if (ru == a || en == a || ru.StartsWith(a) || en.StartsWith(a))
                return r.Role;
        }
        return (RoleTypes)255;
    }
    ...
    private static PlayerControl Find(string name)
    {
        ...
        foreach (PlayerControl pc in PlayerControl.AllPlayerControls)
        {
            ...
            if (n == name)
                return pc;
            if (partial == null && (n.StartsWith(name) || n.Contains(name)))
                partial = pc;
        }
        return partial;
    }
