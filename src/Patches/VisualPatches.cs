...
internal static class VisualAssist
{
    ...
    private static bool Overlay()
    {
        ...
        if (Minigame.Instance != null && !NocturneConfig.ZoomDuringTasks.Value) return true;
    ...
    private static string RoleDisplayName(int roleId, string fallback)
    {
        switch (roleId)
        {
            ...
            case 19: return NocturneText.T("Судья", "Judge");
    ...
    private static Color RoleColor(int roleId, Color fallback)
    {
        switch (roleId)
        {
            ...
            case 19: return new Color(0.35f, 0.85f, 0.95f);
            default: return fallback;
        }
    }
    ...
    {
        if (!NocturneConfig.ForceNamesInHns.Value) return;
