...
internal static class PlatformSpoofOutboundPatch
{
    ...
    public static void Prefix(PlatformSpecificData __instance)
    {
        if (__instance == null)
            return;
        if (!NocturneConfig.SpoofPlatformEnabled.Value) return;
        try
        {
            int idx = Mathf.Clamp(NocturneConfig.SpoofPlatformIndex.Value, 0, PlatformValues.Length - 1);
...
public sealed class NocturneSpoofDriver : MonoBehaviour
{
    ...
    {
        int display = Mathf.Clamp(NocturneConfig.SpoofLevelValue.Value, 1, 9999);
...
internal static class DeviceIdSpoofPatch
{
    ...
    {
        if (!NocturneConfig.SpoofDeviceId.Value)
            return;
...
internal static class NocturneFcSpoof
{
    ...
    internal static bool On => NocturneConfig.SpoofFriendCodeEnabled.Value;
    ...
    internal static void Apply()
    {
        ...
        try
        {
            ...
            if (EOSManager.Instance != null)
                edit = EOSManager.Instance.editAccountUsername;
...
internal static class NocturneFcRegisterPatch
{
        ...
        [HarmonyArgument(1)] ref Il2CppSystem.Action<Assets.InnerNet.ResponseState, Assets.InnerNet.Response<Assets.InnerNet.ResponseFriendCode>> resultCallback)
    {
        ...
        if (string.IsNullOrEmpty(fallback) || fallback == value) return true;
    ...
    {
        string plain = NocturneNameColor.Strip(value).Trim();
        if (plain.Length == 0)
            return string.Empty;
...
internal static class NocturneFcScreenPatch
{
    ...
    {
        if (!NocturneFcSpoof.On || __instance == null)
            return;
        string value = NocturneFcSpoof.Value;
        if (string.IsNullOrEmpty(value))
            return;
...
internal static class NocturneFcRandomPatch
{
    public static void Postfix(EditAccountUsername __instance)
    {
        if (!NocturneFcSpoof.On || __instance == null || __instance.UsernameText == null)
            return;
        string value = NocturneFcSpoof.Value;
        if (!string.IsNullOrEmpty(value))
            try
...
internal static class NocturneFcButtonPatch
{
    ...
    {
        if (!NocturneFcSpoof.On || __instance == null || enable)
            return;
        try
        {
