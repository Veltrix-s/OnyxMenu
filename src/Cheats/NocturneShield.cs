...
internal static class NocturneShield
{
    ...
    {
        _marked.Clear();
        if (NocturneConfig.ShieldAll != null)
            NocturneConfig.ShieldAll.Value = false;
    }
    ...
    internal static void Tick()
    {
        ...
        while (e.MoveNext())
        {
            ...
            if (!Alive(p))
                continue;
            if (!all && !_marked.Contains(p.PlayerId))
                continue;
            Send(p);
        }
    }
    ...
    private static bool Ready()
        => Host() && ShipStatus.Instance != null && LobbyBehaviour.Instance == null
