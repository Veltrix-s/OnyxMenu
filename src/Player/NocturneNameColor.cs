...
public sealed class NocturneColoredName : MonoBehaviour
{
    ...
    {
        if (!NocturneConfig.NameColor.Value)
            return;
        PlayerControl me = PlayerControl.LocalPlayer;
        if (me == null || me.cosmetics == null || me.cosmetics.nameText == null)
            return;

        if (NocturneConfig.VisualPlayerInfoNames.Value || NocturneConfig.RevealRoles.Value
            || (NocturneConfig.UnmaskShapeshifter.Value)
        ...
        if (!anim && raw == _raw && style == _style && !_anim)
            return;
        _raw = raw;
        _style = style;
        _anim = anim;
