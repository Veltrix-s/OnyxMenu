...
public sealed class NocturneMusicPlayer : MonoBehaviour
{
    ...
    private static float AccentHue()
    {
        try
        {
            string name = NocturneConfig.MusicAccent != null ? NocturneConfig.MusicAccent.Value : "Red";
    ...
    private void Awake()
    {
        ...
        _source.pitch                = 1f;
        ScanTracks();
        if (NocturneConfig.MusicEqPreset != null && NocturneConfig.MusicEqPreset.Value > 0
    ...
    private void Update()
    {
        using var _perf = NocturneProfiler.Sample("Music.Update");
        ...
        if (!NocturneMenu.Rebinding && NocturneConfig.MusicToggleKey != null && NocturneConfig.MusicToggleKey.Value != KeyCode.None && UnityEngine.Input.GetKeyDown(NocturneConfig.MusicToggleKey.Value) && noTextFieldFocused && !chatOpen)
        {
        ...
        if (noTextFieldFocused && !chatOpen && !NocturneMenu.Rebinding)
        {
            ...
            if (NocturneConfig.MusicPrevKey      != null && NocturneConfig.MusicPrevKey.Value      != KeyCode.None && UnityEngine.Input.GetKeyDown(NocturneConfig.MusicPrevKey.Value))      Prev();
            if (NocturneConfig.MusicNextKey      != null && NocturneConfig.MusicNextKey.Value      != KeyCode.None && UnityEngine.Input.GetKeyDown(NocturneConfig.MusicNextKey.Value))      Next();
            if (NocturneConfig.MusicPlayPauseKey != null && NocturneConfig.MusicPlayPauseKey.Value != KeyCode.None && UnityEngine.Input.GetKeyDown(NocturneConfig.MusicPlayPauseKey.Value)) PlayPause();
            if (NocturneConfig.MusicStopKey      != null && NocturneConfig.MusicStopKey.Value      != KeyCode.None && UnityEngine.Input.GetKeyDown(NocturneConfig.MusicStopKey.Value))      Stop();
            float volDelta = 0f;
            if (NocturneConfig.MusicVolumeUpKey   != null && NocturneConfig.MusicVolumeUpKey.Value   != KeyCode.None && UnityEngine.Input.GetKey(NocturneConfig.MusicVolumeUpKey.Value))   volDelta += dt0 * 0.6f;
            if (NocturneConfig.MusicVolumeDownKey != null && NocturneConfig.MusicVolumeDownKey.Value != KeyCode.None && UnityEngine.Input.GetKey(NocturneConfig.MusicVolumeDownKey.Value)) volDelta -= dt0 * 0.6f;
    ...
    {
        using var _perf = NocturneProfiler.Sample("Music.OnGUI");
    ...
    private void DrawEqPanel(int _)
    {
        ...
        string curAcc = NocturneConfig.MusicAccent != null ? NocturneConfig.MusicAccent.Value : "Red";
        const int perRow = 9;
    ...
    private void EnsureStyles()
    {
        ...
        string acc = NocturneConfig.MusicAccent != null ? NocturneConfig.MusicAccent.Value : "Red";
    ...
    private void PcmReaderCallback(Il2CppStructArray<float> data)
    {
        ...
        if (!NocturneProfiler.Enabled) { PcmReaderCallbackImpl(data); return; }
        long _ps = System.Diagnostics.Stopwatch.GetTimestamp();
        try { PcmReaderCallbackImpl(data); }
        finally { NocturneProfiler.RecordAudioCallback(_ps); }
    }
