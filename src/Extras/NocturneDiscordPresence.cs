...
public sealed class NocturneDiscordPresence : MonoBehaviour
{
    ...
    private static string Scene()
    {
        try
        {
            ...
            if (MeetingHud.Instance != null)
                return "🗳 " + NocturneText.T("Собрание", "Meeting") + tail;
            if (ShipStatus.Instance != null)
                return "🎮 " + NocturneText.T("В игре", "In game") + tail;
            if (LobbyBehaviour.Instance != null)
                return "🚪 " + NocturneText.T("Лобби", "Lobby") + tail;
        }
        catch { }
    ...
    private static (int cur, int max, string id)? Party()
    {
        try
        {
            ...
            if (cur <= 0)
            {
                if (PlayerControl.AllPlayerControls != null) cur = PlayerControl.AllPlayerControls.Count;
            }
            if (cur <= 0)
                return null;
            if (max < cur)
