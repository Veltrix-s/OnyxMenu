...
internal enum UpState
{
    Idle, Checking, Found, Loading, Done, Fail
}
...
public sealed class NocturneUpdateCheck : MonoBehaviour
{
    ...
    {
        if (Instance == null || State == UpState.Checking || State == UpState.Loading)
            return;
    ...
    {
        if (Instance == null || State != UpState.Found || Instance._load != null)
            return;
        if (string.IsNullOrWhiteSpace(_url))
        {
            try
            {
                GUIUtility.systemCopyBuffer = Site;
            }
            catch { }
            try
            {
        ...
        try
        {
            Instance._load = Http.GetByteArrayAsync(_url);
        }
        catch (Exception e)
        {
            Fail(e.Message);
    ...
    private static string ErrOf(Task t)
    {
        ...
        if (e == null)
            return t.IsCanceled ? "canceled" : "unknown";
    ...
    private static void Install(byte[] data)
    {
        ...
        {
            Fail(NocturneText.T("пустой файл", "empty file"));
            return;
        }
        try
        {
