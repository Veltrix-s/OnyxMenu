...
internal static class NocturneForceRoles
{
    ...
    internal static void Assign(PlayerControl pc, RoleTypes role, bool canOverride = false)
    {
        try
        {
            pc.RpcSetRole(role, canOverride);
        }
    ...
    {
        if (!Host() || LobbyBehaviour.Instance == null)
            return;
    ...
    internal static bool Distribute()
    {
        ...
        if (gm == null || gm.LogicRoleSelection == null)
            return true;
        try
        {
            if (gm.IsHideAndSeek())
        ...
        foreach (PlayerControl p in players)
            if (_forced.TryGetValue(p.PlayerId, out RoleTypes r) && ImpTeam(r))
                imps.Add(p);

        int num;
        try
        {
            num = opt.GetInt(Int32OptionNames.NumImpostors);
        }
        catch
        {
            num = 1;
        ...
        foreach (PlayerControl p in players)
        {
            if (!_forced.TryGetValue(p.PlayerId, out RoleTypes role))
                continue;
            if (role == RoleTypes.Crewmate || role == RoleTypes.Impostor || (int)role == 255)
                continue;
            try
            {
                RoleManager.Instance.SetRole(p, role);
            }
            catch { }
            try
            {
    ...
    private static void Refresh(PlayerControl p)
    {
        try
        {
            if (p == null || p.Data == null)
                return;
            if (p.Data.Role != null)
                p.Data.Role.Initialize(p);
            if (ImpTeam(p.Data.RoleType))
                p.SetKillTimer(0f);
        }
        catch { }
    }
    ...
    private static void EnsureRate(RoleTypes role)
    {
        ...
        try
        {
            if (!NocturneLobbySettings.Ready() || NocturneLobbySettings.RoleNum(role) > 0)
                return;
...
internal static class NocturneForceRolesSelectPatch
{
    public static bool Prefix()
    {
        try
        {
            return NocturneForceRoles.Distribute();
        }
        catch
        {
            return true;
