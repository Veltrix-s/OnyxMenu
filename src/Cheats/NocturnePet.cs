...
internal static class NocturnePet
{
    ...
    internal static string TogglePaint()
    {
        if (!HasPet(PlayerControl.LocalPlayer))
            return NocturneText.T("Нет пета.", "No pet.");
        if (On && Paint)
        {
            Stop();
    ...
    private static void RefreshRooms()
    {
        ...
        try
        {
            ...
            foreach (var r in ss.AllRooms)
            {
                ...
                try
                {
                    n = TranslationController.Instance.GetString(r.RoomId);
                }
                catch
                {
                    n = r.RoomId.ToString();
    ...
    private static void EnsureRooms()
    {
        try
        {
            ...
            if (_roomNames.Count > 0 && Time.unscaledTime - _roomsAt < 1f)
                return;
    ...
    internal static string FillRoom()
    {
        if (!HasPet(PlayerControl.LocalPlayer))
            return NocturneText.T("Нет пета.", "No pet.");
        RefreshRooms();
        if (_roomAreas.Count == 0)
            return NocturneText.T("Нет комнат (не в матче?).", "No rooms (not in match?).");
        _room = Mathf.Clamp(_room, 0, _roomAreas.Count - 1);
        Collider2D area = _roomAreas[_room];
        if (area == null)
            return NocturneText.T("Нет комнаты.", "No room.");
    ...
    {
        if (!On || !Paint || e == null || e.type != EventType.Repaint || NocturneMenu.Opened)
            return;
