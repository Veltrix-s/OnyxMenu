...
public sealed class NocturneMenu : MonoBehaviour
{
    ...
    private Rect Card(float x, ref float y, float w, string title, float bodyH)
    {
        ...
        if (_cardGrp >= 0 && _cardGrp != _subTab[_tab])
            return new Rect(-20000f, -20000f, w - 36f, bodyH);
    ...
    private int SubBar(float x, ref float y, float w, string[] ru, string[] en)
    {
        ...
        NocturnePalette p = NocturneStyle.Current;
        int cols = names.Length <= 4 ? names.Length : 3;
        int rows = Mathf.CeilToInt(names.Length / (float)cols);
        float cellW = (w - (cols - 1) * 6f) / cols;

        for (int i = 0; i < names.Length; i++)
        {
            var r = new Rect(x + (i % cols) * (cellW + 6f), y + (i / cols) * 32f, cellW, 28f);
            if (Cull(r.y, r.height)) continue;
            bool on = cur == i;
            NocturneStyle.FillRounded(r, on ? A(p.Accent, 0.20f) : A(Color.white, 0.045f), 8);
            NocturneStyle.StrokeRounded(r, on ? A(p.Accent, 0.65f) : A(Color.white, 0.07f), 8, 1);
            _smallBtn.normal.textColor = on ? Color.Lerp(p.Accent, Color.white, 0.5f) : p.Muted;
            GUI.Label(r, names[i], _smallBtn);
            if (GUI.Button(r, GUIContent.none, _invisible) && !on)
            {
                _subTab[_tab] = i;
                _scroll = 0f;
                _scrollPending = 0f;
