...
internal static class NocturneLobbyTheme
{
    ...
    private static void PaintPanelBackdrop(GameStartManager m)
    {
        ...
        {
            if (img == null || !LayerGeom(img, out Vector3 c, out Vector2 size))
                continue;
    ...
    private static bool MainPanelLayer(string n, Vector3 c, Vector2 size, float left, float right, float bottom, float top)
    {
        ...
        if (n.Contains("copy") || n.Contains("edit") || n.Contains("view") || n.Contains("button") || n.Contains("icon") || n.Contains("glyph"))
            return false;
        if (size.x < 1.5f || size.y < 2.8f || size.x > 7.2f || size.y > 11.5f)
            return false;
        if (c.x < left - 0.42f || c.x > right + 0.42f)
            return false;
    ...
    private static void PaintActionButton(PassiveButton b, bool isView)
    {
        ...
        {
            if (firstImage != null)
                PaintImage(firstImage, ActionAccentColor(((Component)firstImage).transform.localPosition, isView));
            else if (firstSprite != null)
                PaintSprite(firstSprite, ActionAccentColor(((Component)firstSprite).transform.localPosition, isView));
        }
    ...
    private static Color ActionLayerColor(string n, Vector3 local, bool isView)
    {
        ...
        if (n.Contains("top") || n.Contains("cap") || n.Contains("edge"))
            return a6;
        if (local.y > 0.08f)
            return a1;
        if (local.y < -0.08f)
    ...
    private static void PaintButton(PassiveButton b, NocturnePalette p, Role role)
    {
        ...
        {
            case Role.Start:
                fill = Color.Lerp(p.ButtonHover, p.Accent, 0.45f);
                text = Color.white;
                break;
            case Role.Accent:
                fill = Color.Lerp(p.Button, p.Accent, 0.42f);
                text = p.Text;
                break;
            case Role.Success:
                fill = new Color(0.10f, 0.40f, 0.22f, 1f);
                text = new Color(0.94f, 1f, 0.95f, 1f);
                break;
            case Role.Danger:
                fill = new Color(0.42f, 0.06f, 0.10f, 1f);
                text = new Color(1f, 0.96f, 0.96f, 1f);
                break;
            case Role.DarkBlue:
                fill = Color.Lerp(new Color(0.02f, 0.027f, 0.045f, 1f), p.Accent, 0.45f);
                text = Color.white;
                break;
            default:
                fill = Color.Lerp(p.Panel, p.Button, 0.6f);
                text = p.Text;
                break;
        }
    ...
    private static void PaintImage(Image img, Color col)
    {
        ...
        {
            imageOrig[id] = ((Graphic)img).color;
            imageRef[id] = img;
        }
