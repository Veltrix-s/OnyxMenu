...
public sealed class NocturneTwins : MonoBehaviour
{
    ...
    internal static void Prune()
    {
        for (int i = _live.Count - 1; i >= 0; i--)
        {
            ...
            try
            {
                dead = t == null || t.Pc == null || t.Pc.transform == null;
            }
            catch
            {
                dead = true;
    ...
    internal static bool IsClone(PlayerControl pc)
    {
        ...
        try
        {
            if (_netIds.Contains(((InnerNetObject)(object)pc).NetId))
                return true;
        }
        catch { }
        try
    ...
    internal static string CloneOf(PlayerControl src)
    {
        ...
        if (_live.Count + _pend.Count >= Max)
            return NocturneText.T("Лимит клонов.", "Clone limit.");
    ...
    internal static string Text(string text, PlayerControl src = null)
    {
        ...
        if (string.IsNullOrWhiteSpace(text))
            return NocturneText.T("Пустой текст.", "Empty text.");
        ...
        if (offs.Count == 0)
            return NocturneText.T("Пусто.", "Empty.");
    ...
    {
        if (_batches.Count == 0)
            return NocturneText.T("Нет фигур.", "No figures.");
    ...
    private void Clicks()
    {
        if (!NocturneConfig.NetCloneMode.Value)
            return;
        if (!Ready() || NocturneMenu.Opened || Camera.main == null)
            return;
        PlayerControl me = PlayerControl.LocalPlayer;
        if (me == null)
            return;
    ...
    private static void Freeze(PlayerControl pc)
    {
        ...
        {
            if (pc.Collider != null)
                ((Behaviour)pc.Collider).enabled = false;
        }
        catch { }
        try
        {
