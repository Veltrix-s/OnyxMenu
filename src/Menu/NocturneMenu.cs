...
public sealed class NocturneMenu : MonoBehaviour
{
    ...
    private void RecentPlayersCard(float x, ref float y, float w)
    {
        ...
        for (int i = 0; i < rows.Count; i++)
        {
            ...
            var r = new Rect(b.x, by, b.width, 28f);
            HoverFill(r);
            Lab(new Rect(r.x + 8f, r.y, r.width - 190f, r.height),
    ...
    private void TpRow(float x, ref float y, float w, PlayerControl pc)
    {
        ...
        HoverFill(r);
        DrawColorDot(new Rect(r.x + 6f, r.y + 9f, 12f, 12f), pc);
        Lab(new Rect(r.x + 26f, r.y, r.width - 200f, r.height), pc.Data != null ? pc.Data.PlayerName : "?", _rowName);
    ...
    private void VotekickRow(float x, ref float y, float w, PlayerControl pc)
    {
        ...
        if (NocturneStyle.Painting)
        {
            ...
            if (IsHost(pc))
                nm += "  [H]";
            Lab(new Rect(r.x + 26f, r.y, r.width - 200f, r.height), nm, _rowName);
        }
    ...
    private void CheatsSelf(float x, ref float y, float w)
    {
        ...
        b = Card(x, ref y, w, NocturneText.T("Бафы ролей", "Role buffs"), RowH * 29f + 234f + (reach ? 54f : 4f));
        by = b.y;
    ...
    private void DrawNet(float x, ref float y, float w)
    {
        ...
        {
            _fcText = FcClamp(CustomText(new Rect(b.x + 2f, by, b.width - 190f, 26f), _fcText ?? NocturneConfig.SpoofFriendCodeValue.Value ?? "", "fcSpoof", 15));
    ...
    private static string FcClamp(string s)
    {
        if (string.IsNullOrEmpty(s))
            return s;
        ...
        string name = s.Substring(0, h > 10 ? 10 : h);
        string tail = s.Substring(h + 1);
        if (tail.Length > 4)
