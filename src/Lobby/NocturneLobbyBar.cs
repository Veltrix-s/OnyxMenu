...
public sealed class NocturneLobbyBar : MonoBehaviour
{
    ...
    private struct Mote
    {
        public float X, Y, Speed, Size, Phase, Bright;
    }
    ...
    private void Update()
    {
        ...
        if (_lobbyEnteredAt < 0f)
        {
            _lobbyEnteredAt = Time.unscaledTime;
            _avatarSetUp = false;
            _avatarSig = string.Empty;
        }
    ...
    internal void DrawGui()
    {
        try
        {
            if (!Enabled || LobbyBehaviour.Instance == null)
            {
                RestoreVanilla();
                return;
            }
            ...
            if (ev != null && ev.type != EventType.Repaint && ev.type != EventType.MouseDown)
                return;
    ...
    private static bool IsChatOpen()
    {
        ...
        try
        {
            return HudManager.Instance != null && HudManager.Instance.Chat != null && HudManager.Instance.Chat.IsOpenOrOpening;
        }
        catch
        {
            return false;
    ...
    {
        if (Time.frameCount == _settingsFrame)
    ...
    private bool IsSettingsOpenUncached()
    {
        try
        {
            ...
            if (_optionsMenu != null && ((Component)_optionsMenu).gameObject.activeInHierarchy)
                return true;
        }
        catch { }
        ...
        try
        {
            ...
            if (_gameSettingMenu != null && ((Component)_gameSettingMenu).gameObject.activeInHierarchy)
                return true;
        }
        catch { }
        ...
        try
        {
            ...
            if (_customizeMenu != null && ((Component)_customizeMenu).gameObject.activeInHierarchy)
                return true;
        }
        catch { }
        ...
        try
        {
            ...
            if (_friendsMenu != null && ((Component)_friendsMenu).gameObject.activeInHierarchy)
                return true;
        }
        catch { }
    ...
    private GameStartManager ResolveGsm()
    {
        if (_gsm != null)
            return _gsm;
        if (Time.unscaledTime < _nextGsmLookup)
            return null;
    ...
    private void SuppressVanilla(GameStartManager gsm)
    {
        try
        {
            ...
            ReassertSuppressed();
            if (Time.unscaledTime < _nextSuppressScan)
                return;
            ...
            try
            {
                ...
                {
                    if (t != null && ((Behaviour)t).enabled)
                    {
                        ((Behaviour)t).enabled = false;
                        _suppressedTexts.Add(t);
                    }
                }
            }
            catch { }
    ...
    {
        foreach (SpriteRenderer r in _suppressed)
        {
            try
            {
                if (r != null && r.enabled)
        ...
        foreach (Behaviour b in _suppressedTexts)
        {
            try
            {
                if (b != null && b.enabled)
        ...
        foreach (Collider2D c in _suppressedColliders)
        {
            try
            {
                if (c != null && c.enabled)
        ...
        if (_disabledButton != null)
        {
            try
            {
                if (((Behaviour)_disabledButton).enabled)
                    ((Behaviour)_disabledButton).enabled = false;
            }
            catch { }
        }
    }
    ...
    private void RestoreVanilla()
    {
        ...
        if (_disabledButton != null)
        {
            try
            {
                ((Behaviour)_disabledButton).enabled = true;
            }
            catch { }
    ...
    private void DrawBar(GameStartManager gsm, float a)
    {
        ...
        if (_shake > 0.001f)
            x += Mathf.Sin(Time.unscaledTime * 60f) * _shake * 10f * s;

        bool amHost = false;
        try
        {
            amHost = AmongUsClient.Instance != null && AmongUsClient.Instance.AmHost;
        }
        catch { }
        ...
        if (hostText != _hostTextC || _hostFontC != _hostStyle.fontSize)
        {
            ...
            try
            {
                _hostWC = _hostStyle.CalcSize(new GUIContent(hostText)).x;
                _hostTextC = hostText;
            }
            catch
            {
        ...
        if (amHost)
        {
            ...
            if (_countingDown && now >= _countdownEndAt)
            {
                _countingDown = false;
                DoStart(gsm);
            }
            ...
            if (_countingDown)
            {
                ...
                if (rem != _lastCountSecond)
                {
                    _lastCountSecond = rem;
                    if (rem > 0)
                        PlayCountdownSound(gsm);
                }
        ...
        else
        {
            ...
            int rem = -1;
            if (Patches.NocturneStartCounter.TryGet(out int rpcSec))
                rem = rpcSec;
            else
            {
                try
                {
                    if (gsm != null && (int)gsm.startState == 1)
                        rem = Mathf.CeilToInt(Mathf.Max(0f, gsm.countDownTimer));
                }
                catch { }
            }
            if (rem >= 0)
                waitLabel = NocturneText.T("СТАРТ  (", "START  (") + rem + ")";
        ...
        if (Time.frameCount != _chipFrame)
        {
            ...
            if (remain <= 60f)
            {
                ...
                _timeCol = Color.Lerp(accent, Color.white, 0.18f);
        }
    ...
    {
        if (_moteStyle == null || !NocturneConfig.LobbyAnims.Value)
            return;
    ...
    private void DrawMedallion(Rect r, Color body, float a)
    {
        if (_avatarReady && _avatarTex != null && _avatarBoxStyle != null)
        {
            ...
            DrawCrewIcon(new Rect(r.x + r.width * 0.14f, r.y + r.height * 0.12f, r.width * 0.72f, r.height * 0.80f), body);
    ...
    {
        if (ping < 0)
            return new Color(0.70f, 0.72f, 0.78f, 1f);
        if (ping <= 90)
            return new Color(0.40f, 0.92f, 0.50f, 1f);
        if (ping <= 180)
    ...
    {
        if (_avatarReady && _avatarPlayer != null && _avatarCam != null && _avatarRT != null && _avatarTex != null)
            return true;

        try
        {
            HudManager hud = DestroyableSingleton<HudManager>.Instance;
            if (hud == null || hud.IntroPrefab == null || hud.IntroPrefab.PlayerPrefab == null)
                return false;
            ...
            if (_avatarRT == null)
            {
                _avatarRT = new RenderTexture(192, 224, 16, RenderTextureFormat.ARGB32) { hideFlags = HideFlags.HideAndDontSave };
                _avatarRT.Create();
                Object.DontDestroyOnLoad(_avatarRT);
            }
            if (_avatarTex == null)
            {
                _avatarTex = new Texture2D(192, 224, TextureFormat.RGBA32, false)
                {
                    filterMode = FilterMode.Bilinear,
                    hideFlags = HideFlags.HideAndDontSave,
                };
                Object.DontDestroyOnLoad(_avatarTex);
            }
    ...
    private void UpdateAvatar()
    {
        try
        {
            ...
            if (now >= _avatarSettleUntil)
                return;
            if (now < _nextAvatarRender)
                return;
            _nextAvatarRender = now + 0.25f;
            _avatarHolder.transform.position = AvatarPos;

            if (!_avatarRT.IsCreated())
            {
                _avatarRT.Create();
                _avatarCam.targetTexture = _avatarRT;
            }
    ...
    private void DoStart(GameStartManager gsm)
    {
        try
        {
            ...
            try
            {
                Patches.StartControl.UnlockStartButton(gsm);
            }
            catch { }
            ...
            try
            {
                ok = Patches.StartControl.TryInstantStart(gsm);
            }
            catch { }
            if (!ok)
                gsm.BeginGame();
    ...
    {
        float now = Time.unscaledTime;
        if (_hostNameC != null && now - _hostNameAt < 0.5f)
            return _hostNameC;
        _hostNameAt = now;
        try
        {
    ...
    private void AdvanceTimeRoll(string value)
    {
        ...
        if (value != _timeShown)
        {
            _timePrev = _timeShown;
            _timeShown = value;
            _timeRoll = 0f;
        }
        if (_timeRoll < 1f)
            _timeRoll = Mathf.Min(1f, _timeRoll + Time.unscaledDeltaTime * 2.4f);
    }
    ...
    private static int _bakeAccentSig = int.MinValue;
    private static int _pendAccentSig = int.MinValue;
    private static float _pendAccentAt = -99f;

    private static int AccentSig()
    {
        Color c = Accent;
    ...
    {
        int accSig = AccentSig();
        if (accSig != _bakeAccentSig)
        {
            if (accSig != _pendAccentSig)
            {
                _pendAccentSig = accSig;
                _pendAccentAt = Time.unscaledTime;
            }
            else if (Time.unscaledTime - _pendAccentAt >= 0.2f)
            {
                _bakeAccentSig = accSig;
                if (_panelBakeTex != null)
                    Object.Destroy(_panelBakeTex);
                _panelBakeTex = null;
                for (int k = 0; k < 3; k++)
                {
                    if (_chipIconTex[k] != null)
                        Object.Destroy(_chipIconTex[k]);
                    _chipIconTex[k] = null;
                }
            }
        }
        ...
        try
        {
            ...
            if (_panelTex == null)
                _panelTex = RoundedTex(new Color(0.035f, 0.035f, 0.037f, 0.98f), 20);
            if (_btnTex == null)
                _btnTex = RoundedTex(new Color(1f, 1f, 1f, 1f), 16);
            if (_chipTex == null)
                _chipTex = RoundedTex(new Color(0.09f, 0.09f, 0.095f, 0.96f), 14);
            if (_glowTex == null)
                _glowTex = RoundedTex(new Color(1f, 1f, 1f, 1f), 16);
            if (_lineTex == null)
                _lineTex = Solid(new Color(1f, 1f, 1f, 1f));
            if (_glossTex == null)
                _glossTex = RoundedVFadeTex(new Color(1f, 1f, 1f, 1f), true, 18);
            if (_moteTex == null)
                _moteTex = SoftDotTex(16);
            ...
            for (int k = 0; k < 3; k++)
                if (_chipIconTex[k] == null)
                    _chipIconTex[k] = BakeChipIcon(k, 48);
    ...
    private static Texture2D BakePanel(int w, int h, float s)
    {
        ...
        for (int yimg = 0; yimg < h; yimg++)
            for (int xp = 0; xp < w; xp++)
            {
                ...
                if (mask <= 0f)
                {
                    px[(h - 1 - yimg) * w + xp] = new Color(0f, 0f, 0f, 0f);
                    continue;
                }
                ...
                if (fx >= gi && fx <= w - gi && fy >= gradTop && fy <= gradBot)
                {
                    ...
                    if (sa > 0f)
                    {
                        oR = warm.r * sa + oR * (1f - sa);
                        oG = warm.g * sa + oG * (1f - sa);
                        oB = warm.b * sa + oB * (1f - sa);
                        oA = sa + oA * (1f - sa);
                    }
                }
        ...
        if (kind == 0)
        {
            ...
            for (int y = 0; y < size; y++)
                for (int x = 0; x < size; x++)
                {
                    ...
                    if (Mathf.Abs(fx - cx) <= st * 0.5f && fy >= cy && fy <= cy + u * 0.26f)
                        aa = 1f;
                    if (Mathf.Abs(fy - cy) <= st * 0.5f && fx >= cx && fx <= cx + u * 0.22f)
                        aa = 1f;
                    if (aa > 0f)
                        px[y * size + x] = new Color(1f, 1f, 1f, Mathf.Clamp01(aa));
                }
        }
        else if (kind == 1)
        {
