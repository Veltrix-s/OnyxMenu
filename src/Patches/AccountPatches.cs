...
internal static class AccountGate
{
    ...
    {
        if (NocturneConfig.RemoveGuestLimits.Value)
            result = true;
    }
    ...
    internal static void WipePenalty(PlayerBanData data)
    {
        if (data != null && ShouldNeutralizeDisconnectPenalty())
            data.banPoints = 0f;
    }
}
...
internal static class DisconnectBanFlagPatch
{
    public static void Prefix(PlayerBanData __instance) => AccountGate.WipePenalty(__instance);
}

[HarmonyPatch(typeof(PlayerBanData), nameof(PlayerBanData.BanMinutes), MethodType.Getter)]
internal static class DisconnectBanMinutesPatch
{
    public static void Prefix(PlayerBanData __instance) => AccountGate.WipePenalty(__instance);
}

[HarmonyPatch(typeof(PlayerBanData), nameof(PlayerBanData.BanMinutesLeft), MethodType.Getter)]
internal static class DisconnectBanLeftPatch
{
    public static void Prefix(PlayerBanData __instance) => AccountGate.WipePenalty(__instance);
}
