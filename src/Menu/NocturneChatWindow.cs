...
public sealed class NocturneChatWindow : MonoBehaviour
{
    ...
    internal static void Feed(PlayerControl src, string chatText)
    {
        ...
        try
        {
            ...
            string key = sn + "|" + st;
            if (key == _lastKey && now - _lastAt < 0.75f)
                return;
            _lastKey = key;
            _lastAt = now;
    ...
    internal void DrawGui()
    {
        ...
        try
        {
            ...
            if (Hover(clr))
                NocturneStyle.FillRounded(clr, A(Color.white, 0.07f), 6);
            if (Hover(cls))
            ...
            if (GUI.Button(cpy, GUIContent.none, GUIStyle.none))
                CopyAll();
            ...
            if (symH > 0f)
            {
                ...
                for (int i = 0; i < Syms.Length; i++)
                {
                    ...
                    if (Btn(sb, Syms[i], A(Color.white, Hover(sb) ? 0.14f : 0.07f)))
                        if (_text.Length + Syms[i].Length <= MaxInput)
                            _text += Syms[i];
                }
            }
            ...
            if (Btn(send, NocturneText.T("Отпр.", "Send"), Hover(send) ? A(p.Accent, 0.9f) : A(p.Accent, 0.72f)))
                Send();
            if (Btn(symBtn, "☺ +", _syms ? A(p.Accent, 0.7f) : A(Color.white, Hover(symBtn) ? 0.14f : 0.07f)))
                _syms = !_syms;
    ...
    private void InputBox(Rect r, NocturnePalette p, Event e)
    {
        ...
        if (e != null && e.type == EventType.MouseDown)
        {
            if (r.Contains(e.mousePosition))
                _edit = true;
            else if (!_win.Contains(e.mousePosition))
                _edit = false;
        }
    ...
    {
        if (Mathf.Abs(textW - _measuredAt) < 0.5f)
            return;
        _measuredAt = textW;
        for (int i = 0; i < _lines.Count; i++)
            _lines[i].H = 0f;
