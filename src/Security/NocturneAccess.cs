...
internal static class NocturneAccess
{
    ...
    internal static int WhiteCount
    {
        get
        {
            EnsureLoaded();
    ...
    internal static int NickBanCount
    {
        get
        {
            EnsureLoaded();
            return NickBans.Count;
        }
    }
    internal static int PlatformBanCount
    {
        get
        {
            EnsureLoaded();
    ...
    internal static IReadOnlyList<AccessEntry> BanEntries
    {
        get
        {
            EnsureLoaded();
    ...
    internal static IReadOnlyList<AccessEntry> WhiteEntries
    {
        get
        {
            EnsureLoaded();
    ...
    internal static IReadOnlyList<string> NickBanEntries
    {
        get
        {
            EnsureLoaded();
    ...
    internal static IReadOnlyList<string> PlatformBanEntries
    {
        get
        {
            EnsureLoaded();
    ...
    internal static bool IsBanned(string fc, string puid)
    {
        EnsureLoaded();
        return Has(Bans, fc, puid);
    }
    internal static bool IsWhite(string fc, string puid)
    {
        EnsureLoaded();
        return Has(Whites, fc, puid);
    }
    internal static bool IsNickBanned(string name)
    {
        EnsureLoaded();
        return HasNick(NickBans, name);
    }
    internal static bool IsPlatformBanned(string raw)
    {
        EnsureLoaded();
        return HasPlatform(PlatformBans, raw);
    }
    ...
    {
        EnsureLoaded();
        if ((string.IsNullOrWhiteSpace(fc) && string.IsNullOrWhiteSpace(puid)) || Has(Bans, fc, puid))
            return;
    ...
    {
        EnsureLoaded();
        if ((string.IsNullOrWhiteSpace(fc) && string.IsNullOrWhiteSpace(puid)) || Has(Whites, fc, puid))
            return;
    ...
    internal static void RemoveBan(string key)
    {
        EnsureLoaded();
        if (RemoveByKey(Bans, key))
            Persist(NocturneConfig.BanList, Bans, BanTxt);
    }
    internal static void RemoveWhite(string key)
    {
        EnsureLoaded();
        if (RemoveByKey(Whites, key))
            Persist(NocturneConfig.WhiteList, Whites, WhiteTxt);
    }
    ...
    internal static void AddNickBan(string name)
    {
        ...
        if (clean.Length == 0 || HasNick(NickBans, clean))
            return;
        NickBans.Add(clean);
        PersistNicks();
    }
    ...
    internal static void AddPlatformBan(string raw)
    {
        ...
        if (clean.Length == 0 || HasPlatform(PlatformBans, clean))
            return;
    ...
    internal static void RemovePlatformBan(string raw)
    {
        ...
        for (int i = PlatformBans.Count - 1; i >= 0; i--)
            if (PlatformKey(PlatformBans[i]) == key)
            {
                PlatformBans.RemoveAt(i);
                PersistPlatforms();
                return;
            }
    }
    ...
    private static void PersistPlatforms()
    {
        ...
        {
            var sb = new StringBuilder();
            for (int i = 0; i < PlatformBans.Count; i++)
            {
                if (i > 0)
                    sb.Append(';');
                sb.Append(Esc(PlatformBans[i]));
            }
        ...
        try
        {
            ...
            for (int i = 0; i < PlatformBans.Count; i++)
                sb2.Append(PlatformBans[i]).Append('\n');
    ...
    private static void PersistNicks()
    {
        ...
        {
            var sb = new StringBuilder();
            for (int i = 0; i < NickBans.Count; i++)
            {
                if (i > 0)
                    sb.Append(';');
                sb.Append(Esc(NickBans[i]));
            }
    ...
    private static int ImportFile(string path, List<AccessEntry> list)
    {
        try
        {
            ...
            foreach (string raw in File.ReadAllLines(path))
            {
                ...
                {
                    name = p[0].Trim();
                    fc = p.Length >= 2 ? p[1].Trim() : string.Empty;
                    puid = p.Length >= 3 ? p[2].Trim() : string.Empty;
                }
