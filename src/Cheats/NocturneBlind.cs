...
internal static class NocturneBlind
{
    ...
    private static string Mass(Func<PlayerControl, string> action)
    {
        if (_selected.Count == 0)
            return NocturneText.T("никто не отмечен", "nobody selected");
        int total = _selected.Count, n = 0;
        try
        {
            foreach (PlayerControl pc in PlayerControl.AllPlayerControls)
            {
                if (pc == null || pc.Data == null || !_selected.Contains(pc.PlayerId))
                    continue;
                if (action(pc).StartsWith(NocturneText.T("готово", "done")))
                    n++;
            }
        }
        catch { }
    ...
    private static bool HasAnticheat()
    {
        try
        {
            return AmongUsClient.Instance != null && (int)AmongUsClient.Instance.NetworkMode == 1;
        }
        catch
        {
            return false;
    ...
    private static string GuardTarget(PlayerControl target)
    {
        ...
        if (!Ready(target))
            return target == null || target.Data == null ? NocturneText.T("нет цели", "no target") : NocturneText.T("только в матче", "in-match only");
        return GuardSelf();
    }
    ...
    internal static string Restore(PlayerControl target)
    {
        ...
        try
        {
            ...
            if (real == null || !Push(real, target.OwnerId))
                return NocturneText.T("не удалось", "failed");
    ...
    private static int LogicOptionsIndex()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < list.Count; i++)
                if (list[i].GetType() == typeof(LogicOptions))
                {
                    _logicIndex = i;
                    return _logicIndex;
                }
