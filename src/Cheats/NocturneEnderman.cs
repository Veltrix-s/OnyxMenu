...
internal static class NocturneEnderman
{
    ...
    private static bool Imp(PlayerControl pc)
    {
        try
        {
            return pc.Data != null && pc.Data.Role != null && pc.Data.Role.IsImpostor && pc.Data.Role.Role != RoleTypes.Viper;
        }
        catch
        {
            return false;
    ...
    {
        if (Running)
            return NocturneText.T("Занято.", "Busy.");
        if (!Ready(out PlayerControl me))
            return NocturneText.T("Только импостером, в матче.", "Impostor only, in match.");
        if (!Target(target, me))
        ...
        try
        {
            ((MonoBehaviour)AmongUsClient.Instance).StartCoroutine(Co(target).WrapToIl2Cpp());
        }
        catch
        {
    ...
    internal static void Restore()
    {
        ...
        try
        {
            ...
            {
                me.NetTransform.SnapTo(_backPos);
                me.NetTransform.RpcSnapTo(_backPos);
            }
        }
        catch { }
        try
        {
            if (!string.IsNullOrEmpty(_backup))
                NocturneOutfits.Apply(me, _backup);
        }
        catch { }
    }
}
