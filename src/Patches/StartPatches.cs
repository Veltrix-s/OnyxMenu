...
internal static class StartControl
{
    ...
    internal static void UnlockStartButton(GameStartManager gsm)
    {
        ...
        gsm.MinPlayers = 1;
        if (gsm.StartButton != null)
            gsm.StartButton.SetButtonEnableState(true);
        if (gsm.GameStartText != null)
            ((TMP_Text)gsm.GameStartText).color = Color.white;
    }

    internal static void HandleQuickStartOnEnter(GameStartManager gsm)
    {
        if (gsm == null || AmongUsClient.Instance == null || LobbyBehaviour.Instance == null)
            return;
        if (!AmongUsClient.Instance.AmHost)
            return;
        if (!Input.GetKeyDown(KeyCode.Return) && !Input.GetKeyDown(KeyCode.KeypadEnter))
            return;
        if (IsEnterStartBlockedByChat())
            return;
        ...
        if (!NocturneConfig.QuickStartOnEnter.Value)
            return;
        gsm.MinPlayers = 1;
    ...
    private static bool TextHasDigit(object raw)
    {
        ...
        if (text == null || !((Component)text).gameObject.activeInHierarchy)
            return false;
    ...
    private static void ForceTimersToZero(GameStartManager gsm)
    {
        ...
        {
            SetFloatField(gsm, "countDownTimer", 0f);
        }
