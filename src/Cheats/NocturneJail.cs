...
internal static class NocturneJail
{
    ...
    internal static void ToggleTarget(byte pid)
    {
        if (!_targets.Remove(pid))
            _targets.Add(pid);
    }
        ...
        try
        {
            ...
            while (e.MoveNext())
            {
                ...
                if (pc == null || pc.Data == null || pc.Data.Disconnected || pc.Data.IsDead)
                    continue;
                if (!_targets.Contains(pc.PlayerId))
                    continue;
                if (cell.OverlapPoint(pc.GetTruePosition()))
                    continue;
    ...
    private static void SendToVent(PlayerControl target, int ventId)
    {
        try
        {
            ...
            if (net == null || target == null || target.MyPhysics == null)
                return;
