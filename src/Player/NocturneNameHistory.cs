...
internal static class NocturneNameHistory
{
    ...
    {
        try { return NocturneNameColor.Strip(p.Data.PlayerName ?? string.Empty).Trim(); }
...
public sealed class NocturneHistoryTracker : MonoBehaviour
{
    ...
    public void Update()
    {
        if (NocturneConfig.NameHistory == null || !NocturneConfig.NameHistory.Value) return;
        ...
        try
        {
            ...
            if (net.GameId != _room) { _room = net.GameId; _greeted.Clear(); }
            bool greet = NocturneConfig.NotifyKnownPlayer != null && NocturneConfig.NotifyKnownPlayer.Value;
            ...
            while (e.MoveNext())
            {
                ...
                if (prev != null) NocturneToast.Push(NocturneText.T("Смена ника", "Nick change"), $"{prev} → {NocturneNameHistory.NickOf(c)}", 4f, NocturneNotifyKind.Info);
            }
        }
        catch { }
    }
    ...
    private static void Greet(ClientData c, string since, string oldNick)
    {
        ...
        bool joinToast = NocturneConfig.JoinDetect != null && NocturneConfig.JoinDetect.Value;
