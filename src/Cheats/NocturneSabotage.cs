...
public sealed class NocturneSabotage : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        int m = Map();
        if (NocturneConfig.SabAutoLights.Value && m != 5)
            Lights();
        if (NocturneConfig.SabInfMushroom.Value && m == 5)
            Mush();
        if (NocturneConfig.SabSpamReactor.Value)
            Main();
        if (NocturneConfig.SabAutoFix.Value && !Mine() && AnyActive())
            Fix();
    }
    ...
    private static bool AnyActive()
    {
        ...
        for (int i = 0; i < Watch.Length; i++)
        {
            try
            {
                if (!ss.Systems.ContainsKey(Watch[i]))
                    continue;
    ...
    public static void Main()
    {
        ...
        {
            case 2:
                Up(SystemTypes.Laboratory, 128);
                break;
            case 4:
                Up(SystemTypes.HeliSabotage, 128);
                break;
            default:
                Up(SystemTypes.Reactor, 128);
                break;
        }
    }
    ...
    private static void Up(SystemTypes sys, byte amt)
    {
        ...
        {
            if (ShipStatus.Instance != null)
                ShipStatus.Instance.RpcUpdateSystem(sys, amt);
        }
        catch { }
    }
