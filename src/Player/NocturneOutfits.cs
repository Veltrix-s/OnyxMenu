...
internal static class NocturneOutfits
{
    ...
    internal static int MaxColor()
    {
        try
        {
            return Palette.PlayerColors != null ? Mathf.Max(0, Palette.PlayerColors.Length - 1) : 11;
        }
        catch
        {
            return 11;
    ...
    {
        if (!Usable(target) || string.IsNullOrWhiteSpace(data))
            return false;
        string[] p = data.Split(Sep);
        if (p.Length < 6 || !int.TryParse(p[0], out int col))
            return false;
    ...
    internal static void SetColor(PlayerControl p, int col)
    {
        ...
        try
        {
            host = AmongUsClient.Instance != null && AmongUsClient.Instance.AmHost;
        }
        catch { }
    ...
    {
        if (!Usable(target))
            return NocturneText.T("нет игрока", "no player");
        ...
        if (NocturneConfig.RandomOutfitSave.Value)
            SaveProfile(col, hat, skin, visor, pet);
...
public sealed class NocturneOutfitApplier : MonoBehaviour
{
    ...
    private bool _prevInLobby;

    internal static bool Borrowed => _borrowed;

    internal static void Borrow(PlayerControl src)
    {
        PlayerControl me = PlayerControl.LocalPlayer;
        if (!NocturneOutfits.Usable(src) || !NocturneOutfits.Usable(me))
            return;
        if (!_borrowed)
        {
            _original = NocturneOutfits.Capture(me);
            _borrowed = true;
        }
        string data = NocturneOutfits.Capture(src);
        if (!string.IsNullOrEmpty(data))
            NocturneOutfits.Apply(me, data);
    }

    internal static void Restore() => _restorePending = true;
    ...
    {
        bool inGame = false, inLobby = false;
        try
        {
            inGame = ShipStatus.Instance != null;
            inLobby = LobbyBehaviour.Instance != null;
        }
        catch { }

        if (_borrowed)
        {
            if (_prevInGame && !inGame && NocturneConfig.QuickOutfitResetMatch.Value)
                _restorePending = true;
            else if (_prevInLobby && !inLobby && !inGame && NocturneConfig.QuickOutfitResetLobby.Value)
                Forget();
        }

        _prevInGame = inGame;
        _prevInLobby = inLobby;
        ...
        if (_restorePending)
        {
            ...
            {
                PlayerControl me = PlayerControl.LocalPlayer;
                if (NocturneOutfits.Usable(me))
                {
                    NocturneOutfits.Apply(me, _original);
                    _borrowed = false;
                    _original = null;
                    _restorePending = false;
                }
    ...
    public void Update()
    {
        ...
        try
        {
            ...
            {
                case 0:
                    NocturneOutfits.SetColor(pc, s.Col);
                    break;
                case 1:
                    pc.RpcSetHat(s.Val);
                    break;
                case 2:
                    ...
                    break;
                case 3:
                    pc.RpcSetVisor(s.Val);
                    break;
                case 4:
                    pc.RpcSetPet(s.Val);
                    break;
                case 5:
                    pc.RpcSetNamePlate(s.Val);
                    break;
            }
        }
        catch { }
