...
internal static class DeviceIdSpoofPatch
{
    ...
    {
        if (!NocturneConfig.SpoofDeviceId.Value) return;
...
internal static class NocturneFcRegisterPatch
{
    ...
    private static string Fallback(string value)
    {
        ...
        if (hash <= 0) return plain;

        string suffix = plain.Substring(hash + 1);
        if (suffix.Length == 0) return plain.Substring(0, hash);
        for (int i = 0; i < suffix.Length; i++)
            if (suffix[i] < '0' || suffix[i] > '9') return plain.Substring(0, hash);
        return plain;
    }
}
