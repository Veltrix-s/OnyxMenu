...
public sealed class NocturneModStampDriver : MonoBehaviour
{
    ...
    public void LateUpdate()
    {
        ...
        if (_mgr == null) return;
        if (NocturneConfig.HideModStamp.Value)
            NocturneModStamp.Hide(_mgr);
        else
            NocturneModStamp.Show(_mgr);
    }
}
...
internal static class NocturneModStamp
{
    ...
    internal static void Show(ModManager mgr)
    {
        ...
        try
        {
            ...
            if (stamp != null && ((Renderer)stamp).enabled)
                return;
        }
        catch { }
...
internal static class NocturneModStampPatch
{
    public static void Postfix(ModManager __instance)
    {
        try
        {
            ...
            if (NocturneConfig.HideModStamp.Value)
                NocturneModStamp.Hide(__instance);
            else
                NocturneModStamp.Show(__instance);
        }
        catch { }
    }
