...
public sealed class NocturneRoleBuffs : MonoBehaviour
{
    ...
    private void Aura(PlayerControl me)
    {
        ...
        if (me.killTimer > 0.05f && !(NocturneConfig.BuffNoKillCd.Value && Utils.Host))
            return;
...
internal static class BuffKillCdPatch
{
    public static void Prefix(PlayerControl __instance, ref float __0)
    {
        if (__instance != PlayerControl.LocalPlayer)
            return;
        if (NocturneConfig.BuffNoKillCd.Value && Utils.Host)
            __0 = 0f;
    }
}
...
internal static class BuffShiftDeadPatch
{
    private static readonly List<byte> Flipped = new List<byte>();
    ...
    private static void Restore()
    {
        ...
        var all = GameData.Instance != null ? GameData.Instance.AllPlayers : null;
        if (all != null)
        {
            for (int i = 0; i < all.Count; i++)
            {
                NetworkedPlayerInfo info = all[i];
                if (info != null && Flipped.Contains(info.PlayerId))
                    info.IsDead = true;
            }
        }
        Flipped.Clear();
    ...
    {
        Restore();
        if (!NocturneConfig.BuffSsDead.Value || GameData.Instance == null)
            return;
