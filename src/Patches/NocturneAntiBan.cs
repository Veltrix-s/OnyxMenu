...
internal static class NocturneAntiBan
{
    ...
    private static bool Prefix(byte callId, MessageReader reader)
    {
        ...
        try
        {
            net = (InnerNetClient)AmongUsClient.Instance;
        }
        catch
        {
            return true;
        ...
        if (Inspect(reader, out PlayerControl src) != 2)
            return true;
        ...
        if (!NocturneConfig.AntiBanHost.Value)
            return true;
    ...
    private static int Inspect(MessageReader reader, out PlayerControl src)
    {
        ...
        try
        {
            ...
            if ((int)(SystemTypes)r.ReadByte() != 37)
                return 0;
            ...
            if (op == 2 && num == 0 && vid == 0)
            {
                _pending[owner] = Time.realtimeSinceStartup;
                return 1;
            }
            if (op != 5 || num != 1 || vid != 0)
                return 0;
            if (!_pending.TryGetValue(owner, out float t))
                return 0;
