...
public sealed class NocturneReplay : MonoBehaviour
{
    ...
    internal static bool Open => NocturneConfig.ReplayView != null && NocturneConfig.ReplayView.Value;
    private static int Mask => NocturneConfig.ReplayFilterMask != null ? NocturneConfig.ReplayFilterMask.Value : 255;
    ...
    public void Update()
    {
        ...
        try
        {
            ...
            {
                _wasMeeting = false;
                if (NocturneConfig.ReplayClearAfterMeeting != null && NocturneConfig.ReplayClearAfterMeeting.Value) Clear();
                else ClearPaths();
            }
    ...
    private void KeyToggle()
    {
        try
        {
            KeyCode k = NocturneConfig.ReplayKey != null ? NocturneConfig.ReplayKey.Value : KeyCode.None;
            if (k == KeyCode.None) { _keyPrev = false; return; }
            bool down = Input.GetKey(k);
            if (down && !_keyPrev && NocturneConfig.ReplayView != null && !NocturneMenu.Typing)
    ...
    internal void DrawGui()
    {
        ...
        try
        {
            ...
            _win.width = Mathf.Clamp(_win.width, 360f, sw);
            _win.height = Mathf.Clamp(_win.height, 260f, sh);
            ...
            for (int i = 0; i < cols; i++)
            {
                ...
                if (GUI.Button(fr, GUIContent.none, GUIStyle.none) && NocturneConfig.ReplayFilterMask != null)
