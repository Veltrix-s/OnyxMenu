...
internal static class NocturneVentTp
{
    ...
    {
        if (!_marked.Remove(pid)) _marked.Add(pid);
    }
    ...
    internal static string KickAllFromVents()
    {
        ...
        try
        {
            ...
            while (e.MoveNext())
            {
                ...
                int idx = Utils.NearestVentIndex(p.GetTruePosition());
                if (idx < 0) continue;
                BootOnly(p, idx);
                n++;
            }
    ...
    private static byte VentId(int index)
    {
        try
        {
            ...
            if (vents == null || vents.Count == 0) return 0;
    ...
    internal static void EnforceVentMode()
    {
        ...
        {
            PlayerControl p = e.Current;
            if (p == null || p.Data == null || p.Data.Disconnected || p.Data.IsDead || !p.inVent) continue;

            bool imp = p.Data.Role != null && p.Data.Role.IsImpostor;
            if (VentAllowed(mode, imp)) continue;
