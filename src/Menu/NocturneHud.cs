...
public sealed class NocturneHud : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (NocturneConfig.CopyCodeKey.Value != KeyCode.None && Input.GetKeyDown(NocturneConfig.CopyCodeKey.Value))
            CopyLobbyCode();

        if (NocturneConfig.EndMatchKey.Value != KeyCode.None && Input.GetKeyDown(NocturneConfig.EndMatchKey.Value))
            TryEndMatch();
        ...
        if (!NocturneMenu.Rebinding)
        {
            ...
            if (NocturneConfig.MeetingRoamKey.Value != KeyCode.None && Input.GetKeyDown(NocturneConfig.MeetingRoamKey.Value))
            ...
            if (NocturneConfig.ZiplineSelectKey.Value != KeyCode.None && Input.GetKeyDown(NocturneConfig.ZiplineSelectKey.Value))
            {
                RideTargets.All();
            ...
            if (NocturneConfig.ZiplineDownKey.Value != KeyCode.None && Input.GetKeyDown(NocturneConfig.ZiplineDownKey.Value))
                NocturneToast.Push(NocturneText.T("Зиплайн", "Zipline"), Zipline.RideSelected(true), 2.5f, NocturneNotifyKind.Info);
            if (NocturneConfig.ZiplineUpKey.Value != KeyCode.None && Input.GetKeyDown(NocturneConfig.ZiplineUpKey.Value))
    ...
    private static void TryEndMatch()
    {
        if (AmongUsClient.Instance == null || !AmongUsClient.Instance.AmHost)
            return;
        if (ShipStatus.Instance == null || LobbyBehaviour.Instance != null || GameManager.Instance == null)
            return;
        try
        {
    ...
    internal static void RenderTracker(PingTracker tracker)
    {
        ...
        if (NocturneConfig.LobbyBar.Value && LobbyBehaviour.Instance != null)
        {
            if (_lastTracker != string.Empty)
            {
                tracker.text.text = string.Empty;
                _lastTracker = string.Empty;
            }
            return;
        }
        ...
        try
        {
            if (AmongUsClient.Instance != null)
                ping = ((InnerNetClient)AmongUsClient.Instance).Ping;
        }
        catch
        {
            ping = 0;
