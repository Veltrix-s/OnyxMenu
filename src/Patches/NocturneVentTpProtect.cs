...
internal static class NocturneVentTpProtectPhysics
{
    private static bool Prefix(PlayerPhysics __instance, [HarmonyArgument(0)] byte callId)
    {
        try
        {
            if (__instance == null || callId != (byte)RpcCalls.BootFromVent)
                return true;
            if (!NocturneConfig.VentTpProtect.Value || AmongUsClient.Instance == null)
                return true;
