...
internal static class NocturneAuthor
{
    ...
    private struct Mark
    {
        public bool Ok; public float Until;
    }
    ...
    internal static bool Is(PlayerControl pc)
    {
        ...
        if (byPlayer.TryGetValue(pid, out Mark m) && now < m.Until)
            return m.Ok;
    ...
    private static bool Resolve(PlayerControl pc)
    {
        try
        {
            if (pc.Data != null && (Match(pc.Data.FriendCode) || Match(pc.Data.Puid)))
                return true;
        }
        catch { }
        ...
        try
        {
            ...
            if (c != null && (Match(c.FriendCode) || Match(c.ProductUserId)))
                return true;
        }
        catch { }
    ...
    internal static bool Match(string id)
    {
        ...
        if (seen.TryGetValue(key, out bool known))
            return known;
