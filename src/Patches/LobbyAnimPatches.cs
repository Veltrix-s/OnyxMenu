...
internal static class NocturneLobbyAnim
{
    ...
    {
        public Transform T; public Vector3 Pos; public Vector3 Scale;
    }
    ...
    {
        if (m == null) return;
        bool panel = !NocturneConfig.Vanilla && NocturneConfig.LobbyAnims.Value;
        bool start = !NocturneConfig.Vanilla && NocturneConfig.LobbyStartAnim.Value;
        if (!panel && !start)
        {
            RestoreAll();
            Reset();
        ...
        if (panel && el <= PanelDelay + PanelDuration + 0.2f) AnimatePanel(m, el);

        if (start)
            AnimateStart(m.StartButton, Mathf.Clamp01(el / 0.85f));
        else
            RestoreStart(m.StartButton);
    }
    ...
    private static void RestoreStart(PassiveButton b)
    {
        if (b == null) return;
        Transform t = ((Component)b).transform;
        t.localScale = Capture(t).Scale;
    }
    ...
    private static bool HasParent(Transform t)
    {
        for (int i = 0; i < targets.Count; i++)
        {
            ...
            if (o != null && o != t && t.IsChildOf(o))
                return true;
        }
        return false;
    }

    private static void Add(GameObject go)
    {
        if (go != null) AddT(go.transform);
    }
    private static void Add(SpriteRenderer r)
    {
        if (r != null)
            AddT(r.transform);
    }
    private static void Add(TMPro.TMP_Text t)
    {
        if (t != null) AddT(t.transform);
    }
    private static void AddB(PassiveButton b)
    {
        if (b != null)
            AddT(((Component)b).transform);
    }
...
{
    private sealed class SState
    {
        public Transform T; public Vector3 Scale; public float Last = float.NaN;
    }
    ...
    internal static void Apply(GameStartManager m)
    {
        if (m == null || LobbyBehaviour.Instance == null)
        {
            RestoreAll();
            Reset();
            return;
    ...
    private static Camera Cam()
    {
        if (cam == null || Time.unscaledTime >= nextCam)
        {
            cam = Camera.main;
            nextCam = Time.unscaledTime + 1f;
        }
        return cam;
    }
    ...
    {
        if (b == null || (UnityEvent)(object)b.OnClick == null)
            return;
    ...
    {
        viewStart = Time.time;
        viewUntil = Time.time + ClickDur;
    }
    private static void OnEdit()
    {
        editStart = Time.time;
        editUntil = Time.time + ClickDur;
    }
    ...
    private static void Anim(PassiveButton b, float clickStart, float clickUntil, bool hover)
    {
        ...
        if (hover)
            scale *= 1f + (0.5f + 0.5f * Mathf.Sin(now * 3.7f)) * 0.032f;
...
public sealed class NocturneLobbyAnimDriver : MonoBehaviour
{
    ...
    {
        bool panel = !NocturneConfig.Vanilla && NocturneConfig.LobbyAnims.Value;
        if ((!panel && (NocturneConfig.Vanilla || !NocturneConfig.LobbyStartAnim.Value)) || LobbyBehaviour.Instance == null)
        {
