...
internal static class NocturneAccess
{
    ...
    internal static void BanClient(InnerNetClient net, ClientData c)
    {
        ...
        Kick(net, c.Id, true);
        NocturneToast.Push("Бан-лист", SafeName(c), 2.5f, NocturneNotifyKind.Danger);
    }
    ...
    internal static void WhiteClient(ClientData c)
    {
        ...
        NocturneToast.Push("Вайтлист", SafeName(c), 2.5f, NocturneNotifyKind.Success);
    }
    ...
    internal static void NickBanClient(InnerNetClient net, ClientData c)
    {
        ...
        Kick(net, c.Id, true);
        NocturneToast.Push("Ник-бан", SafeName(c), 2.5f, NocturneNotifyKind.Danger);
    }

    internal static void PlatformBanClient(InnerNetClient net, ClientData c)
    {
        if (c == null) return;
        string raw = SafePlatform(c);
        if (raw.Length == 0) { NocturneToast.Push(NocturneText.T("Платформ-бан", "Platform ban"), NocturneText.T("У игрока нет имени платформы.", "Player has no platform name."), 2.5f, NocturneNotifyKind.Warning); return; }
        AddPlatformBan(raw);
        Kick(net, c.Id, true);
        NocturneToast.Push(NocturneText.T("Платформ-бан", "Platform ban"), raw, 2.5f, NocturneNotifyKind.Danger);
    }
    ...
    internal static void Enforce(InnerNetClient net, ClientData client)
    {
        try
        {
            ...
            if (NocturneConfig.AccessBanEnabled.Value && IsBanned(fc, puid))
            {
                ...
                NocturneToast.Push("Бан-лист", SafeName(client), 3f, NocturneNotifyKind.Danger);
                return;
            }
            ...
            if (NocturneConfig.AccessNickBanEnabled.Value && IsNickBanned(SafeName(client)))
            {
                Touch(client.Id);
                Kick(net, client.Id, true);
                NocturneToast.Push("Ник-бан", SafeName(client), 3f, NocturneNotifyKind.Danger);
                return;
            }
            ...
            if (NocturneConfig.AccessWhitelistOnly.Value && WhiteCount > 0 && !IsWhite(fc, puid))
            {
                ...
                NocturneToast.Push("Не в вайтлисте", SafeName(client), 3f, NocturneNotifyKind.Warning);
                return;
            }
