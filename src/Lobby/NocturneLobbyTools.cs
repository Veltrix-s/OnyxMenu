...
internal static class NocturneLobbyTools
{
    internal static string DespawnMap()
    {
        ...
        if (ship == null)
            return NocturneText.T("Карты сейчас нет.", "No map right now.");
        try
        {
    ...
    internal static string SpawnMap(int mapId)
    {
        ...
        try
        {
            ((MonoBehaviour)AmongUsClient.Instance).StartCoroutine(CoSpawnMap(mapId).WrapToIl2Cpp());
        }
        catch
        {
            return NocturneText.T("Не удалось.", "Failed.");
    ...
    internal static string CreateLobby()
    {
        ...
        try
        {
            ...
            if (lobby == null)
                return NocturneText.T("Не удалось создать лобби.", "Failed to create the lobby.");
    ...
    internal static string DestroyLobby()
    {
        ...
        if (lobby == null)
            return NocturneText.T("Объекта лобби сейчас нет.", "No lobby object right now.");
    ...
    private static bool IsHost()
    {
        try
        {
            return AmongUsClient.Instance != null && ((InnerNetClient)AmongUsClient.Instance).AmHost;
        }
        catch
        {
            return false;
