...
public sealed class NocturneMusicPlayer : MonoBehaviour
{
    ...
    {
        public float[] Samples; public int Channels; public int Frequency;
    }
    ...
    {
        get => _shuffle; set => _shuffle = value;
    }
    ...
    private Rect _winRect = new Rect(80f, 80f, 460f, 198f);
    ...
    private GUIStyle _displayBakeStyle;
    private static Texture2D _displayBakeTex;
    private GUIStyle _controlsBarStyle;
    private GUIStyle _brandLabelStyle;

    private const float kDisplayW = 460f;
    private const float kDisplayH = 80f;
    private const float kWinH = 198f;
    private static readonly float[] kShadowAlpha = { 0.28f, 0.15f, 0.06f };
    private static readonly float[] kTrailAlpha = { 0.13f, 0.07f, 0.035f };
    ...
    private void Update()
    {
        ...
        {
            if (!_visible && _closingAt < 0f)
                BeginPlayerOpen();
            else if (_visible && _closingAt < 0f)
                BeginPlayerClose();
        }
        ...
        if (noTextFieldFocused && !chatOpen && !NocturneMenu.Rebinding)
        {
            ...
            if (NocturneConfig.MusicPrevKey.Value != KeyCode.None && UnityEngine.Input.GetKeyDown(NocturneConfig.MusicPrevKey.Value))
                Prev();
            if (NocturneConfig.MusicNextKey.Value != KeyCode.None && UnityEngine.Input.GetKeyDown(NocturneConfig.MusicNextKey.Value))
                Next();
            if (NocturneConfig.MusicPlayPauseKey.Value != KeyCode.None && UnityEngine.Input.GetKeyDown(NocturneConfig.MusicPlayPauseKey.Value))
                PlayPause();
            if (NocturneConfig.MusicStopKey.Value != KeyCode.None && UnityEngine.Input.GetKeyDown(NocturneConfig.MusicStopKey.Value))
                Stop();
            float volDelta = 0f;
            if (NocturneConfig.MusicVolumeUpKey.Value != KeyCode.None && UnityEngine.Input.GetKey(NocturneConfig.MusicVolumeUpKey.Value))
                volDelta += dt0 * 0.6f;
            if (NocturneConfig.MusicVolumeDownKey.Value != KeyCode.None && UnityEngine.Input.GetKey(NocturneConfig.MusicVolumeDownKey.Value))
                volDelta -= dt0 * 0.6f;
            if (volDelta != 0f)
                Volume = Mathf.Clamp01(_volume + volDelta);
        }
        ...
        if (_source == null || _isLoading || _source.isPlaying || _source.clip == null)
            return;
    ...
    internal void DrawGui()
    {
        ...
        if (_toastTimer > 0f && _toastText.Length > 0 && Event.current.type == EventType.Repaint)
        {
            EnsureStyles();
            GUI.skin = null;
        ...
        Rect drawRect = new Rect(_winRect.x, _winRect.y + offsetY, 460f, kWinH);
        Rect nextRect = drawRect;
        try
        {
            if (_winFn == null)
                _winFn = (GUI.WindowFunction)DrawWindow;
            nextRect = GUI.Window(772100, drawRect, _winFn,
        ...
        if (_eqSeg != null && Event.current.type == EventType.Repaint)
        {
            ...
            {
                float o = (s + 1) * 2f;
                GUI.color = new Color(0f, 0f, 0f, kShadowAlpha[s] * alpha * beatMul);
        ...
        if (_showPlaylist || _listClosingAt >= 0f)
        {
            ...
            float listBody = TrackCount == 0 ? 60f : Mathf.Min(TrackCount * 30f + 4f, 220f);
            _listRect.height = 31f + listBody;
            ...
            {
                if (_listFn == null)
                    _listFn = (GUI.WindowFunction)DrawPlaylistWindow;
                nextList = GUI.Window(772101, listDraw, _listFn,
                    string.Empty, _winStyle);
            }
            catch { }
        ...
        if (_showEqPanel || _eqClosingAt >= 0f)
        {
            ...
            {
                if (_eqFn == null)
                    _eqFn = (GUI.WindowFunction)DrawEqPanel;
        ...
        if (waveform && _waveformHeights != null && _eqSeg != null && Event.current.type == EventType.Repaint)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (frac <= v)
                {
                    ...
                    if (distSlots < 1.5f)
                        c = Color.Lerp(c, WithA(_clrScrubFillHi, _winAlpha), 1f - distSlots / 1.5f);
                }
                else
                {
                    c = WithA(_clrScrubDim, 0.72f * _winAlpha);
                    if (distSlots < 1.5f)
                        c = Color.Lerp(c, WithA(_clrScrubDimHi, 0.85f * _winAlpha), 1f - distSlots / 1.5f);
                }
                GUI.color = c;
    ...
    private void DrawWindow(int _)
    {
        ...
        Rect titleRow = new Rect(0f, 0f, kWinW, 26f);
        ...
        Rect accentRow = new Rect(0f, 26f, kWinW, 2f);
        ...
        if (_waveformHeights == null)
            ComputeWaveform(-1);
        Rect seekRow = new Rect(0f, 28f, kWinW, 34f);
        ...
        if (_eqSeg != null)
        {
            var ocZ = GUI.color;
            GUI.color = new Color(1f, 1f, 1f, 0.07f * _winAlpha);
            GUI.Box(new Rect(seekRow.x, seekRow.yMax, seekRow.width, 1f), GUIContent.none, _eqSeg);
            GUI.color = ocZ;
        }

        UpdateArtTex();
        Rect dispRow = new Rect(0f, 62f, kWinW, 80f);
        GUI.Box(dispRow, GUIContent.none, _displayBakeStyle);
        ...
        if (_eqSeg != null)
        {
            var ocZ2 = GUI.color;
            GUI.color = new Color(1f, 1f, 1f, 0.07f * _winAlpha);
            GUI.Box(new Rect(dispRow.x, dispRow.yMax, dispRow.width, 1f), GUIContent.none, _eqSeg);
            GUI.color = ocZ2;
        }

        Rect statusRow = new Rect(0f, 142f, kWinW, 18f);
        ...
        if (_eqSeg != null)
        {
            var ocZ3 = GUI.color;
            GUI.color = new Color(1f, 1f, 1f, 0.07f * _winAlpha);
            GUI.Box(new Rect(statusRow.x, statusRow.yMax, statusRow.width, 1f), GUIContent.none, _eqSeg);
            GUI.color = ocZ3;
        }

        Rect ctrlRow = new Rect(0f, 160f, kWinW, 38f);
        GUI.Box(ctrlRow, GUIContent.none, _controlsBarStyle);
        float cx = ctrlRow.x + 6f;
        float cy = ctrlRow.y + 6f;
        if (HoverButton(new Rect(cx, cy, 24f, 26f), "|<", _btnStyle))
            Prev();
        if (HoverButton(new Rect(cx + 26f, cy, 28f, 26f), "-10", _btnStyle))
        {
            if (_source != null)
                _source.time = Mathf.Max(0f, _source.time - 10f);
        }
        ...
        if (HoverButton(new Rect(cx + 56f, cy, 76f, 26f), playLabel, _btnPlay))
            PlayPause();
        GUI.color = ocP;
        if (HoverButton(new Rect(cx + 134f, cy, 28f, 26f), "+10", _btnStyle))
        {
            if (_source != null)
                _source.time = Mathf.Min(Mathf.Max(0f, TotalTime - 0.5f), _source.time + 10f);
        }
        if (HoverButton(new Rect(cx + 164f, cy, 24f, 26f), ">|", _btnStyle))
            Next();
        if (HoverButton(new Rect(cx + 190f, cy, 22f, 26f), "■", _btnStop))
            Stop();
        ...
        if (HoverButton(new Rect(rx - 66f, cy + 1f, 34f, 24f), "MIX",
                _shuffle ? _btnToggleOn : _btnToggleOff))
            Shuffle = !_shuffle;
        if (HoverButton(new Rect(rx - 104f, cy + 1f, 34f, 24f), "RPT",
                _loop ? _btnToggleOn : _btnToggleOff))
            Loop = !_loop;
    ...
    private void DrawPlaylistWindow(int _)
    {
        ...
        Rect hdr = new Rect(0f, ly, kPW, 26f);
        ly += 26f;
        ...
        Rect div = new Rect(0f, ly, kPW, 1f);
        ly += 1f;
        if (_dividerStyle != null)
            GUI.Box(div, GUIContent.none, _dividerStyle);

        if (TrackCount == 0)
        {
            ly += 8f;
            Rect emptyR = new Rect(0f, ly, kPW, 44f);
            ly += 44f;
            if (_emptyStyle != null)
                GUI.Label(emptyR,
        ...
        else
        {
            ...
            Rect scrollArea = new Rect(0f, ly, kPW, listH);
            ly += listH;
    ...
    {
        if (Event.current.type != EventType.Repaint || _eqSeg == null)
            return;
        const int kBars = 14;
    ...
    {
        if (_trackLabelStyle == null || w < 4f)
            return;
        Rect clip = new Rect(x, y, w, h);

        if (_marqueeContent == null)
            _marqueeContent = new GUIContent(text);
        else if (_marqueeContent.text != text)
            _marqueeContent.text = text;
    ...
    {
        if (idx < 0 || idx >= _trackNames.Count)
            return "♪";
        string name = _trackNames[idx];
        int i = 0;
        while (i < name.Length && (char.IsDigit(name[i]) || name[i] == ' '))
            i++;
                              ...
                              Color discColor, string letter, float rotationDeg, bool drawHalo)
    {
        ...
        if (drawHalo && _eqHeights != null && _artBgStyle != null)
        {
            ...
            for (int i = 0; i < nBars; i++)
            {
                ...
                if (specIdx >= _eqHeights.Length)
                    specIdx = _eqHeights.Length - 1;
    ...
    private void EnsureStyles()
    {
        ...
        if (acc != _appliedAccent)
        {
            _appliedAccent = acc;
            _stylesDirty = true;
        }
        if (!_stylesDirty && _bgTex != null)
            return;
        _stylesDirty = false;
        try
        {
    ...
    private void BuildStyles()
    {
        ...
        float hue = AccentHue();
        cBg = Rehue(cBg, hue);
        cHeader = Rehue(cHeader, hue);
        cPanel = Rehue(cPanel, hue);
        cListHdr = Rehue(cListHdr, hue);
        cAccent = Rehue(cAccent, hue);
        cAccentHov = Rehue(cAccentHov, hue);
        cAccentDim = Rehue(cAccentDim, hue);
        cBtn = Rehue(cBtn, hue);
        cBtnHov = Rehue(cBtnHov, hue);
        cBtnStop = Rehue(cBtnStop, hue);
        cTrackRow = Rehue(cTrackRow, hue);
        cTrackHov = Rehue(cTrackHov, hue);
        cTrackAct = Rehue(cTrackAct, hue);
        cProgBg = Rehue(cProgBg, hue);
        cThumb = Rehue(cThumb, hue);
        ...
        _headerStyle.padding.left = 12;
        _headerStyle.padding.right = 4;
        _headerStyle.padding.top = 2;
        _headerStyle.padding.bottom = 2;
        ...
        _listHeaderStyle.padding.left = 12;
        _listHeaderStyle.padding.right = 4;
        _listHeaderStyle.padding.top = 4;
        _listHeaderStyle.padding.bottom = 4;
        ...
        if (_displayBakeTex == null)
            _displayBakeTex = MakeDisplayBgTex((int)kDisplayW, (int)kDisplayH, new Color(0.040f, 0.028f, 0.040f, 1f));
        _displayBakeStyle = new GUIStyle(GUI.skin.box);
        _displayBakeStyle.normal.background = _displayBakeTex;
        _displayBakeStyle.padding.left = _displayBakeStyle.padding.right = 0;
        _displayBakeStyle.padding.top = _displayBakeStyle.padding.bottom = 0;
        _displayBakeStyle.margin.left = _displayBakeStyle.margin.right = 0;
        _displayBakeStyle.margin.top = _displayBakeStyle.margin.bottom = 0;
        _displayBakeStyle.border.left = _displayBakeStyle.border.right = 0;
        _displayBakeStyle.border.top = _displayBakeStyle.border.bottom = 0;
        ...
        _trackNormal.margin.top = 1;
        _trackNormal.margin.bottom = 1;
        ...
        _cornerStyleTL = new GUIStyle(GUIStyle.none);
        _cornerStyleTL.normal.background = _cornerTL;
        _cornerStyleTR = new GUIStyle(GUIStyle.none);
        _cornerStyleTR.normal.background = _cornerTR;
        _cornerStyleBL = new GUIStyle(GUIStyle.none);
        _cornerStyleBL.normal.background = _cornerBL;
        _cornerStyleBR = new GUIStyle(GUIStyle.none);
        _cornerStyleBR.normal.background = _cornerBR;
    }
    ...
    private static Texture2D MakeDisplayBgTex(int w, int h, Color baseCol)
    {
        ...
        {
            float topFrac = (float)(h - 1 - ty) / (h - 1);
            float botFrac = (float)ty / (h - 1);
            Color row = baseCol;
            if (topFrac < edge)
                row = Color.Lerp(row, Color.white, 0.055f * (1f - topFrac / edge));
            if (botFrac < edge)
                row = Color.Lerp(row, Color.black, 0.07f * (1f - botFrac / edge));
            for (int x = 0; x < w; x++)
                px[ty * w + x] = row;
        }
        for (int sy = 6; sy < h - 2; sy += 8)
        {
            int ty = h - 1 - sy;
            if (ty < 0 || ty >= h)
                continue;
            for (int x = 5; x < w - 2; x += 9)
                px[ty * w + x] = Color.Lerp(px[ty * w + x], Color.white, 0.045f);
        }
        t.SetPixels(px);
        t.Apply();
        UnityEngine.Object.DontDestroyOnLoad(t);
        ...
        try
        {
            ...
            if (ext == ".wav")
                using (var r = new WaveFileReader(path))
                    return (float)r.TotalTime.TotalSeconds;
            if (ext == ".ogg")
                using (var vr = new VorbisReader(path))
                    return (float)vr.TotalTime.TotalSeconds;
        }
        catch { }
        return 0f;
    }
    ...
    private void PrefetchNeighbors(int center)
    {
        ...
        if (!_loadedRaw.ContainsKey(next))
            this.StartCoroutine(PrefetchTrack(next));
        if (prev != next && !_loadedRaw.ContainsKey(prev))
            this.StartCoroutine(PrefetchTrack(prev));
    }
    ...
    {
        if (index < 0 || index >= _tracks.Count || _loadedRaw.ContainsKey(index))
            yield break;
    ...
    internal void Stop()
    {
        ...
        if (_source.clip != null)
        {
            try
            {
                UnityEngine.Object.Destroy(_source.clip);
            }
            catch { }
            _source.clip = null;
        }
    ...
    {
        if (_source == null || raw == null || raw.Samples == null || raw.Channels < 1)
            return;
        ...
        if (startCrossfade)
        {
            ...
            {
                _eqX1B = _eqX2B = _eqY1B = _eqY2B = null;
            }
            if (_bassEnhState != null)
                _bassEnhStateB = (float[])_bassEnhState.Clone();
            if (_lcBassState != null)
                _lcBassStateB = (float[])_lcBassState.Clone();
            if (_deessLpfLow != null)
                _deessLpfLowB = (float[])_deessLpfLow.Clone();
            if (_deessLpfHigh != null)
                _deessLpfHighB = (float[])_deessLpfHigh.Clone();
            if (_deessEnv != null)
                _deessEnvB = (float[])_deessEnv.Clone();
            if (_deessGain != null)
                _deessGainB = (float[])_deessGain.Clone();
    ...
    private void EnsureEqState()
    {
        ...
        {
            Array.Clear(_eqX1, 0, needed);
            Array.Clear(_eqX2, 0, needed);
            Array.Clear(_eqY1, 0, needed);
            Array.Clear(_eqY2, 0, needed);
        }

        if (_bassEnhState == null || _bassEnhState.Length != _rawChannels)
            _bassEnhState = new float[_rawChannels];
        else
            Array.Clear(_bassEnhState, 0, _rawChannels);
        if (_lcBassState == null || _lcBassState.Length != _rawChannels)
            _lcBassState = new float[_rawChannels];
        else
            Array.Clear(_lcBassState, 0, _rawChannels);
    ...
    private void RecalculateEqCoefficients()
    {
        ...
        bool flat = true;
        for (int b = 0; b < _eqGains.Length; b++)
            if (_eqGains[b] != 0f)
            {
                flat = false;
                break;
        ...
        var b0c = new double[kB];
        var b1c = new double[kB];
        var b2c = new double[kB];
        var a1c = new double[kB];
        var a2c = new double[kB];
        for (int b = 0; b < kB; b++)
        {
            float gEff = SoftKneeEq(_eqGains[b], b);
    ...
    private void PcmReaderCallback(Il2CppStructArray<float> data)
    {
        ...
        float bassEnhScale = 1f;
        {
            ...
            if (gainsSnapshot[1] > bassBoostMax)
                bassBoostMax = gainsSnapshot[1];
            if (gainsSnapshot[2] > bassBoostMax)
                bassBoostMax = gainsSnapshot[2];
            if (bassBoostMax > 3f)
            {
        ...
        var y1B = _eqY1B;
        var y2B = _eqY2B;
        var bsB = _bassEnhStateB;
        var lcB = _lcBassStateB;
        ...
        for (int f = 0; f < framesNow; f++)
        {
            ...
            if (stereo)
            {
                ...
                if (aBandL > _deessEnv[0])
                    _deessEnv[0] = aBandL;
                else
                    _deessEnv[0] *= 0.99955f;
                float tgL;
                if (_deessEnv[0] > kDeessThreshold)
                {
                    ...
                    if (tgL < kDeessMinGain)
                        tgL = kDeessMinGain;
                }
                else
                    tgL = 1f;
                if (tgL < _deessGain[0])
                    _deessGain[0] += (tgL - _deessGain[0]) * 0.55f;
                else
                    _deessGain[0] += (tgL - _deessGain[0]) * 0.018f;
                ...
                if (aBandR > _deessEnv[1])
                    _deessEnv[1] = aBandR;
                else
                    _deessEnv[1] *= 0.99955f;
                float tgR;
                if (_deessEnv[1] > kDeessThreshold)
                {
                    ...
                    if (tgR < kDeessMinGain)
                        tgR = kDeessMinGain;
                }
                else
                    tgR = 1f;
                if (tgR < _deessGain[1])
                    _deessGain[1] += (tgR - _deessGain[1]) * 0.55f;
                else
                    _deessGain[1] += (tgR - _deessGain[1]) * 0.018f;
                ...
                if (xfd)
                {
                    ...
                    if (rawB != null && posB < totB && chB == 2)
                    {
                        ...
                        if (_deessLpfLowB != null && _deessLpfHighB != null)
                        {
                            ...
                            if (aBBL > _deessEnvB[0])
                                _deessEnvB[0] = aBBL;
                            else
                                _deessEnvB[0] *= 0.99955f;
                            float tBL;
                            if (_deessEnvB[0] > kDeessThreshold)
                            {
                                ...
                                if (tBL < kDeessMinGain)
                                    tBL = kDeessMinGain;
                            }
                            else
                                tBL = 1f;
                            if (tBL < _deessGainB[0])
                                _deessGainB[0] += (tBL - _deessGainB[0]) * 0.55f;
                            else
                                _deessGainB[0] += (tBL - _deessGainB[0]) * 0.018f;
                            ...
                            if (aBBR > _deessEnvB[1])
                                _deessEnvB[1] = aBBR;
                            else
                                _deessEnvB[1] *= 0.99955f;
                            float tBR;
                            if (_deessEnvB[1] > kDeessThreshold)
                            {
                                ...
                                if (tBR < kDeessMinGain)
                                    tBR = kDeessMinGain;
                            }
                            else
                                tBR = 1f;
                            if (tBR < _deessGainB[1])
                                _deessGainB[1] += (tBR - _deessGainB[1]) * 0.55f;
                            else
                                _deessGainB[1] += (tBR - _deessGainB[1]) * 0.018f;
                ...
                if (target < limGain)
                    limGain += (target - limGain) * 0.55f;
                else
                    limGain += (target - limGain) * 0.012f;
                int wIdx = limWP * 2;
    ...
    private void PcmSetPositionCallback(int newFrame)
    {
        ...
        if (_eqX1 != null)
        {
            Array.Clear(_eqX1, 0, _eqX1.Length);
            Array.Clear(_eqX2, 0, _eqX2.Length);
            Array.Clear(_eqY1, 0, _eqY1.Length);
            Array.Clear(_eqY2, 0, _eqY2.Length);
        }
        if (_bassEnhState != null)
            Array.Clear(_bassEnhState, 0, _bassEnhState.Length);
        if (_lcBassState != null)
            Array.Clear(_lcBassState, 0, _lcBassState.Length);
        if (_deessLpfLow != null)
            Array.Clear(_deessLpfLow, 0, _deessLpfLow.Length);
        if (_deessLpfHigh != null)
            Array.Clear(_deessLpfHigh, 0, _deessLpfHigh.Length);
        if (_deessEnv != null)
            Array.Clear(_deessEnv, 0, _deessEnv.Length);
        if (_deessGain != null)
            for (int i = 0; i < _deessGain.Length; i++)
                _deessGain[i] = 1f;
        if (_cfLpf != null)
            Array.Clear(_cfLpf, 0, _cfLpf.Length);
        if (_dcX1 != null)
            Array.Clear(_dcX1, 0, _dcX1.Length);
        if (_dcY1 != null)
            Array.Clear(_dcY1, 0, _dcY1.Length);
        _sideBassState = 0f;
        if (_limBuf != null)
            Array.Clear(_limBuf, 0, _limBuf.Length);
        _limEnvPeak = 0f;
        _limCurrGain = 1f;
    }
