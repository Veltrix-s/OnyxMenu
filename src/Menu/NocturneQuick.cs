...
internal sealed class QuickItem
{
    ...
    internal QuickItem(string id, string ru, string en, ConfigEntry<bool> cfg)
    {
        Id = id;
        Ru = ru;
        En = en;
...
internal static class NocturneQuick
{
    ...
    internal static List<string> FavIds()
    {
        ...
        {
            string p = part.Trim();
            if (p.Length > 0 && ById(p) != null && !_favs.Contains(p))
                _favs.Add(p);
        }
        return _favs;
    }
