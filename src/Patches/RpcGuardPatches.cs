...
internal static class NocturneRpcGuard
{
    ...
    private static float _shipAt = -1f;
    ...
    {
        if (!NocturneConfig.RpcGuard.Value || AmongUsClient.Instance == null)
            return false;
        try
        {
    ...
    internal static bool RealVent(int id)
    {
        try
        {
            ...
            if (s == null || s.AllVents == null)
                return true;
            foreach (Vent v in s.AllVents)
                if (v != null && v.Id == id)
                    return true;
            return false;
        }
    ...
    internal static bool GraceOver()
    {
        ...
        if (_shipAt < 0f)
            _shipAt = Time.realtimeSinceStartup;
        return Time.realtimeSinceStartup - _shipAt >= 3f;
    }
    ...
    internal static void Flag(PlayerControl actor, string reason)
    {
        try
        {
            if (actor == null || actor == PlayerControl.LocalPlayer)
                return;
            ...
            if (Hit.TryGetValue(cid, out float t) && now - t < 6f)
                return;

            if (!StrikeAt.TryGetValue(cid, out float sa) || now - sa > StrikeWindow)
                Strikes[cid] = 0;
            StrikeAt[cid] = now;
            Strikes[cid] = (Strikes.TryGetValue(cid, out int c) ? c : 0) + 1;
...
internal static class NocturneRpcGuardPlayer
{
    ...
    {
        if (!NocturneRpcGuard.On() || __instance == null || __instance == PlayerControl.LocalPlayer || __instance.Data == null || reader == null)
            return true;
        ...
        try
        {
            ...
            {
                if (reader.ReadBoolean() && NocturneRpcGuard.IsImp(__instance))
                {
                    cheat = true;
                    reason = NocturneText.T("скан импостером", "scan by impostor");
                }
            }
            else if (callId == (byte)RpcCalls.PlayAnimation)
            {
                reader.ReadByte();
                if (NocturneRpcGuard.IsImp(__instance))
                {
                    cheat = true;
                    reason = NocturneText.T("таск-анимация импостером", "task anim by impostor");
                }
            }
        ...
        if (cheat)
        {
            NocturneRpcGuard.Flag(__instance, reason);
            return false;
        }
        return true;
    }
...
internal static class NocturneRpcGuardVent
{
    ...
    {
        if (!NocturneRpcGuard.On() || __instance == null || reader == null)
            return true;
        if (callId != (byte)RpcCalls.EnterVent && callId != (byte)RpcCalls.ExitVent)
            return true;

        PlayerControl actor = __instance.myPlayer;
        if (actor == null || actor == PlayerControl.LocalPlayer || actor.Data == null)
            return true;
        ...
        try
        {
            ...
            if (!NocturneRpcGuard.RealVent(ventId))
            {
                cheat = true;
                reason = NocturneText.T("фейк vent id", "fake vent id");
            }
            else if (NocturneRpcGuard.GraceOver() && !actor.Data.IsDead && actor.Data.Role != null && !actor.Data.Role.CanVent)
            {
                cheat = true;
                reason = NocturneText.T("вент без права", "vent without permission");
            }
        }
        catch { }
        finally { try { reader.Position = pos; } catch { } }
        ...
        if (cheat)
        {
            NocturneRpcGuard.Flag(actor, reason);
            return false;
        }
        return true;
    }
...
internal static class NocturneRpcGuardShip
{
    private static bool Prefix(ShipStatus __instance, [HarmonyArgument(0)] byte callId, [HarmonyArgument(1)] MessageReader reader)
    {
        ...
        try
        {
            ...
            {
                if (NocturneRpcGuard.IsHS())
                {
                    block = true;
                    reason = NocturneText.T("двери в H&S", "doors in H&S");
                }
            }
            else if (callId == (byte)RpcCalls.UpdateSystem)
            {
                ...
                if (sys == (int)SystemTypes.Ventilation)
                {
                    ...
                    if (op == 2)
                    {
                        if (!NocturneRpcGuard.RealVent(vid))
                        {
                            block = true;
                            reason = NocturneText.T("фейк vent id", "fake vent id");
                        }
                        else if (NocturneRpcGuard.GraceOver() && actor != null && actor != PlayerControl.LocalPlayer && actor.Data != null
        ...
        if (block)
        {
            if (actor != null)
                ...
                NocturneToast.Push(NocturneText.T("Защита", "Protection"), reason, 2.5f, NocturneNotifyKind.Warning);
            return false;
        }
        return true;
    }
