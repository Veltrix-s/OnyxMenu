...
public sealed class NocturneToast : MonoBehaviour
{
    private sealed class Entry
    {
        ...
        internal string TitleFit;
        internal float TitleFitW = -1f;
        internal int TitleFitN;
    }
    ...
    internal static void Push(string title, string detail, float life, NocturneNotifyKind kind)
    {
        ...
        float dur = Mathf.Clamp(life, 1.25f, 8f);

        for (int i = Entries.Count - 1; i >= 0; i--)
        {
            Entry old = Entries[i];
            if (old == null || old.Kind != kind || old.Title != t || old.Detail != d)
                continue;
    ...
    public void Update()
    {
        ...
        for (int i = Entries.Count - 1; i >= 0; i--)
        {
            if (Entries[i] == null || now - Entries[i].CreatedAt > Entries[i].Duration) Entries.RemoveAt(i);
        }
    }
    ...
    internal void DrawGui()
    {
        ...
        float now = Time.unscaledTime;
        float scaleUi = Mathf.Clamp(NocturneConfig.HudScale.Value, 0.6f, 2f);
    ...
    [HideFromIl2Cpp]
    private string FitTitle(Entry e, float w)
    {
        if (e.TitleFit != null && e.TitleFitN == e.Count && Mathf.Abs(e.TitleFitW - w) < 0.5f) return e.TitleFit;

        e.TitleFitW = w;
        e.TitleFitN = e.Count;
        string s = e.Title ?? string.Empty;
        if (e.Count > 1) s += "  ×" + e.Count;
        if (_title.CalcSize(new GUIContent(s)).x <= w) return e.TitleFit = s;

        int lo = 0, hi = s.Length;
        while (lo < hi)
        {
            int mid = (lo + hi + 1) / 2;
            if (_title.CalcSize(new GUIContent(s.Substring(0, mid) + "…")).x <= w)
                lo = mid;
            else
                hi = mid - 1;
        }
        return e.TitleFit = lo > 0 ? s.Substring(0, lo).TrimEnd() + "…" : "…";
    }
    ...
    private void DrawCard(Rect rect, Entry e, float alpha, float now)
    {
        ...
        string title = FitTitle(e, textW);

        _titleShadow.normal.textColor = new Color(0f, 0f, 0f, 0.85f * alpha);
        GUI.Label(new Rect(textX + 1f, titleY + 1f, textW, 24f), title, _titleShadow);
        _title.normal.textColor = new Color(1f, 1f, 1f, alpha);
        GUI.Label(new Rect(textX, titleY, textW, 24f), title, _title);
        ...
        if (fillW > 1f) NocturneStyle.FillRounded(new Rect(trackX, trackY, fillW, 2.5f), A(bar, 0.95f * alpha), 1);
    }
