...
public sealed class NocturneJoinDetector : MonoBehaviour
{
    ...
    private void Forget()
    {
        if (_known.Count == 0 && _pending.Count == 0 && _names.Count == 0 && _seen.Count == 0 && !_primed)
            return;
        _known.Clear();
        _pending.Clear();
        _names.Clear();
    ...
    private static RecentEntry Snapshot(ClientData client)
    {
        ...
        try
        {
            e.Fc = client.FriendCode ?? string.Empty;
        }
        catch { }
        try
        {
            e.Puid = client.ProductUserId ?? string.Empty;
        }
        catch { }
        return e;
    }
    ...
    {
        if (!_seen.TryGetValue(id, out RecentEntry e))
            return;
        if (_names.TryGetValue(id, out string n))
            e.Name = n;
    ...
    private static void AnnounceJoin(ClientData client)
    {
        ...
        try
        {
            ...
            if (!showBots && NocturneAccess.IsBotPlatform(raw))
                botHide = true;
        }
        catch { }
        ...
        if (DetectOn && !botHide)
            NocturneToast.Push(mark + name, tail, 4.5f, kind);
    ...
    private void AnnounceLeave(int id)
    {
        ...
        if (DetectOn)
            NocturneToast.Push("－ " + name, NocturneText.T("вышел", "left"), 3.5f, NocturneNotifyKind.Warning);
    ...
    private static string CleanRaw(string value)
    {
        ...
        if (raw.Equals("TESTNAME", System.StringComparison.OrdinalIgnoreCase))
            return string.Empty;
