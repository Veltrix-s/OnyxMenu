...
public sealed class NocturneLobbyPranks : MonoBehaviour
{
    ...
    public void LateUpdate()
    {
        if (_animMode != 1)
            return;
        if (!IsInGame() || !IsHost() || PlayerControl.AllPlayerControls == null)
            return;
        ...
        try
        {
            foreach (PlayerControl pc in PlayerControl.AllPlayerControls)
            {
                if (!Valid(pc) || !_egged.Contains(pc.PlayerId))
                    continue;
                try
                {
    ...
    {
        if (_rainbow || _skinCycle || !_rolesGranted)
            return;
    ...
    private static PlayerControl ById(byte id)
    {
        try
        {
            ...
            while (e.MoveNext())
                if (e.Current != null && e.Current.PlayerId == id)
                    return e.Current;
        }
        catch { }
        return null;
