...
public sealed class NocturneGlichRooms : MonoBehaviour
{
    private enum Step
    {
    ...
    internal static NocturneGlichRooms Instance { get; private set; }
    ...
    private static void MoveOldFile()
    {
        try
        {
            string old = Path.Combine(Dir, "BugRooms.txt");
            if (File.Exists(old) && !File.Exists(Txt)) File.Move(old, Txt);
        }
        catch { }
    }
    ...
    private static bool Wild()
    {
        ...
        try
        {
            n = Object.FindObjectsOfType<PassiveButton>().Length;
        }
        catch (Exception e)
        {
            Say("СКАН УПАЛ: " + e.Message);
        }
    ...
    {
        if (Patches.NocturneMainArtDriver.Live != null)
            return Patches.NocturneMainArtDriver.Live;

        if (Held != null && ((Component)Held).gameObject != null) return Held;
    ...
    {
        NocturnePlugin.Logger?.LogInfo((object)("[GlichRooms] " + what));
    }
    ...
    {
        string v = NocturneConfig.GlichRoomTargets.Value;
    ...
    private static bool Wanted(string tail)
    {
        if (tail.Length < 4)
            return false;
        foreach (string p in NocturneConfig.GlichRoomTargets.Value.Split(',', ';', ' ', '\n', '\r', '\t'))
        {
            string q = p.Trim();
    ...
    private void Tally(float now)
    {
        ...
        if (got)
        {
            if (NocturneConfig.GlichRoomLog.Value) Keep(_code);
            Hits++;
        ...
        if (got && NocturneConfig.GlichRoomHunt.Value)
        {
            NocturneConfig.GlichRoomHunt.Value = false;
            _s = Step.Wait;
            _at = now + _gap;
    ...
    public void Update()
    {
        if (!NocturneConfig.GlichRoomCycle.Value)
        {
            if (_s != Step.Off)
            {
                _s = Step.Off;
        ...
        _gap = Mathf.Clamp(NocturneConfig.GlichRoomDelay.Value, 1f, 10f);
        ...
        if (EndScreen())
        {
            if (_done)
            {
                Stage = NocturneText.T("итоги", "results");
                return;
            }
            if (_s != Step.Reading)
            {
            ...
            {
                Stage = NocturneText.T("жду уровень", "waiting level");
                return;
            }
            Tally(now);
            return;
        ...
        if (ShipStatus.Instance != null && LobbyBehaviour.Instance == null)
        {
            ...
            {
                _at = now + 1f;
                Stage = NocturneText.T("вступление", "intro");
                return;
            }
            if (now < _at)
            {
                Stage = NocturneText.T("в игре", "in game");
                return;
            }
            Finish();
            _at = now + 2.5f;
        ...
        if (LobbyBehaviour.Instance != null && gid != 0 && _s != Step.Out)
        {
            ...
            if (_code != gid)
            {
                ...
                _s = Step.Wait;
                _at = now + _gap;
                if (NocturneConfig.GlichRoomHunt.Value && !_seen.Add(_code))
                {
                    _s = Step.Out;
                    _at = now;
            ...
            if (NocturneConfig.GlichRoomHunt.Value && Listed())
            {
                string tail = Tail(gid);
                if (tail.Length < 4)
                {
                    _tail = "";
                    Stage = NocturneText.T("жду код", "waiting code");
                    return;
                }
                ...
                if (now < _firm)
                {
                    Stage = NocturneText.T("читаю код ", "reading code ") + tail;
                    return;
                }
                ...
                NocturneConfig.GlichRoomHunt.Value = false;
                Hits++;
                if (NocturneConfig.GlichRoomLog.Value)
                    Keep(gid);
                NocturneToast.Push(NocturneText.T("Баг-комната", "Bug room"),
            ...
            {
                Stage = NocturneText.T("не хост", "not host");
                return;
            }
            if (_s == Step.Started)
            {
                if (now < _at)
                {
                    Stage = NocturneText.T("старт", "starting");
                    return;
                }
                _s = Step.Wait;
                _at = now;
            ...
            if (now < _at)
            {
                Stage = NocturneText.T("пауза", "idle");
                return;
            }
            if (Launch())
            {
        ...
        bool hunt = NocturneConfig.GlichRoomHunt.Value;
        ...
        if (_s == Step.Back)
        {
            if (InRoom())
            {
                _s = Step.Wait;
                _at = now + _gap;
                Stage = NocturneText.T("в лобби", "in lobby");
                return;
            }
            if (now < _at)
            {
                if (gid != 0)
                {
                    Stage = NocturneText.T("подключаюсь", "connecting");
                    return;
                }
                if (hunt) Nudge();
                return;
            ...
            NocturneConfig.GlichRoomCycle.Value = false;
            try
            {
                GUIUtility.systemCopyBuffer = GameCode.IntToGameName(_code);
            }
            catch { }
            NocturneToast.Push(NocturneText.T("Баг-комнаты", "Glich Rooms"), NocturneText.T("Не зашёл, код скопирован.", "Rejoin failed, code copied."), 5f, NocturneNotifyKind.Danger);
            _s = Step.Off;
            return;
        }
        ...
        if (gid != 0)
        {
            Stage = NocturneText.T("жду лобби", "waiting lobby");
            return;
        }
        if (now < _at)
        {
...
internal static class NocturneGlichRoomsMenuHook
{
    public static void Postfix(MainMenuManager __instance) => NocturneGlichRooms.Held = __instance;
}
