...
internal static class NocturneSeeThrough
{
    ...
    private static void Postfix(PlayerPhysics __instance)
    {
        ...
        try
        {
            ...
            if (p == null || me == null || p.Data == null || me.Data == null)
                return;
            if (p.cosmetics == null || p == me || me.Data.IsDead)
                return;

            if (vents && p.inVent)
            {
                if (!faded.ContainsKey(p.PlayerId))
                    faded[p.PlayerId] = p.invisibilityAlpha;
                if (!p.Visible)
                {
                    p.Visible = true;
...
internal static class NocturneSeePhantoms
{
    ...
    {
        if (!NocturneConfig.SeePhantoms.Value)
            return;
...
internal static class NocturneSeeProtections
{
    ...
    {
        if (NocturneConfig.SeeProtections.Value)
            visible = true;
    }
}
