...
internal static class NocturneMainArt
{
    ...
    private static void Top()
    {
        ...
        for (int i = 0; i < all.Length; i++)
        {
            ...
            if (cp.GetComponentInParent<PoolablePlayer>() != null)
                continue;
            if (cp.GetComponentInParent<PlayerControl>() != null)
                continue;

            Vector3 vp = cam.WorldToViewportPoint(sr.bounds.center);
            if (vp.z <= 0f || vp.y < 0.88f)
                continue;

            Vector3 sz = sr.bounds.size;
            if (sz.x < 0.5f || sz.y < 0.05f)
                continue;
            Dye(sr, Ink);
        }
    ...
    private static Color TexTint(Sprite sp)
    {
        ...
        if (_tex.TryGetValue(id, out Color hit))
            return hit;
    ...
    private static void Unpaint()
    {
        foreach (KeyValuePair<int, SpriteRenderer> kv in _srRef)
            if (kv.Value != null && _wasCol.TryGetValue(kv.Key, out Color c))
                kv.Value.color = c;
        foreach (KeyValuePair<int, TMP_Text> kv in _txtRef)
            if (kv.Value != null && _wasTxt.TryGetValue(kv.Key, out Color c))
                kv.Value.color = c;
        _wasCol.Clear();
        _srRef.Clear();
        _wasTxt.Clear();
    ...
    internal static void Repeat()
    {
        ...
        for (int i = 0; i < _offGo.Count; i++)
            if (_offGo[i] != null && _offGo[i].activeSelf)
                _offGo[i].SetActive(false);
        for (int i = 0; i < _offRen.Count; i++)
            if (_offRen[i] != null && _offRen[i].enabled)
                _offRen[i].enabled = false;
    }
