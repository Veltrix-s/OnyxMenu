...
public sealed class NocturneDummies : MonoBehaviour
{
    ...
    internal static void Forget()
    {
        _bots.Clear();
        _pending.Clear();
        NocturneDummyAI.Reset();
        NocturneDummyChat.Reset();
    }
    ...
    public void FixedUpdate()
    {
        ...
        try
        {
            NocturneDummyAI.Tick(_scratch);
        }
        catch { }
        try
        {
            NocturneDummyChat.TickVision();
        }
        catch { }
        try
        {
            NocturneDummyChat.TickMeeting();
        }
        catch { }
    }
    ...
    public void Update()
    {
        ...
        {
            if (Time.unscaledTime < _pending[i].at)
                continue;
            try
            {
                NocturneForceRoles.ForceNow(_pending[i].pid);
            }
            catch { }
            _pending.RemoveAt(i);
        }
    ...
    {
        if (Instance == null)
            return NocturneText.T("Недоступно.", "Unavailable.");
    ...
    {
        if (!NocturneConfig.DummyEnabled.Value)
            return NocturneText.T("Манекены выключены.", "Dummies are off.");
        if (!CanSpawn())
            return NocturneText.T("Только хост в лобби.", "Host in lobby only.");

        try
        {
            if (PlayerControl.AllPlayerControls.Count >= Cap)
                return NocturneText.T("Лобби заполнено.", "Lobby is full.");

            AmongUsClient client = AmongUsClient.Instance;
            PlayerControl prefab = client.PlayerPrefab;
            if (prefab == null)
                return NocturneText.T("Нет префаба.", "No prefab.");
    ...
    private static void SetName(PlayerControl pc, string name)
    {
        ...
        if (client != null)
        {
            ...
            {
                if (other == null || other == PlayerControl.LocalPlayer)
                    continue;
                try
                {
        ...
        try
        {
            if (pc.cosmetics != null && pc.cosmetics.nameText != null)
                ((TMP_Text)pc.cosmetics.nameText).text = name;
        }
        catch { }
    }
    ...
    private static string RandomCosmo(int kind)
    {
        try
        {
            ...
            {
                var a = hm.allHats;
                return a.Length > 0 ? a[Random.Range(0, a.Length)].ProdId : string.Empty;
            }
            if (kind == 1)
            {
                var a = hm.allSkins;
            ...
            var v = hm.allVisors;
            return v.Length > 0 ? v[Random.Range(0, v.Length)].ProdId : string.Empty;
        }
        catch
        {
            return string.Empty;
    ...
    {
        if (NocturneText.IsRussian)
            return Ru[Random.Range(0, Ru.Length)];
        const string abc = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        var c = new char[6];
        for (int i = 0; i < c.Length; i++)
            c[i] = abc[Random.Range(0, abc.Length)];
        return new string(c);
    }
    ...
    private static void Restamp()
    {
        ...
        foreach (var kv in _bots)
        {
            ...
            if (host)
            {
                try
                {
                    d.Pc.RpcSetColor(d.Color);
                }
                catch { }
                SetName(d.Pc, d.Name);
                try
                {
                    d.Pc.RpcSetHat(d.Hat);
                }
                catch { }
                try
                {
                    d.Pc.RpcSetSkin(d.Skin);
                }
                catch { }
                try
                {
                    d.Pc.RpcSetVisor(d.Visor);
                }
                catch { }
            }
            else
            {
                try
                {
                    if (d.Pc.cosmetics != null && d.Pc.cosmetics.nameText != null)
                        ((TMP_Text)d.Pc.cosmetics.nameText).text = d.Name;
                }
                catch { }
            }
        }
...
internal static class NocturneDummyKillPatch
{
    public static bool Prefix(PlayerControl __instance, PlayerControl target)
    {
        try
        {
            if (target == null || target.Data == null || !NocturneDummies.IsDummy(target.PlayerId))
                return true;
