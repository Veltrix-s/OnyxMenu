...
internal static class VisualAssist
{
    ...
    private static void PlaceChatButton(HudManager hud)
    {
        try
        {
            ...
            for (int i = 0; i < aps.Length; i++)
            {
                ...
                if (ap == null || ((UnityEngine.Object)ap).GetInstanceID() == chatId) continue;
    ...
    private static void Reassert(PlayerControl player, string display)
    {
        try
        {
            ...
            if (AppliedNames.TryGetValue(pid, out string applied)
                && string.Equals(applied, display, StringComparison.Ordinal)
                && NextAssertAt.TryGetValue(pid, out float at) && now < at)
                return;
    ...
    internal static bool OwnsName(PlayerControl p) =>
        InfoLine || NocturneConfig.RevealRoles.Value || NocturneConfig.UnmaskShapeshifter.Value || NocturneAuthor.Is(p);
