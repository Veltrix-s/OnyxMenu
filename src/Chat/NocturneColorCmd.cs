...
internal static class NocturneColorCmd
{
    ...
    public static void Postfix(PlayerControl sourcePlayer, string chatText)
    {
        try
        {
            if (sourcePlayer == null || string.IsNullOrEmpty(chatText))
                return;
            ...
            if (sourcePlayer == PlayerControl.LocalPlayer && NocturneCommands.TryHostSelf(cmd))
                return;

            if (!NocturneConfig.ColorCmd.Value)
                return;
            if (cmd != "/c" && cmd != "/color" && cmd != "/цвет" && cmd != "/с")
                return;
            ...
            if (NocturneConfig.ColorCmdNotify.Value)
            {
    ...
    internal static int ColorId(string a)
    {
        if (string.IsNullOrEmpty(a))
            return -1;
        if (int.TryParse(a, out int n) && n >= 0 && n <= 17)
            return n;
        switch (a)
        {
            case "red":
