...
internal static class NocturneVoteKickPatch
{
    ...
    {
        if (!NocturneConfig.VoteKickProtect.Value || AmongUsClient.Instance == null)
            return true;
...
public sealed class NocturneAccessGuard : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (AmongUsClient.Instance == null || LobbyBehaviour.Instance == null)
            return;
        var net = (InnerNetClient)AmongUsClient.Instance;
        if (!net.AmHost || net.allClients == null)
            return;
        bool fg = NocturneConfig.KickFortegreen.Value;
        bool colorRes = NocturneConfig.ColorReservationsEnabled.Value;
        if (!fg && !colorRes && !NocturneConfig.AccessBanEnabled.Value && !NocturneConfig.AccessWhitelistOnly.Value
            && !NocturneConfig.AccessNickBanEnabled.Value && !NocturneConfig.MinLevelEnabled.Value && !NocturneConfig.MaxLevelEnabled.Value)
            return;
        ...
        try
        {
            ...
            while (e.MoveNext())
            {
                ...
                if (c == null)
                    continue;
                if (colorRes)
                    NocturneColorReservations.TryApplyOnJoin(c.Character);
