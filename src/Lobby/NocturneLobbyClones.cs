...
public sealed class NocturneLobbyClones : MonoBehaviour
{
    ...
    public void LateUpdate()
    {
        ...
        {
            Clone e = _clones[i];
            if (e.Go == null || e.Rends == null)
                continue;
            for (int j = 0; j < e.Rends.Length; j++)
            {
                SpriteRenderer sr = e.Rends[j];
                if (sr != null)
                    sr.transform.localPosition = e.Pose[j];
            }
        }
    }
    ...
    private void LeftClick(Vector3 world)
    {
        ...
        {
            if (_regular.Count >= NocturneConfig.LobbyCloneMax.Value)
                break;
            Vector3 p = world;
            if (count > 1)
            {
                Vector2 o = Random.insideUnitCircle * 0.6f;
                p += new Vector3(o.x, o.y, 0f);
            }
    ...
    private void Drag(Vector3 world)
    {
        ...
        {
            if (_drag.Go != null)
                _drag.Go.transform.position = world + _dragOff;
        }
        else
            _drag = null;
    }
    ...
    {
        if (!NocturneConfig.LobbyCloneGuard.Value || PlayerControl.LocalPlayer == null || _regular.Count == 0)
            return;
        ...
        {
            if (_regular[i].Go == null || _regular[i].Static)
                continue;
    ...
    {
        if (!NocturneConfig.LobbyCloneDrift.Value || NocturneConfig.LobbyCloneGuard.Value || PlayerControl.LocalPlayer == null)
            return;
        Vector3 c = PlayerControl.LocalPlayer.transform.position;
        foreach (Clone e in _regular)
        {
            if (e.Go == null || e.Static)
                continue;
            e.DirTimer -= dt;
            if (e.DirTimer <= 0f)
                RandomDrift(e);
    ...
    {
        if (!NocturneConfig.LobbyCloneShadow.Value || PlayerControl.LocalPlayer == null)
            return;
        _trail.Add((Time.time, PlayerControl.LocalPlayer.transform.position));
        float cutoff = Time.time - ShadowDelay - 0.5f;
        while (_trail.Count > 0 && _trail[0].t < cutoff)
            _trail.RemoveAt(0);
    }
    ...
    private void ManageShadow()
    {
        ...
        if (_shadow == null || _shadow.Go == null || _trail.Count == 0)
            return;
    ...
    {
        if (PlayerControl.LocalPlayer == null || _regular.Count == 0)
            return;
        Vector3 c = PlayerControl.LocalPlayer.transform.position;
    ...
    {
        if (string.IsNullOrWhiteSpace(text) || PlayerControl.LocalPlayer == null)
            return;
    ...
    private static Texture2D Capture(PlayerControl me, out float aspect)
    {
        ...
        try
        {
            try
            {
                if (me.cosmetics.nameTextContainer != null)
                    nameGo = me.cosmetics.nameTextContainer;
                else if (me.cosmetics.nameText != null)
                    nameGo = ((Component)me.cosmetics.nameText).gameObject;
                if (nameGo != null)
            ...
            {
                b = bodySr.bounds;
                mask = 1 << bodySr.gameObject.layer;
                has = true;
            }
    ...
    private static void TintBody(Clone e, Color col)
    {
        try
        {
            ...
            if (pc == null || pc.cosmetics == null || pc.cosmetics.currentBodySprite == null)
                return;
            SpriteRenderer body = pc.cosmetics.currentBodySprite.BodySprite;
            if (body != null)
                PlayerMaterial.SetColors(col, (Renderer)body);
        }
        catch { }
    }
    ...
    private static void Strip(GameObject go)
    {
        try
        {
            ...
            if (cos.hat != null)
                ((Component)cos.hat).gameObject.SetActive(false);
            if (cos.skin != null)
                ((Component)cos.skin).gameObject.SetActive(false);
            if (cos.visor != null)
                ((Component)cos.visor).gameObject.SetActive(false);
            if (cos.currentPet != null)
                ((Component)cos.currentPet).gameObject.SetActive(false);
        }
        catch { }
    }
    ...
    private static bool Wing(float x, float y, float sx, float sy, float k)
    {
        for (int i = 0; i < 4; i++)
        {
            ...
            if (Cap(x, y, sx, sy, tx, ty, 0.17f))
                return true;
            if (i == 3)
                break;
            float nx = sx + (WingTips[i * 2 + 2] - 0.4f) * k, ny = sy + (WingTips[i * 2 + 3] - 1.2f) * k;
            if (Tri(x, y, sx, sy, sx + (tx - sx) * 0.8f, sy + (ty - sy) * 0.8f, sx + (nx - sx) * 0.8f, sy + (ny - sy) * 0.8f))
                return true;
        }
        return false;
    }
    ...
    private static bool DragonTail(float x, float y)
    {
        for (int k = 0; k < 40; k++)
        {
            ...
            if (Cap(x, y, ax, ay, bx, by, 1.0f - 0.82f * t))
                return true;
        }
        return false;
    }
    ...
    {
        if (Ell(x, y, 0f, 0f, 3.0f, 1.7f))
            return true;
        if (Cap(x, y, 2.4f, 0.8f, 4.6f, 2.5f, 0.75f))
            return true;
        if (Cap(x, y, 4.6f, 2.5f, 6.4f, 3.6f, 0.6f))
            return true;
        if (Ell(x, y, 7.2f, 3.9f, 1.35f, 0.9f))
            return true;
        if (Cap(x, y, 7.9f, 3.6f, 9.3f, 3.3f, 0.4f))
            return true;
        if (Cap(x, y, 7.0f, 4.6f, 6.2f, 6.1f, 0.22f))
            return true;
        if (Wing(x, y, 0.4f, 1.2f, 1f))
            return true;
        if (Wing(x, y, -1.2f, 0.9f, 0.72f))
            return true;
        if (DragonTail(x, y))
            return true;
        if (Tri(x, y, -8.8f, 3.0f, -10.6f, 4.6f, -10.2f, 1.9f))
            return true;
        if (Cap(x, y, -1.5f, -1.3f, -2.0f, -3.2f, 0.34f))
            return true;
        if (Cap(x, y, 1.4f, -1.3f, 1.8f, -3.2f, 0.34f))
            return true;
        return false;
    }
    ...
    {
        if (_dragFor == total && _dragCells != null)
            return;
        _dragFor = total;
        ...
        for (int gy = ny - 1; gy >= 0; gy--)
            for (int gx = 0; gx < nx; gx++)
            {
                ...
                if (InDragon(px, py))
                    _dragCells.Add(new Vector2(px, py));
            }
    }
    ...
    internal static Vector3 FormationPos(int idx, int i, int total, Vector3 c)
    {
        ...
        switch (idx)
        {
            ...
            case 20:
            {
                ...
                int j = i - faceN;
                if (j == 0)
                    return new Vector3(c.x - r * 0.38f, c.y + r * 0.32f, 0f);
                if (j == 1)
                    return new Vector3(c.x + r * 0.38f, c.y + r * 0.32f, 0f);
            ...
            case 26:
            {
                ...
                if (i == 0)
                    return new Vector3(c.x + r * 0.12f, c.y + r * 0.45f, 0f);
            ...
            case 29:
            {
                ...
                if (i == 0)
                    return new Vector3(c.x, c.y - s, 0f);
            ...
            case 30:
            {
                ...
                if (t < 0.42f || t > 0.68f)
                    ...
                    y = (t - 0.42f) / 0.08f * 1.6f;
                else if (t < 0.56f)
                    y = 1.6f - (t - 0.5f) / 0.06f * 2.5f;
                else if (t < 0.62f)
                    y = -0.9f + (t - 0.56f) / 0.06f * 1.3f;
                else
                    y = 0.4f - (t - 0.62f) / 0.06f * 0.4f;
                return new Vector3(c.x + x, c.y + y, 0f);
            }
            default:
    ...
    private Clone Spawn(Vector3 pos, bool shadow)
    {
        ...
        if (NocturneConfig.CloneNaked.Value)
            Strip(go);
        pos.z = CloneZ;
    ...
    private static void CapturePose(Clone e, GameObject go)
    {
        ...
        for (int i = 0; i < rends.Length; i++)
            if (rends[i] != null)
                pose[i] = rends[i].transform.localPosition;
        e.Rends = rends;
        e.Pose = pose;
    }
    ...
    private static GameObject MakeClone(PlayerControl src)
    {
        ...
        {
            PlayerControl pc = go.GetComponent<PlayerControl>();
            if (pc != null && PlayerControl.AllPlayerControls != null)
                PlayerControl.AllPlayerControls.Remove(pc);
            if (realLocal != null && PlayerControl.LocalPlayer != realLocal)
                PlayerControl.LocalPlayer = realLocal;
        ...
        try
        {
            foreach (Component comp in go.GetComponentsInChildren<Component>(true))
            {
                switch (comp)
                {
                    case InnerNetObject net:
                        net.NetId = uint.MaxValue;
                        break;
                    case Collider2D col:
                        col.enabled = false;
                        break;
    ...
    private static void CopySprites(GameObject src, GameObject clone, int off)
    {
        try
        {
            ...
            for (int i = 0; i < d.Length; i++)
            {
                if (d[i] == null)
                    continue;
                if (!map.TryGetValue(PathOf(d[i].transform, droot), out SpriteRenderer sr) || sr == null)
                    continue;
