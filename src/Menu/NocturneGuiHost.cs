...
public sealed class NocturneGuiHost : MonoBehaviour
{
    ...
    private void Update() => NocturneGate.Tick();
    ...
    private void Bind()
    {
        ...
        if (lobby != null)
            order.Add(lobby.DrawGui);
        if (tracers != null)
            order.Add(tracers.DrawGui);
        if (over != null)
            order.Add(over.DrawGui);
        if (radar != null)
            order.Add(radar.DrawGui);
        if (bar != null)
            order.Add(bar.DrawGui);
        if (radial != null)
            order.Add(radial.DrawGui);
        if (log != null)
            order.Add(log.DrawGui);
        if (replay != null)
            order.Add(replay.DrawGui);
        if (chat != null)
            order.Add(chat.DrawGui);
        if (toast != null)
            order.Add(toast.DrawGui);
        if (btn != null)
            order.Add(btn.DrawGui);
        _draw = order.ToArray();
    }

    public void OnGUI()
    {
        if (NocturneGate.Tripped)
            return;
        Bind();
        if (_draw == null)
            return;
