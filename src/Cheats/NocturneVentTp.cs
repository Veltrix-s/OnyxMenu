...
internal static class NocturneVentTp
{
    ...
    {
        if (NocturneConfig.VentTpAuto == null || !NocturneConfig.VentTpAuto.Value) return;
        if (AmongUsClient.Instance == null || ShipStatus.Instance == null || _marked.Count == 0) return;
        if (VentCount() <= 0) return;

        float now = Time.unscaledTime;
        float delay = NocturneConfig.VentTpAutoDelay != null ? Mathf.Clamp(NocturneConfig.VentTpAutoDelay.Value, 0.3f, 10f) : 1f;
