...
public sealed class NocturneGodMode : MonoBehaviour
{
    ...
    private const float KeepGap = 3f;

    private bool _last;
    private float _keepAt;
    ...
    private static bool Apply(bool on)
    {
        PlayerControl me = PlayerControl.LocalPlayer;
        if (me == null || ShipStatus.Instance == null) return false;
        if (on && (me.Data == null || me.Data.IsDead || me.inVent))
            return false;

        return Send(on ? 2 : 3);
    }

    private static bool Send(int op)
    {
        try
        {
            VentilationSystem.Update((VentilationSystem.Operation)op, VentId);
            return true;
        }
        catch
        {
