...
public sealed class NocturneLobby : MonoBehaviour
{
    ...
    private int _moonAccent = int.MinValue;
    private GUIStyle _moonStyle;

    internal void DrawGui()
    {
        if (!NocturneConfig.LobbyBrand.Value)
            return;
        ...
        DrawMoon(new Rect(badge.x + 20f, badge.center.y - 15f, 30f, 30f), accent);
    ...
    private void DrawMoon(Rect box, Color accent)
    {
        ...
        int key = ((int)(accent.r * 255f) << 16) | ((int)(accent.g * 255f) << 8) | (int)(accent.b * 255f);
        if (_moonTex == null || _moonAccent != key)
        {
            if (_moonTex != null)
                Object.Destroy(_moonTex);
            _moonTex = BuildMoon(accent);
            _moonAccent = key;
        }
        if (_moonStyle == null)
            _moonStyle = new GUIStyle();
