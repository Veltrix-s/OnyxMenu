...
public sealed class NocturneColorSnipe : MonoBehaviour
{
    ...
    internal static bool Taken(int id, PlayerControl self)
    {
        try
        {
            ...
            while (e.MoveNext())
            {
                ...
                if (p == null || p == self || p.Data == null || p.Data.Disconnected)
                    continue;
                if (p.Data.DefaultOutfit != null && p.Data.DefaultOutfit.ColorId == id)
                    return true;
            }
        }
        catch { }
    ...
    public void Update()
    {
        if (!NocturneConfig.SnipeColor.Value || LobbyBehaviour.Instance == null)
        {
            _lastTry = -1;
            return;
        }
        ...
        if (me == null || me.Data == null || me.Data.DefaultOutfit == null)
            return;
        if (Time.time < _next)
            return;
